...
* Changed `create-models` to validate and render generated files before replacing output, so validation/rendering failures do not leave refreshed generated files on disk.
...
* Added `datalinq create-models --stamp-generated-header` to opt into CLI version and UTC generation timestamp stamp lines in generated model files.
...
* **Use header stamping deliberately.** `create-models --stamp-generated-header` records the CLI version and one UTC timestamp for the generation run. That is useful for provenance, but it creates timestamp-only diffs every time you regenerate.
