...
* **Validation and generation errors are reported more completely.**
  CLI validation now reports every independent issue it can reach, JSON output exposes structured `issues`, `diff` writes no SQL when validation is not trustworthy, and `create-models` validates/renders before replacing generated files. The source generator also reports focused diagnostics and keeps useful output for unaffected scopes where that is safe.

* **Generated C# files now identify themselves and declare nullable context.**
  Generated model/database files start with the DataLinq generated-file banner and an explicit `#nullable` directive. Nullable reference generation now defaults on, with `"UseNullableReferenceTypes": false` as the explicit compatibility opt-out.

* **The CLI and config workflow is much less ad hoc.**
  The command surface is now grouped under `generate`, `database`, `config`, and `secrets`; configs can be initialized, listed, schema-wired, validated, and run recursively; local secrets can stay out of committed JSON; and model regeneration now has a single `ModelDirectory` workflow instead of split source/destination paths.
...
* Added structured validation `issues` to JSON output and line/column-aware issue formatting for text output when source spans can be mapped.
* Changed validation and metadata-loading paths to report all reachable independent issues instead of collapsing most failures to the first branch-local error.
...
* Changed `diff` to report validation issues and write no SQL output when validation cannot produce a trustworthy comparison.
* Changed `generate models` and the deprecated `create-models` alias to validate and render generated files before replacing output, so validation/rendering failures do not leave refreshed generated files on disk.
* Kept destructive, ambiguous, unsupported, and provider-specific changes as manual-review comments in generated diff output.
* Clarified schema drift output by grouping validation differences by severity and showing clearer drift summaries.
* Normalized typed default comparisons, including numeric default literals and enum defaults, so equivalent provider/model defaults do not drift just because one side prints `0D`, `(sbyte)0`, or an enum member name.
* Aggregated existing-index validation errors instead of hiding later index issues behind the first failure.
* Limited provider metadata warnings to imported tables when an `Include` filter is configured, while still warning when an included table references an excluded table.
...
* Added generated C# file preambles with `<auto-generated />`, a DataLinq do-not-edit banner, and an explicit `#nullable enable` or `#nullable disable` directive.
* Added `datalinq generate models --stamp-generated-header` to opt into CLI version and UTC generation timestamp stamp lines in generated model files.
* Changed `UseNullableReferenceTypes` to default on for generated files while preserving explicit `"UseNullableReferenceTypes": false` opt-out behavior.
* Taught source-generator output to follow source-file nullable context from model/database declarations instead of only project-level nullable settings.
* Fixed source-generator emission when source models use `#nullable disable`, including generated null-forgiving initializers and nullable reference annotations.
* Changed source-generator emission to report multiple focused diagnostics and preserve safe table output for unaffected scopes while suppressing incomplete database bootstraps.
* Added generated model layout config through `ModelLayout.PropertyOrder`, `PrimaryKeyPlacement`, `ForeignKeyPlacement`, and `RelationPlacement`.
* Changed generated database model properties to use target-typed `DbRead` initialization.
* Preserved source namespaces, source usings, qualified custom value property types, external enum ownership, and nullable relation metadata during regeneration.
* Restricted generated enum attributes to database-backed enum columns and added support for numeric database columns mapped to custom enum types.
...
### CLI Configuration and Regeneration Workflow

* Migrated the CLI to a nested command surface: `generate models`, `generate sql`, `database create`, `config init`, `config list`, `config schema`, `config validate`, and `secrets` commands.
* Kept `create-models` as a deprecated compatibility command, while empty CLI invocation now prints help instead of trying to generate models.
* Added `--all` and `--recursive` batch target support for model generation and validation, plus recursive config discovery for inventory and config-only validation.
* Added batch generation preflight so one bad recursive target does not leave another target partially rewritten.
* Added `ModelDirectory` as the active regeneration directory. Existing model files are read from the same directory that regenerated files are written to.
* Kept `DestinationDirectory` as a compatibility alias for `ModelDirectory`, while `SourceDirectories` is deprecated and ignored by active generation/validation/diff workflows.
* Added `--fresh` for database-only regeneration and `--overwrite-types` for explicitly replacing preserved C# property types.
* Added smart `config init` for new projects, missing user configs, existing configs, and orphaned user configs.
* Added JSON Schema publication through `config schema`, including default `datalinq.schema.json` file output and `--update-config` wiring for existing config files.
* Added strict config reading. Unknown properties and unsupported option values now fail loudly instead of being silently ignored.
* Added `config validate` and `config validate --recursive` to validate `datalinq.json` / `datalinq.user.json` without opening database connections.
* Added `${env:...}`, `${secret:...}`, and `${prompt:...}` config references, plus `datalinq secrets list|set|remove` for local secret storage.
* Removed the unsupported `UseRecord` config option and old record-generation plumbing.
...
* **Expect one-time generated C# diffs.** Regenerated model/database files now include a DataLinq generated-file banner and an explicit nullable directive before `using` statements. This is intentional generated-code metadata, not hand-editable content.
* **Nullable reference generation now defaults on.** If `UseNullableReferenceTypes` was omitted, regenerated files can gain nullable reference annotations such as `string?`. Set `"UseNullableReferenceTypes": false` on the database config to opt out; DataLinq will emit `#nullable disable` in those generated files.
* **Use header stamping deliberately.** `generate models --stamp-generated-header` records the CLI version and one UTC timestamp for the generation run. That is useful for provenance, but it creates timestamp-only diffs every time you regenerate.
* **Prefer the nested CLI commands.** `create-models` still exists as a deprecated compatibility command, but new usage should be `datalinq generate models`. SQL generation and database creation also moved under `generate sql` and `database create`.
* **Validate configs before release automation.** Config loading is now strict. Unknown JSON properties, stale options, and unsupported option values fail instead of being ignored. Run `datalinq config validate --recursive` from a repo root before wiring a release or CI job.
* **Update old config fields.** Use `ModelDirectory` for generated model files. `DestinationDirectory` is only a compatibility alias, `SourceDirectories` is deprecated/ignored, `UseRecord` is gone, and `UseCache` is model metadata rather than a `datalinq.json` option.
* **Update layout config names.** Generated member layout is controlled by `ModelLayout.PropertyOrder`, `PrimaryKeyPlacement`, `ForeignKeyPlacement`, and `RelationPlacement`. The short-lived experimental `KeyPlacement` name is not accepted.
* **Keep secrets out of committed config.** Move local connection strings and passwords into `datalinq.user.json`, `${env:...}`, `${secret:...}`, or `${prompt:...}` references. `datalinq secrets` currently stores local secrets through Windows Credential Manager.
