...
Compares an exact freshly packed candidate with a locked published package baseline by using the repo-local `Microsoft.DotNet.ApiCompat.Tool` manifest:

```bash
dotnet tool restore --tool-manifest ../.config/dotnet-tools.json
dotnet run --project DataLinq.Dev.CLI -- api-report --baseline-dir artifacts/api-baseline/nuget-org-0.9.2 --candidate-dir artifacts/nuget-release/0.10.0-preview.N --candidate-version 0.10.0-preview.N
dotnet run --project DataLinq.Dev.CLI -- api-report --baseline-version 0.9.2 --baseline-dir artifacts/api-baseline/nuget-org-0.9.2 --candidate-dir artifacts/nuget-release/0.10.0-preview.N --candidate-version 0.10.0-preview.N --output artifacts/release/v0.10/0.10.0-preview.N/api-against-0.9.2 --format markdown
```

`--baseline-dir`, `--candidate-dir`, and `--candidate-version` are required. `--baseline-version` defaults to `0.9.2`, the accepted fixed compatibility baseline for 0.10. Explicit 0.9.0 comparisons remain available as historical diagnostics, not required release gates. Omitting `--baseline-lock` selects `test-infra/api-compatibility/v<baseline-version>-packages.json`. The tracked 0.9.0 and 0.9.2 locks use the 0.10 release policy; explicit `--baseline-version 0.8.0` retains the historical 0.9 policy and unchanged 0.8 lock. Unknown release lines and ambiguous versions are rejected. Each lock binds exact NuGet.org package-byte SHA-256 values, repository URL/commit, and local Git tag identity (including the annotated 0.9.0 tag's peeled commit). The baseline directory is explicit: the command never discovers a convenient global cache copy or silently downloads a replacement. `--output` must name a missing, non-overlapping path; omitting it creates a unique directory under `artifacts/dev/api-report/`.

The 0.10 comparison set is `DataLinq`, `DataLinq.SQLite`, `DataLinq.MySql`, `DataLinq.Memory`, `DataLinq.Tools`, and the exact `tools/<tfm>/any/DataLinq.CLI.dll` assets for `net8.0`, `net9.0`, and `net10.0`. Memory is an existing baseline package: a removed or incompatible Memory API is a compatibility break. CLI assets are compared baseline-to-candidate per TFM and candidate net8 is compared bidirectionally with net9 and net10 so a framework-only addition is not mislabeled as a harmless baseline addition. Only the historical 0.8-to-0.9 policy treats Memory as new, validating its current package consistency and recording its first three public surfaces.
...
The 0.10 policy writes schema `v0.10.api-compatibility-report.v1`; the historical policy retains `v0.9.api-compatibility-report.v2`. Each run records its `ReleasePolicy` and retains `report.json`/`report.md`, raw standard output/error for every pinned ApiCompat invocation, generated suppression XML when emitted, and a metadata snapshot for every selected compile asset. A successful zero-diagnostic invocation has exit/log evidence and a null suppression path because ApiCompat 10.0.400 creates no empty XML file. ApiCompat is authoritative for compatibility classification. Snapshot semantic API hashes exclude MVID and whole-file hash and remain supplemental review/provenance evidence.
...
- only under the historical policy, each first `DataLinq.Memory` surface is a review item; under either policy, an inconsistent Memory package remains a hard failure
