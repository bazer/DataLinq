...
The calibrated allocation-regression counts are scenario-specific: provider initialization and startup PK use `1`; CRUD small and CRUD batch use `350`; update uses `2000`; cold PK and cold relation use `1000`; warm PK uses `60000`; and warm relation uses `1500000`. These counts target BenchmarkDotNet's recommended iteration duration while retaining the same normalized operation and telemetry semantics. Cold PK deliberately remains at 1,000 distinct keys: increasing it against the fixed corpus would mix warm cache hits into a row whose contract is one miss/load per operation. A retained short-iteration warning for that case is preferable to corrupting the workload.

Use `--allocation-stages --profile heavy` to attribute current-runtime work without a macro run. The twenty-two attribution microbenchmarks isolate canonical provider-row ownership/copying, provider-to-model materialization, the combined decoder-to-model pipeline, singular-source argument/SQL/result boundaries, known-miss publication, scalar/composite/typed-ID/converter-backed/binary canonical-key propagation (including a composite reconstruction baseline), source batch slicing/request/result/validation/cache-publication boundaries, and mutation capture/preflight/command/final-drift boundaries. Two additional end-to-end typed-ID rows cover cold and warm exact primary-key terminals. Each 1,000-operation row alternates `Single(row => row.Id == id)` with `SingleOrDefault(row => id == row.Id)`, so both terminal kinds and equality operand orders share one normalized receipt rather than multiplying history rows. Their telemetry records entity and reader execution plus row-cache hit/miss/store/load signals; setup also rejects a route that does not bind or materialize the typed ID as its canonical `Int32` provider key. The existing `--v09-query-backend` lane already isolates structural parsing, initial bind/freeze, invocation rebinding, and capability preparation.
...
| `--allocation-regression` | 9 | `sqlite-memory` | 9 | provider/startup `1`; CRUD small/batch `350`; update `2000`; cold PK/relation `1000`; warm PK `60000`; warm relation `1500000` |
| `--allocation-stages` | 24 | `sqlite-file`, `sqlite-memory` | 48 | `1000` for every method |
