...
Use `--allocation-stages --profile heavy` to attribute current-runtime work without a macro run. The five cases isolate canonical provider-row ownership/copying, provider-to-model materialization, the combined decoder-to-model pipeline, state-change capture from prebuilt changed mutables, and execution preflight against a prepared transaction/state change. The existing `--v09-query-backend` lane already isolates structural parsing, initial bind/freeze, invocation rebinding, and capability preparation.
...
| `--allocation-stages` | 5 | `sqlite-file`, `sqlite-memory` | 10 | `1000` for every method |
