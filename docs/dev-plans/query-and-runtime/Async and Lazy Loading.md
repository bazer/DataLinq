...
- Mutation identity/values and `SaveAsync` lifecycle selection are captured before first suspension. Pending operations have exclusive use of their mutable inputs through supported assignment/reset paths, with transaction-level versus implicit-helper lifetime and existing invalidation/snapshot limits preserved.
- Retain exactly-once synchronous local editing delegates before final capture, with pre-cancellation and local-failure semantics explicit; document the `Action` async-void limitation rather than adding asynchronous editing callbacks. Capture finite multiple-model membership/order/values before writes and reject repeated mutable object references; generated-value dependencies require explicitly sequenced operations.
...
- Key lookups mirror existing database/transaction/generated source families, with model-side typed keys normalized exactly once and canonical provider keys kept distinct. Capture supported key values before suspension and preserve null-key sentinel parity and nullable missing-row results.
- Collection relations use one genuine `AsAsyncEnumerable` primitive with acyclic async defaults, explicit `NotSupportedException` for absent capability, concrete member exposure, relation-scoped key lookup, duplicate-key rejection, and internally consistent results without post-await synchronous getters. Preserve synchronous reference covariance through a separate invariant `IAsyncImmutableForeignKey<T>` capability; generated navigation shares loader state and enforces required/optional/cardinality/capability distinctions with focused DLG collision errors.
- Lower-level execution mirrors string/IDbCommand families and supported Select helpers with verified async provider dispatch, an IDataLinqAsyncDataReader companion, and explicit ephemeral current-row versus materialized results. Preserve owned/borrowed command/reader/connection lifetimes; caller commands are borrowed resources, not automatically cloned invocation snapshots.
- Raw calls through DataLinq transaction adapters share managed execution/lifetime gates and conservative post-dispatch failure handling, without automatic raw mutation tracking, SQL-effects inference, or guarantees for escaped provider handles. Keep consuming attachment synchronous and support async disposal on existing owning roots with explicit dependency/DI ownership.
- Expose typed immutable failure snapshots through DataLinqFailure.GetContext(Exception) and Transaction.FailureContext, including after implicit-helper disposal. Use validated provider-scoped DataLinqExecutionOptions.RecoveryRollbackTimeout, defaulting to 30 seconds and starting at automatic rollback, without global/live settings, budget-restarting retries, or a total-cleanup deadline.
- Mirror async existence probes without swallowing cancellation or confusing probe failure with schema differences. Live metadata factories retain their Option contract for non-cancellation failures; runtime validation returns complete comparisons, captures configuration before suspension, and inspects the provider's effective existing database with explicit ownership and freshness limits.
- Mirror existing mutation and finite collection-insert families, keeping generated typed editing bridges and source-less helpers' independent transaction ownership. Preserve narrow nullable Save behaviors, async result loading for unchanged updates, and untyped provider callback families with the shared completion/recovery contracts.
- AAPI-73 settles synchronous-navigation guidance: use explicit async results and materialized DTOs rather than promising that cache warming makes later getters I/O-free. No strict sync-I/O mode, analyzer, eager loading, or batching is added.
- AAPI-74 through AAPI-78 settle Memory's existing async query subset and narrow single-model-key FindAsync counterpart, retained navigation/prepared-source exclusions, legitimate immediate completion with cooperative cancellation, and explicit missing capability. Standalone async relation graph fixtures remain separate from Memory navigation; no fallback, scheduler/offload promise, public SupportsAsync flag, or broader query language is introduced.
- AAPI-79 through AAPI-81 settle per-operation SQLite blocking/cancellation limits, MySQL/MariaDB operation/connection trust and timeout distinctions, and the backend/consumer evidence matrix. Policies do not prove provider interruption or recovery feasibility; constructor/setup and administrative I/O remain explicitly inventoried.
- AAPI-82 through AAPI-85 retain synchronous construction/preparation and SQLite setup timing, add explicit async journal-mode/provisioning counterparts, and preserve partial-creation/error/resource semantics without atomicity, automatic replay or destructive recovery. Validation remains observational; provisioning is not a migration engine.
- AAPI-86 through AAPI-90 complete fluent read helpers with private per-invocation builder snapshots, exclude disabled fluent mutation execution, add public TableCache.GetRowAsync with canonical-key/cache/source parity, and retain synchronous local maintenance/callbacks. No worker-thread facade or unsupported materialization/source expansion is implied.
- AAPI-91 through AAPI-99 settle immutable DataLinq.Diagnostics failure snapshots/access and identity, independent classification/outcome enums, recovery flags, ordered secondary exceptions, preserved options constructor/property/interface boundaries, 1 ms through 4,294,967,294 ms recovery settings captured before setup, DLG004 navigation conflicts, and preserved synchronous MariaDB version probing alongside SQLite construction I/O.
- AAPI-100 through AAPI-102 accept raw model reader counterparts on DataSourceAccess/ReadOnlyAccess/Transaction, public provider-transaction async completion/disposal with unsupported legacy defaults and preserved managed ownership, exact diagnostic enum values with Unknown = 0, and DataLinq execution-options namespace. The [signature inventory](../roadmap-implementation/v0.10/Async%20Signature%20Inventory%20and%20Compatibility%20Matrix.md) records these resolved G01–G03 findings.
- AAPI-103 through AAPI-105 settle unconstrained selector-based query Min/Max and nullable results, complete direct local relation predicate/numeric/generic extrema overloads, and explicit inherited provider-interface async-disposal defaults with public virtual base/concrete dispatch. Generic signatures do not broaden provider translation; local relation reductions operate on loaded model values.
- AAPI-106 through AAPI-111 settle core validation supporting types/results, independent failure policy, comparison-scoped Include and empty-schema handling, bounded per-command timeout, async LINQ 10.0.12 on .NET 8/9 and locked 0.9.0 compatibility coverage including Memory.
- OAPI-4/OAPI-5/OAPI-6 policies are accepted under AAPI-21 through AAPI-41, OAPI-7's discussed design policies through AAPI-111, and OAPI-8/OAPI-9 policies under AAPI-73 through AAPI-81. G01–G03/E01–E06 choices are accepted; OAPI-7 remains an implementation/manifest/verification gate. Continue the [W0 execution-plan discussion](../roadmap-implementation/v0.10/W0%20Baseline%20and%20Evidence%20Plan.md). Accepted contracts and internal coordination/versioning require consumer, deterministic runtime, provider and cost evidence. Further design questions must come from concrete gaps/contradictions. Async-stream mutation inputs, bulk execution and automatic mutation dependency ordering remain outside 0.10.
