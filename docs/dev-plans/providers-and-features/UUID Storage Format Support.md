...
This closes the bounded source declaration, mutable initialization, and direct metadata-to-model regeneration path for fixed direct-`Guid` defaults. It does not prove source/database `MetadataTransformer` precedence, import of static provider Guid defaults, SQLite expression/BLOB import, converter-backed defaults, dynamic `DefaultNewUUID` completion, or one authoritative constructor/default owner.
