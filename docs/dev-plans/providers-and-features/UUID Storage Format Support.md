...
**Status:** Implementation in progress. Bounded `UUID-1A` declaration/codec primitives and `UUID-1B` immutable provider-keyed resolution are implemented. Bounded `UUID-2` checkpoints now consume resolved formats for non-primary-key full-row canonical `Guid` reads and insert/update writes on SQLite, MySQL, and MariaDB, including direct model `Guid` values and a representative converter-backed typed-ID composition slice. SQLite covers Text36, Text32, little-endian BLOB, and RFC-order BLOB; MySQL/MariaDB cover native MariaDB UUID, Text36, Text32, little-endian `BINARY(16)`, and RFC-order `BINARY(16)`, with nullable Text36/SQL NULL evidence on every provider family. Typed IDs over representative binary storage plus nullable Text36 are green; typed UUID primary keys, projections, queries, relations, defaults, and schema comparison/validation remain open; aggregate `UUID-2` is not complete.
