...
- define `DefaultNewUUID` client/server behavior for both Version4 and Version7 across provider versions and storage formats
- preserve UUID version semantics through import, model generation, comparison, snapshots, and net8-compatible client generation
...
The format comparison and finalized direct-`Guid` DDL-literal portions of the exit signal are green. Source representation, converter-backed defaults, dynamic/version semantics, canonical/source-only validation, and aggregate evidence keep UUID-4 incomplete.
