...
Still open are net8-compatible client-side Version7 generation, avoiding duplicate client generation across generated constructor paths, one authoritative default owner, transformer precedence when source and imported metadata disagree, SQLite expression import, and verified provider-version plus physical-storage mappings for automatic server generation. The fail-closed checkpoint does not claim those behaviors are complete; it only guarantees that DataLinq no longer silently lies about the requested UUID version.
...
The integrated gates pass `58/58` generator, `1180/1180` unit, and `803/803` SQLite file/memory compliance tests. The latest SQLite/MySQL/MariaDB compliance batch passes `1511/1511`; the full provider-specific MySQL/MariaDB lane passes `372/372`, split into `185/185` for MySQL 8.4 plus MariaDB 10.11 and `187/187` for MariaDB 11.4 plus 11.8. Live evidence on all four server targets creates an exact `UUID()` default, imports it from information schema as provider-scoped `DefaultSql`, regenerates DDL, recreates the schema, and preserves the raw expression. This proves the fail-closed provider boundary, raw `UUID()` import/DDL roundtrip, and UUID-version semantic fingerprints. It does not prove automatic server generation or a complete client default contract.

Still open:

- define a legal source-model representation and regeneration roundtrip for fixed `Guid` defaults
- define converter-backed model-default conversion instead of treating model values as canonical values
- complete client-side `DefaultNewUUID` behavior, including net8-compatible Version7 generation and avoiding duplicate generation across constructor paths
- define one authoritative default owner and transformer precedence when source declarations and imported metadata disagree
- add verified provider-version capability and exact physical-storage mappings before enabling automatic server generation
- preserve or diagnose SQLite UUID default expressions during schema import
...
The format comparison, finalized direct-`Guid` DDL-literal, and fail-closed UUID-version truthfulness portions of the exit signal are green. Source representation, converter-backed defaults, complete client generation, automatic server capability/storage mapping, source-transform ownership, SQLite expression import, canonical/source-only validation, and aggregate evidence keep UUID-4 incomplete.
