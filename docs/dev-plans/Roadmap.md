...
- projection recipes are self-contained after parsing, and the SQL adapter still owns the complete retained recipe family; the separate packable experimental memory preview retains an exhaustive 54-token island for nested `&&`/`||`/`!` composition over exact direct nonnullable converter-free model/provider `Int32` `==`/`!=`/`<`/`<=`/`>`/`>=` leaves, exact positive or negated local-`Int32` membership leaves, and direct-`Guid` or resolved Guid-backed typed-ID `==`/`!=` leaves, plus one final `Take`, one exact ordered final `Skip`, or one exact ordered final `Skip`/`Take` window and exact unpaged `Single`/`SingleOrDefault` over root entities or direct-`Int32` scalars, rather than general local or joined recipes
...
- `DataLinq.Memory` now has a public, packable experimental read-only preview surface with generated-mutable seed, capability-gated query, model-valued exact single-column `Find<TModel>(object)` lookup, and a first-class targetless Testing CLI `memory` suite; aggregate M0, bounded M1-A exact non-null inequality, bounded M1-B exact Boolean composition, bounded M1-C exact non-null `Int32` relational comparison, bounded M1-D exact local `Int32` membership, bounded M1-E exact ordered final `Skip`, bounded M1-F exact `Single`/`SingleOrDefault`, bounded M1-G exact ordered `Skip`/`Take` window, W10 steps 1-2 / RE-1D package-tool integration, and W10 step 3 / RE-1A suite registration are complete, with raw SQL structurally absent from Memory-owned services and inherited legacy row access rejecting before backend work, while composite lookup, aggregate M1/M2, membership outside the exact direct non-nullable `Int32` fence, paging outside the exact ordered final-`Skip`/`Take` fence (including alternate or repeated windows and element terminals after paging), `First`/`FirstOrDefault`/`Last`/`LastOrDefault`, RE-1C, RE-1E/F/G/H, W10 steps 4-9, aggregate RE-1/RE-4/W10/W11, packaged constrained-runtime evidence, consumer smoke, final release-candidate closeout, and publication remain open; there is still no JSON memory persistence package or migration execution engine
