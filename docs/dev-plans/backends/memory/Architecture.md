...
The currently implemented Memory profile is deliberately narrower than the intended subset below. It contains exactly 49 capability tokens and admits `Predicate:And`, `Predicate:Or`, and `Predicate:Not` as nested Boolean composition over these exact predicate shapes:
