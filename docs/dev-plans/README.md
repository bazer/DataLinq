...
- Phase 14 source-slot join composition is in progress after Phase 13 and the single-source Phase 13B grouping slice: the first slice composes filtering/ordering/paging/counting over explicit two-source joined rows that can bind projected members back to source slots.
