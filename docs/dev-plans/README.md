...
- Phase 8 Native AOT and WebAssembly readiness is implemented historically for the generated SQLite Native AOT, trimmed runtime, and Blazor WebAssembly AOT smoke boundary. The 2026-06-28 compatibility refresh has green Native AOT and trimmed evidence, and the 0.8 release gate tooling now automates browser WebAssembly smoke. Phase 23 current `wasm-aot` browser evidence is green for the generated SQLite smoke after fixing generated metadata startup; clean-output WebAssembly publish and SQLitePCLRaw warning disposition remain release-evidence caveats.
...
- Phase 22 LINQ parser plan cleanup is planned as the final parser implementation hardening slice before release evidence: freeze plan bindings, remove allocation-heavy render-time binding lookup, and preserve existing query behavior.
- Phase 23 browser AOT debugging is implemented for the current `wasm-aot` browser failure at generated SQLite startup. It records passing AOT and no-AOT browser smokes, plus the remaining clean-output SDK and SQLitePCLRaw warning caveats.
- Phase 24 release evidence, benchmarks, and docs is planned as the final closeout: compatibility report, package report, focused benchmark refresh, public docs pass, and release wording that matches the evidence.
...
The next 0.8 work is not another broad query feature. The query-runtime feature sequence moved through Phase 13 query composition and subquery pushdown, Phase 13B grouped aggregate projection, Phase 14 source-slot join composition, Phase 15 relation-aware and implicit joins, Phase 16 grouped numeric aggregates, Phase 17 grouped-row composition/HAVING, Phase 18 advanced keys plus joined grouping, Phase 19 SQL-backed projection rows, Phase 20 single query-syntax inner joins, and Phase 21 joined post-paging pushdown without expanding the documented support matrix by wishful thinking. The remaining 0.8 work should now stay focused on Phase 22 parser cleanup and Phase 24 release evidence/docs.
