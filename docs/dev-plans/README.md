...
- The active release roadmap is `roadmap-implementation/v0.8/`. The 0.8 parser-removal track is complete through Phase 7; Phases 8 through 12 now own browser AOT runtime proof, WebAssembly warning/no-AOT disposition, query coverage, deploy-size hardening, and final release gates. Phase 13, Phase 13B, and Phases 14 through 17 cover implemented query-composition, grouped count and numeric aggregate projection, source-slot joins, implicit singular relation slices, and grouped row composition/HAVING. Phase 18 now holds the remaining planned GroupBy completion work for advanced keys plus joined grouping. Phases 19 through 21 hold the planned SQL-backed projection row, query-syntax join, and joined post-paging pushdown work. Completed global phase execution records belong under `archive/roadmap-implementation/`.
...
The next broad query-runtime feature work after the 0.8 AOT release gates moved through query-composition hardening and the narrow grouped aggregate projection baseline, then into source-slot join expansion: Phase 13 query composition and subquery pushdown, Phase 13B grouped aggregate projection, Phase 14 source-slot join composition, Phase 15 relation-aware and implicit joins, Phase 16 grouped numeric aggregates, and Phase 17 grouped-row composition/HAVING. The source-slot-aware query plan exists; the remaining GroupBy roadmap work is Phase 18 advanced keys plus joined grouping. Phases 19 through 21 then cover SQL-backed projection rows, query-syntax joins, and joined post-paging pushdown, without expanding the documented support matrix by wishful thinking.
