...
- Phase 15 relation-aware and implicit joins is in progress after Phase 14: the first slice targets SQL-backed implicit singular relation traversal in predicates and ordering, while `JoinBy(...)`, `JoinMany(...)`, join-local `on:` predicates, left-join nullability semantics, and a `net10.0` `Queryable.LeftJoin(...)` support decision remain deferred until their API/nullability design is real.
