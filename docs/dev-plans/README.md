...
- The active release roadmap is `roadmap-implementation/v0.8/`. The 0.8 parser-removal track is complete through Phase 7; Phases 8 through 12 now own browser AOT runtime proof, WebAssembly warning/no-AOT disposition, query coverage, deploy-size hardening, and final release gates. Phase 13, Phase 13B, and Phases 14 through 15 cover implemented query-composition, grouped count projection, source-slot joins, and implicit singular relation slices. Phases 16 through 18 now hold the planned GroupBy completion work for grouped numeric aggregates, grouped-row composition/HAVING, and advanced keys plus joined grouping. Phases 19 through 21 hold the planned SQL-backed projection row, query-syntax join, and joined post-paging pushdown work. Completed global phase execution records belong under `archive/roadmap-implementation/`.
...
- Phase 16 grouped numeric aggregates is planned after the Phase 13B count baseline: grouped `Sum`, `Min`, `Max`, `Average`, and multiple aggregate members should extend the existing SQL-shaped grouped projection without materialized `IGrouping<TKey,TElement>` support.
- Phase 17 grouped row composition and HAVING is planned after grouped numeric aggregates: grouped aggregate rows should become orderable, pageable, filterable, and able to express narrow SQL `HAVING` predicates without client fallback.
- Phase 18 advanced GroupBy keys and joined grouping is planned after grouped-row composition: composite keys, SQL-renderable computed keys, and grouping over supported joined source-slot shapes should land only after key/member binding is explicit.
- Phase 19 SQL-backed projection rows and implicit relation projection is planned after the SQL-style GroupBy completion track: direct projection rows should materialize from SQL aliases, and singular relation member projection must not hide lazy relation loading inside provider `Select(...)`.
- Phase 20 query-syntax join support is planned after SQL-backed projection rows: C# query-syntax inner joins should bind compiler-generated transparent identifiers back to source slots instead of treating anonymous carrier objects as opaque runtime values.
- Phase 21 joined post-paging pushdown is planned after query-syntax join support: joined row shapes should get Phase 13 operator-order preservation only after derived joined sources can preserve required key and projection aliases.
...
The next broad query-runtime feature work after the 0.8 AOT release gates moved through query-composition hardening and the narrow grouped aggregate projection baseline, then into source-slot join expansion: Phase 13 query composition and subquery pushdown, Phase 13B grouped aggregate projection, Phase 14 source-slot join composition, then Phase 15 relation-aware and implicit joins. The source-slot-aware query plan exists; the remaining GroupBy roadmap work is now split into Phase 16 grouped numeric aggregates, Phase 17 grouped-row composition/HAVING, and Phase 18 advanced keys plus joined grouping. Phases 19 through 21 then cover SQL-backed projection rows, query-syntax joins, and joined post-paging pushdown, without expanding the documented support matrix by wishful thinking.
