...
- Repeated index attributes for the same table/index name now have to agree on index characteristic and type before index parsing, so provider-style composite repeats remain valid while contradictory stale metadata cannot be silently normalized by the first parsed index definition.
