...
**Status:** Complete for the Phase 8B foundation. The primary metadata producers feed typed drafts into `MetadataDefinitionFactory`, and factory-built runtime graphs are frozen against setter-style mutation plus the main structural collection mutation paths. Public array surfaces, including generated declaration table-model arrays, return defensive copies, cache-policy collection surfaces are freeze-aware after finalization, and the public definition mutator plus mutable-factory input surfaces are obsolete compatibility API. SharedCore construction paths use internal construction-only mutation helpers, keeping the remaining mutable lowering graph isolated behind the typed draft/factory boundary.
...
1. [x] Add metadata builder/draft types side by side with the current metadata graph. The typed database/table/model/property/column draft records now exist, carry source diagnostics, and are used by the primary metadata producers; the mutable graph remains the internal lowering/finalization target, but factory-built snapshots are frozen against ordinary setter-style mutation and SharedCore lowering goes through internal construction APIs.
...
9. [x] After parity is proven, remove or obsolete public mutators and expose immutable or read-only collections. Structural list/dictionary mutation is blocked after freeze, public arrays and generated declaration arrays no longer expose live storage, cache-policy lists reject post-finalization mutation, direct global metadata registry mutation is obsolete, public definition mutators are obsolete, and SharedCore no longer builds through those public mutators; the remaining mutable lowering graph is internal construction machinery behind the typed draft/factory boundary.
...
- Array-valued metadata attributes now defensively copy constructor inputs and returned arrays for enum values, index column names, and relation column names, closing a nested mutation path that bypassed frozen metadata snapshots while keeping the existing `string[]` public API shape.
- Enum-property metadata now follows the same defensive-copy pattern for database enum values, C# enum values, and fallback value-list views, preventing `IReadOnlyList` casts from mutating enum metadata inside frozen snapshots.
- Default-value attributes now become freeze-aware when their containing metadata property is finalized, so source-parser `CodeExpression` construction remains available while post-build changes through `SetCodeExpression(...)` are blocked on frozen snapshots.
- The public `DefaultAttribute.SetCodeExpression(...)` route is now marked obsolete like the other runtime metadata mutators; construction-time code-expression metadata uses a constructor overload, and source parsing no longer depends on the public mutation API.
...
- The draft snapshot boundary now checks existing table column/value-property bindings before primary-key and index ownership validation, so null table column entries take the intended `InvalidModel` path instead of being exposed to later graph validators.
- Existing relation-side index tables are now validated against the database graph before draft snapshot copying, so a relation candidate/foreign-key side pointing at an external table returns `InvalidModel` instead of leaking a snapshot map failure.
- Existing relation-side indexes must now also be registered on their owning table before draft snapshot copying, so a half-registered relation side returns `InvalidModel` instead of being silently added by snapshot copying.
- Existing relation definitions now reject empty constraint names before draft snapshot copying, matching source/typed foreign-key attribute validation and avoiding generated `[ForeignKey]` metadata with an invalid empty constraint.
- Existing relation-side index roles are now validated before draft snapshot copying, so foreign-key sides must use foreign-key indexes and candidate-key sides must use primary or unique indexes instead of preserving semantically invalid relation graphs.
- Existing primary-key and foreign-key index column roles are now validated before draft snapshot copying, so primary-key indexes must match the table primary key and foreign-key indexes cannot reference columns that are not marked as foreign keys.
- Existing column indexes now reject duplicate column references before draft snapshot copying, so generated SQL, schema comparison, and relation matching cannot inherit an index that lists the same column more than once.
- Existing index relation-part lists now reject duplicate relation-part references before draft snapshot copying, so relation enumeration and copied snapshots cannot inherit repeated references to the same relation side.
...
- Generated database declarations with a null table-model array now reach declaration validation and return an `InvalidModel` failure for missing `TableModels`, rather than throwing from the declaration constructor before provider initialization can classify the stale output.
...
- Model table-shaping attributes are now checked against the linked table/view metadata before draft snapshot copying, so duplicate `[Table]`/`[View]` attributes, table/view kind mismatches, stale table/view names, and contradictory `[Definition]` SQL return `InvalidModel` instead of being normalized away by generated model output.
- Database and model interface identity attributes are now checked against the linked metadata before draft snapshot copying, so stale `[Database]` names and `[Interface]` generation targets return `InvalidModel` instead of being regenerated from the resolved graph.
...
- Existing relation definitions now validate their foreign-key column `[ForeignKey]` metadata against the registered relation sides before draft snapshot copying, so stale referenced table/column or referential-action metadata returns `InvalidModel` instead of being overwritten by generated model output.
- Existing relation definitions now reject duplicate per-column `[ForeignKey]` metadata for the same constraint before draft snapshot copying, so stale extra relation attributes cannot be silently dropped by generated model output.
- Composite relation foreign-key ordinal metadata is now validated against registered relation-side order before draft snapshot copying, so partial or out-of-order `[ForeignKey]` ordinals return `InvalidModel` instead of being normalized by generated source.
...
- Linked relation-property targets are now checked against their relation parts before draft snapshot copying, so existing graphs with contradictory stored relation names or `[Relation]` table/column/name targets return `InvalidModel` instead of being normalized by source/model generation.
- Linked relation-property C# types are now checked against their relation parts before draft snapshot copying, so existing many-to-one and one-to-many relation properties cannot carry stale model or `IImmutableRelation<T>` type metadata that generated source would otherwise overwrite.
- Relation properties now reject multiple `[Relation]` attributes before draft snapshot copying, so stale extra relation targets cannot be silently dropped when generated model output emits the single resolved relation side.
- Linked relation properties now require a matching `[Relation]` attribute before draft snapshot copying, so existing graphs cannot rely on generated model output to recreate missing relation-target metadata from the resolved relation side.
...
- Enum attribute metadata is now also checked at the parser and factory boundaries, so repeated `[Enum]` attributes or stale `EnumAttribute` entries without matching enum-property metadata return `InvalidModel` instead of leaking `SingleOrDefault` exceptions or being silently ignored.
...
- Cache-policy attributes are now checked against the linked cache metadata before draft snapshot copying, so stale database/model `[UseCache]`, `[CacheLimit]`, `[CacheCleanup]`, and `[IndexCache]` attributes return `InvalidModel` instead of being normalized away by generated model output.
- Model file generation now preserves cache policy metadata from the built graph by emitting database-level `[IndexCache]` attributes plus table-level `[UseCache]`, `[CacheLimit]`, and `[IndexCache]` attributes, preventing regenerated source models from dropping factory-validated cache behavior.
...
- Foreign-key ordinal metadata is now preflighted before draft snapshot copying, so negative `[ForeignKey]` ordinals return `InvalidModel` instead of being accepted as arbitrary relation ordering input and later rewritten by generated output.
...
- Foreign-key attributes sharing a constraint name on the same table now have to target one referenced table and agree on referential actions before relation parsing, so stale constraint metadata cannot be split into multiple generated relations or normalized from the first parsed attribute.
- Repeated index attributes for the same table/index name now have to agree on index characteristic, type, and explicit column targets before index parsing, so provider-style composite repeats remain valid while contradictory stale metadata cannot be silently normalized by the first parsed index definition.
- Generated relation-property names are now checked before auto-created relation properties are added, so many-to-one and one-to-many relation parsing returns `InvalidModel` instead of silently suffixing names when the generated property would collide with an existing model property.
- Value-property column-shaping attributes are now checked against their linked column metadata before draft snapshot copying, so stale `Column`, `PrimaryKey`, `Nullable`, `AutoIncrement`, `ForeignKey`, and `Type` attributes cannot disagree with the runtime column graph and then be overwritten by generated model output. The `TypeAttribute(DatabaseColumnType)` constructor now preserves decimal scale metadata as part of that consistency path.
...
- Source-model parsing now catches unsupported Roslyn property type syntax at the `SyntaxParser` boundary and returns an `InvalidModel` option failure with source context, rather than letting `CsTypeDeclaration` throw through the source metadata factory as a catch-all exception.
- Source-model parsing now also catches unsupported model base-list type syntax while collecting declared interfaces, so malformed base declarations return `InvalidModel` diagnostics through both the direct syntax parser and source metadata factory paths instead of leaking `CsTypeDeclaration` exceptions.
- Source-model table-property resolution now validates same-named declarations before falling back to missing-model stubs, so a malformed or non-table declaration referenced by `DbRead<T>` cannot be silently ignored just because no valid table models were discovered in the source set.
- Source-model table-property discovery now uses the same `DbRead<T>` type recognizer as table-type parsing and supports qualified `DataLinq.DbRead<T>` syntax, preventing qualified table properties from being dropped before validation/finalization.
- Source-model table-property resolution now normalizes qualified and alias-qualified `DbRead<T>` model type arguments, so `DbRead<My.Namespace.UserModel>` resolves the referenced model instead of rejecting otherwise valid source metadata as a non-simple table property; the same parser boundary now exposes allowed missing-model stub syntax as nullable instead of pretending every table property has a concrete source declaration.
- Source-model declaration discovery now normalizes qualified model interface syntax such as `DataLinq.Interfaces.IDatabaseModel` and `DataLinq.Interfaces.ITableModel<TestDb>` before matching DataLinq interfaces, so source generator and file-based metadata collection do not silently skip qualified database or table models.
- Source-model and factory model-interface checks now require exact contract names or generic forms such as `ITableModel<T>`, so lookalike names like `ITableModelBackup<T>` no longer classify a source type as a DataLinq table/view model.
- Source-model and factory model-interface checks now also validate generic contract arity, so malformed contracts such as `ITableModel<TestDb, OtherDb>` and `IViewModel<>` are rejected instead of being classified as valid table/view model metadata.
- Source-model recursive interface discovery now tracks visited declarations, so cyclic non-DataLinq interface graphs cannot recurse indefinitely or prevent a later valid `ITableModel<T>`/`IViewModel<T>` contract from being discovered.
- Source-model declared interface parsing now preserves inherited DataLinq model contracts through ordinary and generic application interfaces, so models can use project-level interfaces such as `IProjectModel<T>` without losing the underlying `ITableModel<TDb>`/`IViewModel<TDb>` contract during table parsing.
- Source-model recursive interface discovery now substitutes generic interface type parameters while walking application interfaces, so contracts such as `IProjectModel<TDatabase> : ITableModel<TDatabase>` are preserved as concrete `ITableModel<TestDb>` metadata when a model implements `IProjectModel<TestDb>`.
- Source-model attribute parsing and property discovery now normalize qualified and `Attribute`-suffixed names such as `DataLinq.Attributes.ColumnAttribute`, so table/model/property metadata is parsed consistently instead of dropping columns before factory validation.
- Source-model `InterfaceAttribute` parsing now requires the exact normalized attribute name, so lookalike attributes such as `[InterfaceBackup]` return the expected not-implemented parse failure instead of creating model-instance interface metadata.
- Source-model generic `InterfaceAttribute<T>` parsing now normalizes qualified interface type arguments such as `InterfaceAttribute<TestNamespace.IUserModel>`, matching runtime `InterfaceAttribute<T>` behavior and preventing generated model-instance interface metadata from being rejected as an invalid C# identifier.
- Source-model generic `InterfaceAttribute<T>` parsing now validates generic arity, so malformed declarations such as `[Interface<IUserModel, IOtherUserModel>]` return an `InvalidArgument` parse failure instead of throwing or silently using only the first interface type.
- Source-model default-value source spans and generator default-expression fallback lookup now use the same normalized attribute-name path, so qualified `DefaultAttribute` syntax still reports validator diagnostics at the default expression rather than falling back to coarser model locations.
- Source-model relation property type parsing now normalizes qualified and alias-qualified type syntax inside generic relation properties, so declarations such as `DataLinq.Interfaces.IImmutableRelation<TestNamespace.OrderModel>` and `global::TestNamespace.UserModel` resolve against relation-shape validation instead of looking like stale C# type metadata.
- Source-model declarations without a C# namespace now fail during metadata validation with `InvalidModel` and source-span context, instead of reaching generator file creation and surfacing as a model-file generation exception.
- Generated runtime metadata import now resolves `[Interface]` model-instance targets by matching the requested generated interface name and requiring the matched application interface to inherit `IModelInstance<T>`, with the previous discovery path retained only as a constrained fallback. Ordinary user interfaces implemented by the model therefore remain in `OriginalInterfaces` instead of being mistaken for the generated interface.
...
- A final Workstream C audit removed the stale transformer TODO that suggested copying a source model class into `OriginalInterfaces`, and the transformer tests now assert snapshot transforms keep original-interface metadata interface-shaped.
- Workstream C deliberately finishes as a factory-owned snapshot foundation rather than a total runtime metadata rewrite. The current graph is frozen against ordinary mutators and the main structural collection mutators after factory finalization, array-shaped properties and generated declaration arrays no longer expose live storage, cache-policy lists are sealed after freeze, the raw global metadata registry is obsolete, public definition mutators are obsolete, SharedCore construction uses internal-only mutators, and generation/cache/provider-default/transformer/schema test fixtures use typed drafts; typed drafts still lower through the mutable graph internally as an implementation detail.
