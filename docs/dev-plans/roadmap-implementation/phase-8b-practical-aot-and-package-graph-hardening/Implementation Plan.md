...
2. [in progress] Represent source-model metadata, generated declarations, SQLite metadata, MySQL metadata, and MariaDB metadata in builder form. SQLite, MySQL, MariaDB, and generated runtime metadata now build typed drafts before factory finalization; source-model metadata still hands a mutable metadata graph through the draft boundary.
...
- Generated runtime metadata import now builds typed drafts from `GeneratedDatabaseModelDeclaration`, `GeneratedTableModelDeclaration`, and reflected model/property attributes before calling `MetadataDefinitionFactory.Build(MetadataDatabaseDraft)`, leaving source-parsed metadata as the remaining mutable producer in this migration slice.
