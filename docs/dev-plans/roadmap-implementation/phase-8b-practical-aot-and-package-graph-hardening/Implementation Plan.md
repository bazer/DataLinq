...
2. [in progress] Represent source-model metadata, generated declarations, SQLite metadata, MySQL metadata, and MariaDB metadata in builder form. SQLite, MySQL, and MariaDB metadata now build typed provider-local drafts before factory finalization; source-model and generated runtime paths still hand mutable metadata graphs through the draft boundary.
