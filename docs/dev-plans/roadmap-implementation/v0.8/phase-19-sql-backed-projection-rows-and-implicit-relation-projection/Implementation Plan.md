...
- [ ] Add a SQL-backed projection-row plan shape distinct from row-local anonymous/computed projections.
- [ ] Parse scalar and anonymous projections whose members bind directly to source-slot values.
- [ ] Parse scalar and anonymous projections over supported implicit singular relation members through planned joins.
- [ ] Render SQL-backed projection members with stable aliases.
- [ ] Materialize SQL-backed scalar, anonymous, DTO, or record projection rows from the data reader.
- [ ] Preserve existing row-local computed projection behavior for documented computed selectors.
- [ ] Keep relation object projection, collection relation projection, nested database projection, multi-hop implicit traversal, and client projection expressions rejected with focused diagnostics.
- [ ] Add provider behavior, SQL-shape, snapshot, transaction-root, and unsupported-diagnostics tests.
