...
This is not yet universal read admission: ordinary synchronous queries do not newly acquire a lease across enumeration in this slice. The old synchronous authoritative-hydration thread-ID allowance remains confined to the existing path; it must not be reused for async dispatch. Its replacement with an explicit private source and complete source/reader admission remains W1 follow-through before provider wiring.
...
Remaining integration includes private authoritative hydration without thread-ID permission; command/materializer/reader lifetime wrappers and ownership on every exit; admission across warm and cold source execution; and helper-owned callback admission/draining. W1.3 must compose recovery, bounded rollback policy and public primary/secondary failure information. W1.4 must connect invocation/source capture and enumeration cleanup. W2 binds native provider initialization, replacing their existing early-open publication only after provider evidence.
