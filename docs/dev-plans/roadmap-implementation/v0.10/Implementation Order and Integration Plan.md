...
The merged [owned read integration](W1%20Owned%20Read%20Execution.md) removes synchronous thread-based hydration permission and carries explicit ownership through single-row caches, source-row loaders, scalar execution and raw reader lifetimes. The subsequent [query and relation ownership integration](W1%20Query%20and%20Relation%20Ownership.md) extends synchronous admission through outer LINQ/fluent/relation enumeration and guards overlapping enumerator calls. Async handoff, recovery, full invocation capture, native initialization and coordination cost acceptance remain pending.
