...
The follow-up [initialization handoff](W1%20Initialization%20Handoff.md) connects the controllable lazy initializer to deferred readers, actual managed helper/disposal ownership and diagnostic publication. Remaining command/mutation/metadata dispatch families still need that integration. The original scripted failed-initialization case proves restrictions, not actual native initialization. Native open/configure/begin, attached-provider feasibility, driver-specific outcome/cancellation classification, low-level public provider completion and public declarations remain W2/W3 work.
