...
This first slice exercises key-bearing SQL relation targets with controllable integer/string/composite fixtures. The [relation keys and targets follow-through](W1%20Relation%20Keys%20and%20Targets.md) adds converted/binary/composite keys, built-in GUID writer capture and accepted keyless candidate-key view targets, including fixes for async rejection and synchronous false cache identity. Full diagnostic correlation and comparable .NET 10 allocation/coordination measurements remain W1 work, including the added per-holder semaphore cost. No performance claim follows from the signature or passing functional tests.
