...
**Status: baseline capture complete; W0-F1 remains an unresolved exit finding.** This capture does not claim async implementation or W0 closeout. The later [limited W1 exception](SQLite%20Pool%20Ownership%20Investigation.md#accepted-limited-w1-exception) permits internal contracts and controllable-provider tests only. Capture ran on 2026-09-16 UTC (continuing past midnight into 2026-09-17 in Europe/Madrid). The [accepted capture plan](W0%20Baseline%20and%20Evidence%20Plan.md) and [source-linked I/O map](W0%20IO%20Execution%20Map.md) define this checkpoint.
...
**Disposition at capture: not waived; W0 handoff blocked.** At this checkpoint the driver defect was reproduced, but its relationship to the transaction error and a safe correction had not yet been established. The planned follow-up was to preserve the reproduction, validate a correction and rerun affected evidence. Passing repeats, suppressing the test, serializing transactions or automatically retrying writes do not establish a correction. Any pooling policy change needs explicit compatibility and performance evaluation. Under the [accepted branch workflow](Branch%20PR%20and%20Benchmark%20Workflow.md), a confirmed defect affecting current consumers is fixed on master and merged forward. No upstream issue/PR or package publication was submitted by this capture.

**Follow-up, 2026-09-17:** the [separate investigation](SQLite%20Pool%20Ownership%20Investigation.md) reproduces the exact transaction error with the published driver, verifies the correction and links upstream issue #39008 and PR #39009. Its accepted limited exception allows W1 internal contracts and controllable-provider tests without waiting for the upstream schedule. W0-F1 remains open until DataLinq adopts a corrected official dependency and recaptures affected evidence under a new identity. This amendment does not change the original archive, failed runs or earlier baseline measurements.
