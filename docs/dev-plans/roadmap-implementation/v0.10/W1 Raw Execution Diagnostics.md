...
At this checkpoint the async evidence adapters still required a separate check that no-dispatch normalization preserves an underlying Initialization effect or Lost integrity. This slice's four explicit restriction controls exercise the synchronous consumer; they are not proof for all async adapters. The later [no-dispatch recovery restriction follow-through](W1%20No-Dispatch%20Recovery%20Restrictions.md) supplies 30 additional cases and fixes the five async adapters. The complete cause/stage/preflight inventory, higher local boundaries, final cross-family I/O map and internal adapter readiness remain open.
