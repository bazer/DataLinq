...
The [initial performance checkpoint](W1%20Initial%20Performance%20Checkpoint.md) predates this change. These additional scopes and iterator wrappers add successful-path work; the value-type resource owner and lazy failure collector do not make the complete path allocation-free.

A subsequent strict .NET 10 `--allocation-regression --profile heavy --release-evidence` run measured clean runtime commit `b9714c8bac89c20bebbccc1d9d80f42ed041c3d9` against W0, with unchanged source during execution and no concurrent local tests/benchmarks. Run `20260919-214401939-1cd99e9d295d4210ab36e2147432a356` has nine matched rows, complete/valid history and comparison, and unchanged recorded telemetry. Its outcome is **ReviewRequired**, with two allocation warnings: update **17,633.28 B/op versus 15,626.24 (+12.8%)**, and cold relation traversal **15,298.56 versus 13,352.96 (+14.6%)**. The other positive allocation deltas are CRUD batch/small +9.2% and cold primary key +7.0%; these are not excused by falling below the warning threshold. This run has no latency warnings, but does not supersede the earlier timing uncertainty. No allocation increase is waived, no budget is changed and this is not W1 performance acceptance.

All 15 raw artifact receipts were independently verified against their recorded sizes/hashes. The local history `artifacts/benchmarks/history/w1-b9714c8b-allocation-regression.json` has SHA-256 `6e418e500a7605fabd96592830a4561c47ca8017c35c08ae88fbe51944ab86e2`; comparison `artifacts/benchmarks/history/w1-b9714c8b-vs-w0-allocation-regression.json` has SHA-256 `0813e40964924e14d394ea26d6b24d3db874aec77d14b9c56d687e4368564228`. Raw output remains under the referenced run directory and has not been uploaded. The evidence amendment changes documentation only; the measured runtime code remains unchanged. Profile and reduce/explain coordination costs next, then recapture the complete W0 comparison at final integration.
