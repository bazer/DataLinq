...
[Raw eager commands](W1%20Async%20Raw%20Commands.md) now covers internal lower-level scalar/non-query orchestration. Query telemetry/correlation, raw reader and synchronous adapter safety, tracked mutation/hydration, async relation coordination and full relation/index publication, metadata/provisioning and owning-root disposal remain W1 work. Final I/O-path coverage and comparable .NET 10 coordination/allocation evidence are still required. Public query/lookup declarations, provider-free packed async consumers and source-boundary compatibility remain W3/later gates. The 0.9.2 compatibility baseline and SQLite W0-F1 limitation are unchanged.
