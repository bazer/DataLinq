...
- `SQ-1`/`SQ-2` SQLite evidence: owned scalar/reader/non-query/transaction policy, pooled-state reset, deferred serializable transactions, attached-policy preservation, private-WAL pending insert/update/delete isolation, explicit shared-cache lock behavior, generated file defaults without `Cache`, named-memory shared-cache preservation, resolved/opened file paths, rollback/commit, bounded writer timeout, and full SQLite compliance
...
- separate still-open evidence for provider-specific database-outcome classification/constraint after a throwing commit, low-level raw-handle escape prevention, arbitrary local-cache primitive fault injection, full provider commit-fault coverage, and full concurrency semantics
