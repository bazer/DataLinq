...
- bounded `TX-5A` attached evidence: active wrapper-only commit promotion/reuse and rollback invalidation across every provider, plus no guessed publication and `CommitOutcomeUnknown` cache recovery when wrapper commit follows external commit or rollback
- separate still-open evidence for provider-specific database-outcome classification/constraint after a throwing commit, external completion followed by managed read/write/dispose, low-level raw-handle escape closure, arbitrary local-cache primitive fault injection, full provider commit-fault coverage, and full concurrency semantics
...
- the remaining recovery distinction among a provider-call exception recovered as permanent managed `CommitOutcomeUnknown`, external attached completion first observed by managed read/write/dispose, and ordinary managed-wrapper rollback/disposal; bounded `TX-2B` applies only after the provider commit call returned successfully, uncertain-commit recovery evicts provider-wide caches but does not determine the database outcome, bounded `TX-5A` covers active attached wrapper commit/rollback plus wrapper commit after external completion, and bounded `TX-3` applies when rollback/disposal enters through the wrapper
