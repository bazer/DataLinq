...
The `40`/`610`/`352`/`258` counts in the package-tooling paragraph above describe the historical `w10.2` candidate's source checkpoint. The current project-reference M1-F state is Memory `53`, catalog `613`, and SQL `355` supported / `258` unsupported; no fresh package candidate contains the later M1-D, M1-E, or M1-F changes.
