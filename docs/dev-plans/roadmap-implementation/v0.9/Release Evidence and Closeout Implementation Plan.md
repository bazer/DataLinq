...
The `40`/`610`/`352`/`258` counts in the package-tooling paragraph above describe the historical `w10.2` candidate's source checkpoint. The current project-reference M1-G state is Memory `54`, catalog `614`, and SQL `356` supported / `258` unsupported; no fresh package candidate contains the later M1-D, M1-E, M1-F, or M1-G changes.
