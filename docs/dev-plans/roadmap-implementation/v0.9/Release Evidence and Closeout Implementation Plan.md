...
- separate still-open evidence for database-outcome determination and provider-wide committed-cache recovery after the bounded managed `CommitOutcomeUnknown` fence, attached/external completion beyond the narrow rollback fence, low-level raw-handle escape closure, arbitrary local-cache primitive fault injection, full provider commit-fault coverage, and full concurrency semantics
...
- the remaining recovery distinction among a provider-call exception now fenced as permanent managed `CommitOutcomeUnknown`, unsupported attached/external completion beyond the narrow externally completed rollback fence, and ordinary managed-wrapper rollback/disposal; bounded `TX-2B` applies only after the provider commit call returned successfully, the commit-call fence does not determine the database outcome or repair provider-wide committed caches, and bounded `TX-3` applies only when rollback/disposal enters through the wrapper
