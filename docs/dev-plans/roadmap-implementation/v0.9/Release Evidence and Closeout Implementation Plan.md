...
- the remaining recovery distinction among a provider-call exception recovered as permanent managed `CommitOutcomeUnknown`, unsupported attached/external completion beyond the narrow externally completed rollback fence, and ordinary managed-wrapper rollback/disposal; bounded `TX-2B` applies only after the provider commit call returned successfully, uncertain-commit recovery evicts provider-wide caches but does not determine the database outcome, and bounded `TX-3` applies only when rollback/disposal enters through the wrapper
