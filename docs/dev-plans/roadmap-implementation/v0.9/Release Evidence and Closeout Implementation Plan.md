...
**Status:** Accepted. The scoped 0.9 features and practical receipt-producing tools are implemented. Memory M0-M2 closes at Memory `58`, catalog `617`, and SQL `359` supported / `258` unsupported. This plan now owns only a fresh release-candidate run, human review of the receipts, release notes, go/no-go, and publication; it does not require a manifest composer or further report-schema projects.
