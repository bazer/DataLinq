...
The `40`/`610`/`352`/`258` counts in the package-tooling paragraph above describe the historical `w10.2` candidate's source checkpoint. The current project-reference M1-E state is Memory `51`, catalog `613`, and SQL `355` supported / `258` unsupported; no fresh package candidate contains the later M1-D or M1-E changes.
