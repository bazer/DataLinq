...
| Benchmarks | Existing lanes cover the broad query hot path and provider watchpoints. The `v0.9-query-backend` selector now has six focused cases for unbound structural parsing/template creation, the combined production parse/template/initial-bind route, template freeze/validation, scalar-plus-local-sequence binding, request/capability preparation, and repeated pre-parsed SQL-adapter scalar execution. Clean commit `1cb725d4` has a complete two-provider heavy artifact with exact allocations, latency/error, operation counts, and telemetry; two marginal minimum-iteration warnings and SQLite-memory adapter noise remain explicitly recorded. No `DataLinq.Memory` benchmark lane exists yet. | Add the focused Memory scenarios, repeat both new lanes against the final RC, compare the retained SQL lanes where comparable, and explicitly disclose the missing true pre-foundation artifact if it cannot be recovered. |
