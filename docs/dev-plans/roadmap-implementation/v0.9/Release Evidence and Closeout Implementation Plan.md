...
| Benchmarks | Existing lanes cover the broad query hot path and provider watchpoints. The new `v0.9-query-backend` selector has six focused cases for unbound structural parsing/template creation, the combined production parse/template/initial-bind route, template freeze/validation, scalar-plus-local-sequence binding, request/capability preparation, and repeated pre-parsed SQL-adapter scalar execution. Its two-provider smoke produces complete allocation/telemetry/history rows, but the smoke predates the implementing commit and is wiring evidence only. No `DataLinq.Memory` benchmark lane exists yet. | Add the focused Memory scenarios, capture commit-attested default/heavy artifacts, compare the retained SQL lanes where comparable, and explicitly disclose the missing true pre-foundation artifact if it cannot be recovered. |
...
**Partial implementation checkpoint:** `--v09-query-backend` now selects six exact cases: unbound expression parse/structural-template creation, the combined production parse/template/initial-bind route, template freeze/validation from prebuilt nodes, invocation binding with one non-null scalar plus one three-item local sequence, request/capability preparation without a command, and repeated pre-parsed SQL-adapter scalar `Any`. The wiring smoke contains all `12` expected rows across `sqlite-file` and `sqlite-memory`; every row has allocation data, telemetry, `OperationsPerInvoke=1000`, a non-`other` category, and tracking group `v0.9-query-backend`. Adapter rows record one scalar query and zero entity queries/materializations per operation. Because that smoke ran before the implementation commit and the current history schema does not record dirty state, it is not commit-attested performance or release evidence. The Memory lane, clean-commit default/heavy artifacts, written interpretation, and aggregate RE-1G/W10 step-6 closeout remain open.
