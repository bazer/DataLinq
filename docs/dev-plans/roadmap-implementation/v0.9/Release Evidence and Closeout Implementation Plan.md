...
The `40`/`610`/`352`/`258` counts in the package-tooling paragraph above describe the historical `w10.2` candidate's source checkpoint. The current project-reference M1-D state is Memory `49`, catalog `612`, and SQL `354` supported / `258` unsupported; no fresh package candidate contains that later change.
