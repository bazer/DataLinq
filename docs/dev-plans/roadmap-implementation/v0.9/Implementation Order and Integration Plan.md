...
**W8/F7 memory-spike checkpoint:** The solution contains non-packable `DataLinq.Memory` and focused non-packable `DataLinq.Tests.Memory` projects without adding memory to SQL target matrices or the pre-promotion Testing CLI catalog. The runtime stores copied canonical provider rows and per-table primary-key ordinals separately from existing per-table materialized identity caches. It proves atomic canonical seed publication, duplicate-key rejection, source-local store/cache identity, direct neutral primary-key loading, generated immutable materialization, non-concurrent cache eviction without store loss, one shared SQL/memory metadata-resolution lock with a deterministic blocked-caller proof plus 32-way cold-start convergence, and pass-through generated entity queries. The profile now supports exactly 29 tokens: zero or more equality `Where` operations remain restricted to a direct non-nullable `Int32` root column and exact non-null `Int32` scalar binding, while ordering is exactly one ascending or descending direct non-nullable converter-free `Int32` single-column primary key with an optional final exact nonnegative `Int32` scalar-binding `Take`. A final `ScalarMember` sequence may select a primary-key or non-key root column only when its result, model, and provider types are exact non-nullable converter-free `Int32`; the exhaustive scalar-projection shape rejects every broader case before store access. One canonical-row cursor owns filtering, ordering, `Take`, scan accounting, cancellation, and disposal. Entity sequences materialize selected identities through `RowCache`; scalar sequences materialize only the selected canonical cell and perform zero entity-cache work. Unordered projection streams lazily without a stable-order claim, while ordered projection retains full-match buffering; `Take(0)` still skips scanning. Focused memory tests pass `22/22`; capability contracts pass `25/25`; integrated gates pass `1138/1138` unit, `57/57` generator, and `795/795` SQLite file/memory compliance tests. The runtime builds cleanly for net8/net9/net10 with zero warnings or errors; its resolved runtime graphs remain free of SQL-provider and native-database packages. Cancellation evidence still uses the internal spike execution surface because generated public LINQ supplies `CancellationToken.None`. This is an internal exact-`Int32` semantics proof, not support for other scalar types, broader projections, projection terminals, post-projection composition, general ordering, `ThenBy`, `Skip`, stable ties, performance, or SQL parity. W8 step 6 remains partially complete because `Any`/`Count` is open; semantics/parity, concurrent cache maintenance, public preview promotion, Testing CLI registration, constrained-runtime evidence, and release integration also remain open.
