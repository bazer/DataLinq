...
5. `UUID-4`: static defaults, UUID-version diagnostics, validation, and semantic diffing. In progress: the bounded schema-format validation/diff slice is green; static defaults, version diagnostics, canonical/source-only mapping completion, and aggregate evidence remain open.
