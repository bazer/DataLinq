...
**W5/F6-B progress:** The optional neutral index-row request/result and loader capability are live beside, not inside, the primary-key capability. Existing SQL sources reuse one cached loader for both. `TableCache` dispatches only exact single-column integral canonical index keys through it, preserving cold/warm row-cache telemetry, shared-index warming, and immutable identity. A transaction-visible index set is scoped state: after a focused rollback regression reproduced shared-index poisoning, both full-row and key-only relation paths now allow only `ReadOnlyAccess` to publish shared index entries. The gate may now admit converter-backed index metadata when its caller has already supplied the exact canonical integral value; it neither normalizes nor converts a model wrapper, and a raw/noncanonical wrapper remains ineligible. String/CHAR, `Guid`/UUID, composite, custom sources, key-only/preload neutralization, wrapper redesign, memory relations, raw/manual readers, and broader legacy removal remain open. The original F6-B evidence is `6/6` contract tests, `11/11` adapter/route tests, `2/2` provider-key helper tests, and `1/1` SQLite-memory executions for rollback isolation, cold/warm identity and telemetry, and existing cold traversal. The later SC-3 typed-ID relation checkpoint adds `1/1` focused generator coverage, `4/4` active-provider relation executions, and full `58/58` generator, `1138/1138` unit, and `797/797` SQLite file/memory gates; the unchanged primitive and rollback cases each pass `4/4` across the active providers. F6 and W5 remain open.
...
- [ ] Keep string/CHAR, `Guid`/UUID, composite, converter-backed integral types beyond exact `Int32` and `Int64`, raw/noncanonical model-wrapper operands at the neutral dispatch boundary, custom sources, key-only/preload neutralization, relation-wrapper redesign, memory relations, raw/manual readers, and all broader `F6` read/legacy families gated on their own normalization, transaction/cache characterization, and remaining W3 evidence.
