...
5. `UUID-4`: static defaults, UUID-version diagnostics, validation, and semantic diffing. In progress: bounded schema-format validation/diff, exact-D fixed-`Guid` source representation/regeneration, direct-`Guid` DDL literals, Version4/Version7 preservation, fail-closed provider DDL, and raw `UUID()` import are green. Converter-backed defaults, static provider-default import, complete client generation, default ownership/transformer precedence, SQLite expression/BLOB import, automatic server capability/storage mapping, broader canonical-type/source-only mapping completion, and aggregate evidence remain open.
