...
W10 step 3 / RE-1A registration is green. Direct `--suite memory --build --summary-json` and explicit `--suite memory --alias all` runs each pass `77/77` with exactly one result and `Targets` `-`; the direct run leaves the test-infrastructure state hash and timestamp unchanged. The composite `--suite all --alias quick --build` run passes `2162/2162`: generators `60`, unit `1214`, memory `77` once, and compliance `811` across `sqlite-file` and `sqlite-memory`. The list surface identifies the targetless project lane. `DataLinq.Tests.Memory` intentionally references SQLite for its bounded differential-parity fixtures, so these are project-based Testing CLI results, not provider-free constrained-runtime or package-consumer evidence. W10 remains open for the other steps above.
