...
**Status:** Implementation in progress. W0-W2 are complete; W3 and W4 are active on their gated lanes, and bounded `F6-A` integral-canonical primary-key/cache-cold routing is green under the W3 core-fault gate without making W5 complete. Bounded `SC-2` SQL projection slices are also green for root and joined converter-backed `ScalarMember` results, converter-backed column members in aliased anonymous and explicit-constructor `SqlRow` projections, and model-compatible converter-backed direct `GroupedAggregate` keys without completing W4 or W5. Single-source integral-keyed `ComputedRowLocal` `NewArray` recipe evidence is also green under `AotStrict`. Primitive-key `JoinedRowLocal` `NewArray` SQL execution preserves converter-backed joined-source values while retaining the intentional `SqlOnlyCompatibility` fence. Bounded UUID provider-consumption slices now apply resolved text/native/binary formats to non-primary-key full-row canonical `Guid` mutation and materialization on SQLite, MySQL, and MariaDB, including direct values, representative binary converter-backed typed IDs, and nullable direct/typed Text36/SQL NULL. Bounded `UUID-3A` proves the existing SC-4 canonical-column operand path binds exact resolved non-key UUID parameters for direct equality across every bounded format, representative direct/typed binary equality, inequality, and membership, and nullable direct/typed Text36 non-null and literal-null predicates under no or adversarial connector `GuidFormat`. Converter-backed `Sum`/`Min`/`Max`/`Average` selectors now fail translation before SQL until the converter contract grows explicit aggregate-preserving capability metadata; selectorless count/existence operations remain available.
...
Known-value vectors, bounded `UUID-1A` declaration/codec primitives, and `UUID-1B` immutable provider-keyed resolution are green after `SC-1`. Bounded `UUID-2` checkpoints now consume those definitions for non-primary-key full-row canonical `Guid` reads and mutation writes across the SQLite and MySQL/MariaDB text/native/binary matrices. Direct values cover every bounded provider format; representative converter-backed typed-ID evidence covers SQLite RFC BLOB, MySQL little-endian binary, MariaDB RFC binary, and nullable Text36/SQL NULL. Bounded `UUID-3A` consumes the same resolved definitions through the existing SC-4 operand/writer route for non-key direct equality across every bounded format, representative direct/typed binary equality, inequality, and membership, and nullable direct/typed Text36 non-null and literal-null predicates. Aggregate `UUID-2` remains open for other independently instantiated typed formats, UUID primary keys, projections, defaults, provider-less public-reader fallback removal, and other reader routes; aggregate `UUID-3` remains open for captured-null and null-containing-sequence shapes, manual string-only queries, keys/cache/relations/update-delete predicates, and remaining query routes. Wave 7 remains the integration/completion checkpoint after the scalar key/query/schema prerequisites are ready. Execute and close the UUID plan in its granular dependency order:
...
4. `UUID-3`: equality, nullable equality, membership, keys, cache, relations, update/delete predicates, and column-aware explicit queries. In progress: bounded `UUID-3A` is green for column-aware non-primary-key expression-query equality and representative membership on SQLite, MySQL, and MariaDB.
