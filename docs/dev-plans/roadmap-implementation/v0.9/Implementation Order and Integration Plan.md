...
**Status:** Implementation in progress. W0-W2 are complete; W3 and W4 are active on their gated lanes, and bounded `F6-A` integral-canonical primary-key/cache-cold routing is green under the W3 core-fault gate without making W5 complete. Bounded `UUID-3C` extends that neutral source-row route to exactly one canonical `Guid` primary-key component only when the concrete SQLite, MySQL, or MariaDB source has resolved `GuidStorage`; direct `Guid` and a representative Guid-backed typed ID use exact column codecs for key selection, cold single/batched loading, warm identity, transaction authoritative reload, and mutation predicates. Bounded `SC-2` SQL projection slices are green for root and joined converter-backed `ScalarMember` results, converter-backed column members in aliased anonymous and explicit-constructor `SqlRow` projections, and model-compatible converter-backed direct `GroupedAggregate` keys without completing W4 or W5. Single-source integral-keyed `ComputedRowLocal` `NewArray` recipe evidence is green under `AotStrict`. Primitive-key `JoinedRowLocal` `NewArray` SQL execution preserves converter-backed joined-source values while retaining the intentional `SqlOnlyCompatibility` fence. Bounded UUID provider-consumption slices apply resolved text/native/binary formats to non-primary-key full-row canonical `Guid` mutation and materialization on SQLite, MySQL, and MariaDB, including direct values, representative binary converter-backed typed IDs, and nullable direct/typed Text36/SQL NULL. `UUID-3A` proves exact resolved non-key UUID parameters for the initial equality/membership matrix, while bounded `UUID-3B` makes captured scalar nullness and nullable local-sequence total/null counts structural, rejects mismatched explicit rebinding, and renders CLR-correct nullable membership while binding only non-null values through the column codec. Converter-backed `Sum`/`Min`/`Max`/`Average` selectors fail translation before SQL until the converter contract grows explicit aggregate-preserving capability metadata; selectorless count/existence operations remain available. W3, W5/F6, UUID-2, and UUID-3 remain incomplete.

**W5/F4-A progress:** The first three W5 contract steps now have a bounded implementation: a finite contextual feature catalog, exhaustive recursive requirement extraction with separate invocation shapes, a complete initial SQL disposition table, and deterministic capability diagnostics. The current catalog freezes 598 features and the SQL profile freezes 343 supported versus 255 unsupported dispositions, including the renderer's source-topology, join-right-role, pushdown, relation-direction, Boolean-predicate, nullable-comparison, function-shape, paging-count, grouped-projection, and aggregate-selector fences. Neutral function templates enforce the finite legal arity contract while preserving two-argument `Substring`; the SQL profile separately rejects that shape because the retained renderer still requires an explicit length. Invocation-sensitive paging requirements reject null, negative, and non-convertible or overflowing counts without retaining their values. The focused capability suite passes `18/18` and the full unit gate passes `1077/1077`. No production route invokes the validator yet, no memory capabilities are claimed, and no SQL builder or cache/read family has moved behind a backend adapter. The exactly-once gate and F5 entity-family adapter remain the next W5 dependency; W5 is not complete.
...
Bounded `F4-A` landed the shared vocabulary, recursive structural/invocation requirement model, exact initial SQL matrix, and diagnostic shape without changing execution routing. That separation is intentional: the next slice must create one validated execution request, move terminal-primary-key optimization behind it, and adapt the complete entity result family without validating twice or returning an unconstrained reader-backed sequence. The memory capability profile remains coupled to the W8 spike rather than being guessed here.
