...
**Status:** Implementation in progress. W0-W2 are complete; W3-W5 are active on their gated lanes. The bounded W3/W4 progress below covers neutral row/materialization, primary-key loading, scalar conversion, and UUID storage without completing those workstreams. Bounded `F4-B/F5-A` now adds the production exactly-once capability gate and the complete entity sequence/six-terminal SQL adapter, including post-validation scalar-primary-key optimization and explicit cursor/command ownership. Scalar/aggregate, direct-projection, and retained local-projection execution still use compatibility paths after validation, and no memory capability profile or backend is claimed. W3, W5/F6, UUID-2, and UUID-3 remain incomplete.

**W5/F4-B/F5-A progress:** The initial contract remains frozen at 598 contextual features and 343 supported versus 255 unsupported SQL dispositions. Every production expression query now creates one request, selects the source-owned SQL backend, extracts requirements, and validates exactly once before backend work. The SQL adapter owns entity sequences and all six entity terminals. Its scalar-primary-key shortcut is recognized only after validation, and unsupported two-argument `Substring` sequence and terminal plans fail with a redacted capability diagnostic and zero command, query, cache, row-load, or materialization work. The entity cursor owns early/completed/faulted/cancelled enumeration disposal, and `Select` owns the command for its lazy reader iterator. Integrated gates pass `1088/1088` unit, `772/772` SQLite compliance, `780/780` for the paired MySQL 8.4 plus MariaDB 10.11 run, and `780/780` for the paired MariaDB 11.4 plus MariaDB 11.8 run. Memory capabilities/backend and the scalar/aggregate, direct-projection, retained local-projection, cache/relation, and compatibility-removal work remain open; W5 is not complete.
...
Bounded `F4-A` landed the shared vocabulary, recursive structural/invocation requirement model, exact initial SQL matrix, and diagnostic shape. Bounded `F4-B/F5-A` now creates one validated execution request for every production expression query, selects and validates the source-owned SQL backend exactly once, moves terminal-primary-key recognition behind that gate, and adapts the complete entity sequence/six-terminal family with owned cursor and command lifetimes. The next result-family migration is scalar/aggregate, followed by direct projection and retained local recipes. The memory capability profile remains coupled to the W8 spike rather than being guessed here.
