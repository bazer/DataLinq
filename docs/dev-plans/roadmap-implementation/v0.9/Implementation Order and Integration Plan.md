...
**Status:** Implementation in progress. W0-W2 are complete; W3-W5 are active on their gated lanes. The bounded W3/W4 progress below covers neutral row/materialization, primary-key loading, scalar conversion, and UUID storage without completing those workstreams. Bounded `F4-B/F5-A/F5-B/F5-C` now adds the production exactly-once capability gate plus complete entity, scalar-result, and direct-projection SQL adapters, including post-validation scalar-primary-key optimization and explicit result/cursor/cancellation/command ownership. Retained local-projection execution still uses its compatibility path after validation, and no memory capability profile or backend is claimed. W3, W5/F6, UUID-2, and UUID-3 remain incomplete.

**W5/F4-B/F5-A/F5-B/F5-C progress:** The initial contract remains frozen at 598 contextual features and 343 supported versus 255 unsupported SQL dispositions. Every production expression query creates one request, selects the source-owned SQL backend, extracts requirements, and validates exactly once before backend work. The SQL adapter owns entity sequences and terminals, all six scalar reductions, and the supported direct-projection family: `ScalarMember`, `SqlRow`, and `GroupedAggregate` sequences plus parser-supported scalar/row terminals. The direct adapter returns semantic typed results through an owned cursor and keeps SQL aliases, readers, commands, `DBNull`, provider numeric widths, converter decoding, and reflective construction behind the backend boundary. Unsupported two-argument `Substring` entity, scalar, and direct-projection plans fail with a redacted capability diagnostic and zero command, query, cache, row-load, or materialization work. Cancellation and command/reader disposal are explicit; transaction reader telemetry is preserved without relabeling projection reads as entity or scalar query metrics. Integrated gates pass `1109/1109` unit, `780/780` SQLite compliance, `788/788` for each paired server run, and `160/160` plus `162/162` provider-specific executions. Memory capabilities/backend and the retained local-projection, cache/relation, and compatibility-removal work remain open; W5 is not complete.
...
Bounded `F4-A` landed the shared vocabulary, recursive structural/invocation requirement model, exact initial SQL matrix, and diagnostic shape. Bounded `F4-B/F5-A/F5-B/F5-C` now creates one validated execution request for every production expression query, selects and validates the source-owned SQL backend exactly once, moves terminal-primary-key recognition behind that gate, and adapts the complete entity, scalar-result, and currently supported direct-projection families with owned result, cursor, cancellation, command, and reader lifetimes. The next result-family migration is retained local recipes. The memory capability profile remains coupled to the W8 spike rather than being guessed here.
