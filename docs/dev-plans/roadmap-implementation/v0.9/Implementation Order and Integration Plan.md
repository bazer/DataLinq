...
- [ ] Complete SC-2 for indexed defaults, rows with unknown non-auto keys, collation/codec-sensitive primary keys, UUID provider integration, explicit-constructor and other direct-row evidence, joined/function-derived group keys, aggregate capability metadata and supported converter-backed aggregate execution, general `HAVING`, retained local projection materialization, and foreign-key/relation/index routing; bounded integral-canonical primary-key/cache-cold loading, converter-backed non-indexed default hydration, SQL `ScalarMember` conversion, aliased anonymous `SqlRow` conversion, direct model-compatible `GroupedAggregate` key conversion, and the converted-aggregate semantics guard are green.
