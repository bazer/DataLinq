...
Known-value vectors, bounded `UUID-1A` declaration/codec primitives, and `UUID-1B` immutable provider-keyed resolution are green after `SC-1`. Bounded `UUID-2` checkpoints now consume those definitions for non-primary-key full-row canonical `Guid` reads and mutation writes across the SQLite and MySQL/MariaDB text/native/binary matrices. Direct values cover every bounded provider format; representative converter-backed typed-ID evidence covers SQLite RFC BLOB, MySQL little-endian binary, MariaDB RFC binary, and nullable Text36/SQL NULL. Aggregate `UUID-2` remains open for other independently instantiated typed formats, UUID primary keys, projections, predicates/membership, relations, defaults, provider-less public-reader fallback removal, and other reader routes. Wave 7 remains the integration/completion checkpoint after the scalar key/query/schema prerequisites are ready. Execute and close the UUID plan in its granular dependency order:
