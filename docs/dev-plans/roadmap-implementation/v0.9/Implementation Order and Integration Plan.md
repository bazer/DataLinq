...
**W8/F7 first memory-spike checkpoint:** The solution now contains non-packable `DataLinq.Memory` and focused non-packable `DataLinq.Tests.Memory` projects without adding memory to SQL target matrices or the pre-promotion Testing CLI catalog. The runtime stores copied canonical provider rows and per-table primary-key ordinals separately from existing per-table materialized identity caches. It proves atomic canonical seed publication, duplicate-key rejection, source-local store/cache identity, direct neutral primary-key loading, generated immutable materialization, non-concurrent cache eviction without store loss, one shared SQL/memory metadata-resolution lock with a deterministic blocked-caller proof plus 32-way cold-start convergence, and one pass-through generated entity query. The memory capability profile supports exactly the eight baseline root/entity/sequence tokens; `Where` and every other unimplemented feature reject before enumeration. Internal cancellation evidence does not imply a public cancellable-LINQ API. Focused tests pass `6/6`; the integrated unit and generator gates pass `1129/1129` and `57/57`. The memory assembly builds for net8/net9/net10 with trim/AOT analyzers enabled and has no SQLite, MySQL, connector, or native package dependency. W8 steps 6-10, concurrent cache maintenance, public preview promotion, Testing CLI registration, and release integration remain open.
