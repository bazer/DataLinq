...
5. `UUID-4`: static defaults, UUID-version diagnostics, validation, and semantic diffing. In progress: bounded schema-format validation/diff, finalized-metadata direct-`Guid` DDL literals, Version4/Version7 preservation, fail-closed provider DDL, and raw `UUID()` import are green. Source fixed-`Guid` representation/regeneration, converter-backed defaults, complete client generation, default ownership/transformer precedence, SQLite expression import, automatic server capability/storage mapping, canonical/source-only mapping completion, and aggregate evidence remain open.
