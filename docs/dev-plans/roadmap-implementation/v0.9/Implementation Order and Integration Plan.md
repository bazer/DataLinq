...
**Status:** Implementation in progress. W0-W2 are complete; W3-W5 are active on their gated lanes. The bounded W3/W4 progress below covers neutral row/materialization, primary-key loading, scalar conversion, and UUID storage without completing those workstreams. Bounded `F4-B/F5-A/F5-B` now adds the production exactly-once capability gate plus complete entity and scalar-result SQL adapters, including post-validation scalar-primary-key optimization and explicit result/cursor/cancellation/command ownership. Direct-projection sequences/row terminals and retained local-projection execution still use compatibility paths after validation, and no memory capability profile or backend is claimed. W3, W5/F6, UUID-2, and UUID-3 remain incomplete.

**W5/F4-B/F5-A/F5-B progress:** The initial contract remains frozen at 598 contextual features and 343 supported versus 255 unsupported SQL dispositions. Every production expression query creates one request, selects the source-owned SQL backend, extracts requirements, and validates exactly once before backend work. The SQL adapter owns entity sequences, all six entity terminals, and the six-kind scalar-result family (`Count`, `Any`, `Sum`, `Min`, `Max`, and `Average`), including supported direct-projection, joined, paged, and grouped-projection reductions. Its scalar-primary-key shortcut is recognized only after validation, and unsupported two-argument `Substring` sequence, terminal, and scalar plans fail with a redacted capability diagnostic and zero command, query, cache, row-load, or materialization work. Entity cursor/lazy-reader ownership is unchanged; scalar execution now keeps SQL representation conversion inside the adapter, checks cancellation after command creation, and disposes the command on every exit. Integrated gates pass `1097/1097` unit, `774/774` SQLite compliance, `782/782` for each paired server run, and `160/160` plus `162/162` provider-specific executions. Memory capabilities/backend and the direct-projection sequence/row-terminal, retained local-projection, cache/relation, and compatibility-removal work remain open; W5 is not complete.
...
Bounded `F4-A` landed the shared vocabulary, recursive structural/invocation requirement model, exact initial SQL matrix, and diagnostic shape. Bounded `F4-B/F5-A/F5-B` now creates one validated execution request for every production expression query, selects and validates the source-owned SQL backend exactly once, moves terminal-primary-key recognition behind that gate, and adapts the complete entity sequence/six-terminal and scalar-result families with owned result, cursor, cancellation, and command lifetimes. The next result-family migration is direct projection, followed by retained local recipes. The memory capability profile remains coupled to the W8 spike rather than being guessed here.
