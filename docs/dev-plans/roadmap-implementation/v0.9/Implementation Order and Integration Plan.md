...
**W8/F7 provider-free constrained-runtime checkpoint:** `DataLinq.Memory.PlatformCompatibility.Smoke` is a generated-model runner referenced by separate non-packable `DataLinq.Memory.AotSmoke`, `DataLinq.Memory.TrimSmoke`, and `DataLinq.Memory.BlazorWasm` hosts. The unchanged 31-token profile executes canonical and model-valued seeding, primary-key hit/miss, captured exact-`Int32` equality, primary-key ordering plus `Take`, entity materialization, direct exact-`Int32` projection, `Any`/`Count`, deterministic self-join rejection before work, pre-cancellation, and canonical Guid-backed key/direct-`Guid` storage. Native AOT and full-trim executables publish and exit successfully; isolated WebAssembly no-AOT and AOT publishes execute successfully in a real browser with zero warning/error entries. A recursive scan of all four published outputs finds none of `DataLinq.SQLite`, `DataLinq.MySql`, `Microsoft.Data.Sqlite`, `MySqlConnector`, `SQLitePCLRaw`, or `e_sqlite3`. Switching WebAssembly AOT modes must use clean or isolated intermediate paths because sharing one `obj` graph can retain mode-specific stripped IL. The full memory suite now passes `42/42`. This completes bounded W8 step 10 and the `F7` constrained-runtime vertical proof without widening capabilities, exposing a public seed API, making `DataLinq.Memory` packable, registering release compatibility targets, or replacing the existing SQLite smoke graph.
...
The bounded W8 step-10 graph satisfies the constrained-runtime execution and dependency-isolation part of this gate for the current 31-token profile. D5-A freezes the minimal public construction, generated-mutable seed, query, and failure surface and reruns that path in all four constrained hosts. Independent surface review is green. D5 remains open for D5-B until actual package dependency/assets inspection and an explicit promotion decision are complete; project-reference evidence is not package-promotion evidence.
