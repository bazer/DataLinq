...
The first two exit conditions are the completed `F5` boundary. The third keeps W5 open: `F6` still owns neutral primary-key, joined-key, cache, relation, and legacy-reader routing, while the memory capability profile/backend remains W8 work. In particular, the local SQL adapter preserves rather than generalizes its old mechanics: single-source recipes use synchronous entity loading with bounded outer cancellation checks; joined recipes buffer provider keys and hydrate through `TableCache`; and their existing entity-query, reader-command, transaction, and cache telemetry classifications remain route-specific.
