...
- [ ] Complete SC-2 for indexed defaults, rows with unknown non-auto keys, collation/codec-sensitive primary keys, UUID provider integration, explicit-constructor and other direct-row evidence, joined/function-derived group keys, converter-backed aggregate values, `HAVING`, retained local projection materialization, and foreign-key/relation/index routing; bounded integral-canonical primary-key/cache-cold loading, converter-backed non-indexed default hydration, SQL `ScalarMember` conversion, aliased anonymous `SqlRow` conversion, and direct model-compatible `GroupedAggregate` key conversion are green.
...
Current integrated evidence after the direct SQL `GroupedAggregate` key slice is `1051/1051` unit, `57/57` generator, `746/746` SQLite compliance, and `748/748` MySQL/MariaDB compliance. This is implementation evidence, not the final W11 or W13 frozen-candidate rerun.
