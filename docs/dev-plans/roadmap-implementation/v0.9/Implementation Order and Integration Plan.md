...
**W8/F7 memory-spike checkpoint:** The solution contains non-packable `DataLinq.Memory` and focused non-packable `DataLinq.Tests.Memory` projects without adding memory to SQL target matrices or the pre-promotion Testing CLI catalog. The runtime stores copied canonical provider rows and per-table primary-key ordinals separately from existing per-table materialized identity caches. It proves atomic canonical seed publication, duplicate-key rejection, source-local store/cache identity, direct neutral primary-key loading, generated immutable materialization, non-concurrent cache eviction without store loss, one shared SQL/memory metadata-resolution lock with a deterministic blocked-caller proof plus 32-way cold-start convergence, and pass-through generated entity queries. The profile now supports exactly 26 tokens: zero or more equality `Where` operations remain restricted to a direct non-nullable `Int32` root column and exact non-null `Int32` scalar binding, while ordering is exactly one ascending or descending direct non-nullable converter-free `Int32` single-column primary key with an optional final exact nonnegative `Int32` scalar-binding `Take`. Scope-wide ordering and paging-composition requirements plus shared source/result CLR invariants reject multiple keys/`ThenBy`, manual repeated orderings, non-key/string/converted keys, unordered/repeated `Take`, and negative/non-exact counts before store access. Explicit ordering conversions remain represented in the plan, a new primary ordering resets every prior top-level ordering in its scope, and root/separated `ThenBy` rejects. Ordered execution filters canonical rows before a cancellation-aware buffered merge sort, applies `Take`, and lazily materializes only selected identities; it neither subtracts keys nor promises top-N streaming. The unique admitted key makes order total without defining tie stability. `Take(0)` skips scanning, over-cardinality counts return every ordered row, and public count arguments follow normal query-construction snapshot semantics. Focused memory tests pass `17/17`; capability contracts pass `24/24`; integrated gates pass `1137/1137` unit, `57/57` generator, and `795/795` SQLite file/memory compliance tests. The runtime builds cleanly for net8/net9/net10 with zero warnings or errors; its resolved runtime graphs remain free of SQL-provider and native-database packages. Cancellation evidence still uses the internal spike execution surface because generated public LINQ supplies `CancellationToken.None`. This is an internal exact-`Int32` semantics proof, not general ordering, `ThenBy`, `Skip`, stable ties, performance, or SQL parity. W8 step 6 remains partially complete because projection and `Any`/`Count` are open; semantics/parity, concurrent cache maintenance, public preview promotion, Testing CLI registration, constrained-runtime evidence, and release integration also remain open.
