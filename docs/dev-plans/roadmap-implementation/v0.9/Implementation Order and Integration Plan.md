...
**Status:** Implementation in progress. W0-W2, `F5`, the ordered W8 spike, D5 package promotion, the bounded M0-A public exact single-column lookup, and W10 step-3 Testing CLI memory-suite registration are complete; W3-W6 and the remaining W9 work stay active on their gated lanes. The bounded W3/W4 progress below covers neutral row/materialization, primary-key loading, exact single-column integral and resolved canonical-`Guid` index-row families, neutral test-source composition, scalar conversion, and UUID storage without completing those workstreams. Bounded `F4-B/F5-A/F5-B/F5-C/F5-D` adds the production exactly-once capability gate plus complete SQL adapters for every retained expression-query result family, including post-validation scalar-primary-key optimization and explicit result/cursor/cancellation/command ownership. The memory preview adds canonical read-only store state, public model-valued `Find<TModel>(object)` for exactly one primary-key column, pass-through entity scans, exact direct-`Int32` equality plus exact non-null direct-`Guid` and resolved Guid-backed typed-ID column-to-scalar equality in the unchanged exhaustive 32-token profile, one exhaustively capability-gated total-order/optional-`Take` slice over an exact `Int32` single-column primary key, one final exact direct-`Int32` scalar sequence projection, selectorless `Any`/`Count` reductions, bounded cancellation plus unsupported join/grouping/projection diagnostic evidence, focused SQLite/memory parity for the exact admitted island, and direct execution under Native AOT, full trimming, WebAssembly no-AOT, and WebAssembly AOT without SQL-provider or native-database payload. D5-A froze the earlier public construction/seed/query surface, D5-B promoted `DataLinq.Memory` as a locally inspected experimental package, and M0-A adds no generated `Get(...)` method or LINQ capability. Aggregate M0-M2, the wider W9 query/semantic work, remaining typed-ID/`Guid` cases, composite lookup, the raw-SQL-rejection boundary, and the remaining W10 compatibility/package/release integration remain open. W3, W5/F6, W6, UUID-2, and UUID-3 remain incomplete.
...
2. **Bounded M0-A complete:** seed validation, isolated stores, generated startup, and public exact single-column primary-key lookup are proven; Testing CLI registration is now complete, while aggregate `M0` remains open on the raw-SQL-rejection boundary.
