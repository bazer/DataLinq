...
**W8/F7 memory-spike checkpoint:** The solution contains non-packable `DataLinq.Memory` and focused non-packable `DataLinq.Tests.Memory` projects without adding memory to SQL target matrices or the pre-promotion Testing CLI catalog. The runtime stores copied canonical provider rows and per-table primary-key ordinals separately from existing per-table materialized identity caches. It proves atomic canonical seed publication, duplicate-key rejection, source-local store/cache identity, direct neutral primary-key loading, generated immutable materialization, non-concurrent cache eviction without store loss, one shared SQL/memory metadata-resolution lock with a deterministic blocked-caller proof plus 32-way cold-start convergence, and pass-through generated entity queries. The profile now supports exactly 17 tokens and additionally executes repeated direct `Where` equality filters only for an exact non-nullable `Int32` root column and exact non-null `Int32` scalar binding. Scalars normalize once per invocation; capture rebinding, operand reversal, literal bindings, sequential short-circuiting, empty completion, canonical-row filtering before materialization, identity reuse, and bounded cancellation have focused evidence. That cancellation coverage uses the internal spike execution surface; generated public LINQ still supplies `CancellationToken.None`, so it is not a public cancellable-query claim. String and column-to-column equality, nullable/mismatched binding declarations, promoted numeric comparisons, inequality, ordering, and paging reject through capability metadata before store access. Focused memory tests pass `11/11`; capability contracts pass `19/19`; integrated gates pass `1130/1130` unit, `57/57` generator, and `792/792` SQLite file/memory compliance tests. The runtime builds cleanly for net8/net9/net10 with zero warnings or errors; its dependency closure remains free of SQL-provider and native-database packages. This is an internal exact-`Int32` spike, not general `Where`/numeric support or a SQL-parity claim. W8 step 6 is only partially entered; the remaining ordering-plus-`Take`, projection, `Any`/`Count`, semantics/parity, concurrent cache maintenance, public preview promotion, Testing CLI registration, constrained-runtime evidence, and release integration remain open.
