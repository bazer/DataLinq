...
The bounded W8 step-10 graph satisfied the constrained-runtime execution and dependency-isolation part of this gate for its then-current 31-token profile. D5-A freezes the minimal public construction, generated-mutable seed, query, and failure surface and reruns that path in all four constrained hosts. Independent surface review is green. D5-B packs core and Memory together as local `0.9.0-preview.d5b.5` candidates after adding private MinVer and SourceLink build dependencies and setting the runtime packable. The package embeds a dedicated Memory preview README with the bounded supported surface and explicit unsupported mutation, transaction, durability, persistence, raw SQL, relation, join/grouping, projection, and general-LINQ boundaries. Direct nuspec/archive inspection proves matching package versions, one same-candidate `DataLinq` minimum per net8/net9/net10 dependency group, exactly three managed runtime assemblies, exactly three symbol PDBs, and no analyzer/runtime/native/build/tool or SQL-provider/native-database/Roslyn/Remotion/generator payload. The explicit package report has zero findings, and an ordinary MinVer-driven pack no longer fails with stable-package/prerelease-dependency `NU5104`. The explicit experimental-preview promotion decision is green; D5 and W9 step 1 are complete. W10 still owns release-script/report presets, Testing CLI/catalog integration, packaged compatibility reruns, final public wording, and publication.
...
W10 steps 1-2 / RE-1D are green on the fresh `0.9.0-preview.w10.2` six-package candidate and default schema `v0.9.package-inspection-report.v3` report described in the checkpoint above. W10 step 3 / RE-1A registration is also green. Direct `--suite memory --build --summary-json` and explicit `--suite memory --alias all` runs each passed `77/77` at that registration checkpoint with exactly one result and `Targets` `-`; the direct run left the test-infrastructure state hash and timestamp unchanged. The then-current composite `--suite all --alias quick --build` run passed `2162/2162`: generators `60`, unit `1214`, memory `77` once, and compliance `811` across `sqlite-file` and `sqlite-memory`. The current integrated quick gate now passes `2244/2244`: generators `60`, unit `1233`, memory `140`, and compliance `811`. `DataLinq.Tests.Memory` intentionally references SQLite for bounded differential parity, so neither result is provider-free constrained-runtime or package-consumer evidence. W10 step 4 / RE-1C now registers the separate eight-target catalog and green reporting contract with focused `CompatibilitySizeReportTests` at `18/18`; it deliberately claims no fresh target or package execution. W10 steps 5-9 and aggregate W10 remain open.
