...
Known-value vectors, bounded `UUID-1A` declaration/codec primitives, and `UUID-1B` immutable provider-keyed resolution are green after `SC-1`. Bounded `UUID-2` checkpoints now consume those definitions for direct non-primary-key, non-converter full-row `Guid` reads and mutation writes across SQLite Text36/Text32/little-endian BLOB/RFC BLOB and MySQL/MariaDB native MariaDB UUID/Text36/Text32/little-endian `BINARY(16)`/RFC `BINARY(16)`, plus nullable Text36/SQL NULL. Aggregate `UUID-2` remains open for typed IDs, UUID primary keys, projections, predicates/membership, relations, defaults, provider-less public-reader fallback removal, and other reader routes. Wave 7 remains the integration/completion checkpoint after the scalar key/query/schema prerequisites are ready. Execute and close the UUID plan in its granular dependency order:
...
3. `UUID-2`: provider reads, writes, mutations, and generated/default hydration. In progress: direct non-primary-key, non-converter full-row `Guid` reads plus insert/update writes are green for the bounded SQLite and MySQL/MariaDB text/native/binary matrices, with nullable Text36/SQL NULL evidence.
...
- [ ] Complete SC-2 for indexed defaults, rows with unknown non-auto keys, collation/codec-sensitive primary keys, typed-ID/projection/predicate/relation/default UUID routes on SQLite and MySQL/MariaDB, provider-less public MySQL/MariaDB reader fallback removal, member-init row evidence, joined/function-derived group keys, aggregate capability metadata and supported converter-backed aggregate execution, general `HAVING`, joined-local key handoff beyond primitive integral keys and neutral local-recipe hydration beyond integral SQL keys, and foreign-key/relation/index routing; bounded integral-canonical primary-key/cache-cold loading, converter-backed non-indexed default hydration, SQL `ScalarMember` conversion, aliased anonymous and explicit-constructor `SqlRow` conversion, direct model-compatible `GroupedAggregate` key conversion, single-source integral-keyed `ComputedRowLocal` `NewArray` recipe execution under `AotStrict`, primitive-key `JoinedRowLocal` `NewArray` SQL execution over converter-backed joined-source values, direct non-primary-key/non-converter full-row `Guid` mutation and materialization across the SQLite and MySQL/MariaDB text/native/binary matrices plus nullable Text36/SQL NULL, and the converted-aggregate semantics guard are green.
