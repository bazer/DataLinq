...
**SC-6A exact canonical-`Guid` equality checkpoint:** The current exhaustive Memory capability profile contains 32 tokens and admits exact default-null-semantics column-to-scalar equality only when one direct non-null root column is either an identity-mapped `Guid` or a resolved scalar-converter-backed model type canonicalized to `Guid`, and the scalar is non-null. Both operand orders, selective hits and misses, invocation rebinding, and entity `Any`/`Count` are proven. Each captured model scalar crosses the existing `ModelValueConverter` exactly once per predicate during execution-plan compilation; row evaluation then compares exact canonical `Guid` values. No `GuidStorage` definition, SQL-provider codec, byte/text representation, or parsing participates. The public construction, generated-mutable `Seed(...)`, and `Query()` surface is unchanged. Converter failures cross the public memory boundary as value-redacted `QueryTranslationException` instances without an arbitrary retained inner exception graph; `OperationCanceledException`, `OutOfMemoryException`, and `AccessViolationException` propagate with exact identity. Nullable columns or bindings, inequality, local membership, `Guid` ordering, scalar projection, typed-ID member unwrapping, relations, and public typed primary-key lookup remain unsupported and reject before memory row work. One same-invocation differential fixture executes each parsed `QueryPlanInvocation` unchanged through memory and independently raw-seeded SQLite for the bounded direct and converted `Guid` hit/miss and reduction matrix; this is exact regression pressure, not general SQL parity. Current gates pass `62/62` memory, `1214/1214` unit, and `60/60` generator tests, and `DataLinq.Memory` builds cleanly for net8, net9, and net10. Native AOT and full-trim executables pass, isolated real-browser WebAssembly no-AOT and AOT runs pass without warning/error entries, and all four published outputs scan clean for SQL-provider and native-database payloads. At the SC-6A checkpoint, D5-B/package promotion, the remaining query shapes, aggregate SC-6, and aggregate W6 remained open.
