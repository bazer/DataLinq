...
**Status:** Implementation in progress. W0-W2, `F5`, the ordered W8 spike, and D5 package promotion are complete; W3-W6 and the remaining W9 work stay active on their gated lanes. The bounded W3/W4 progress below covers neutral row/materialization, primary-key loading, exact single-column integral and resolved canonical-`Guid` index-row families, neutral test-source composition, scalar conversion, and UUID storage without completing those workstreams. Bounded `F4-B/F5-A/F5-B/F5-C/F5-D` adds the production exactly-once capability gate plus complete SQL adapters for every retained expression-query result family, including post-validation scalar-primary-key optimization and explicit result/cursor/cancellation/command ownership. The memory preview adds canonical read-only store state, direct primary-key loading, pass-through entity scans, exact direct-`Int32` equality plus exact non-null direct-`Guid` and resolved Guid-backed typed-ID column-to-scalar equality in the current exhaustive 32-token profile, one exhaustively capability-gated total-order/optional-`Take` slice over an exact `Int32` single-column primary key, one final exact direct-`Int32` scalar sequence projection, selectorless `Any`/`Count` reductions, bounded cancellation plus unsupported join/grouping/projection diagnostic evidence, focused SQLite/memory parity for the exact admitted island, and direct execution under Native AOT, full trimming, WebAssembly no-AOT, and WebAssembly AOT without SQL-provider or native-database payload. D5-A freezes its minimal public surface, and D5-B promotes `DataLinq.Memory` as a locally inspected experimental package. M0-M2, the wider W9 query/semantic work, remaining typed-ID/`Guid` cases, and all W10 release integration remain open. W3, W5/F6, W6, UUID-2, and UUID-3 remain incomplete.
