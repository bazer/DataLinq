...
5. `UUID-4`: static defaults, UUID-version diagnostics, validation, and semantic diffing. In progress: bounded schema-format validation/diff and finalized-metadata direct-`Guid` DDL-literal slices are green; source representation/regeneration, converter-backed defaults, dynamic/version semantics, canonical/source-only mapping completion, and aggregate evidence remain open.
