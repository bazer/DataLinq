...
Progress through 2026-07-12: `TX-0` is already closed by W1 characterization. W3 has the bounded `ML-1` provenance substrate and the executable `ML-2` pre-execution guard slice. Existing mutables capture exact provider origin, transaction-local provenance uses an opaque exact-transaction token, and authoritative hydration/delete establish transaction-local state. Every supported model-mutation route rejects read-only/terminal wrappers, invalid/deleted or wrong-owner mutables, unsupported row shapes, primary-key drift, and foreign metadata before command construction; no-change, callback/generated, immutable-delete, and public `StateChange` routes have focused backstops. `TX-1A` records successfully transitioned lifecycle mutables by reference identity. `TX-1B` now owns a private ordered collection of only fully finalized mutations; the public `Changes` list is a detached snapshot and public `StateChange.ExecuteQuery(...)` enters the same transaction-owned path. Successful execution freezes generated primary keys and relation/index impact keys, while captured mutation values detect later assignment or in-place array drift before provider work. `TX-2A` is green for confirmed-success DataLinq-owned transactions: provider commit is followed by global publication, transaction-cache cleanup, explicit touched-mutable promotion, ownership-token commit, registry clearing, and only then the deferred wrapper `Committed` event. Transaction-bound immutable, foreign-key, and relation fallbacks are gated during the narrower provider-committed/local-finalization window. Bounded `TX-2B` is green only after `DatabaseAccess.Commit()` returns successfully and committed publication or transaction-cache cleanup then fails. That path raises `TransactionCommitFinalizationException` with the original failure and any cleanup failures, records `CommittedStateFinalizationFailed` on the ownership token and touched mutables, suppresses rollback and the wrapper `Committed` event, removes transaction-local state best-effort, and structurally clears provider-wide committed rows and indices before best-effort recovery notifications. Bounded `TX-3` is green for managed-wrapper rollback and open disposal: provider completion is attempted first; the ownership token and touched registry are terminalized with `RolledBack`, `RollbackOutcomeUnknown`, or `OpenTransactionDisposed`; exact transaction rows/subscriptions are discarded without clearing committed state; wrapper `RolledBack` observation is deferred until finalization; and a failed rollback that remains open permits only disposal. Bounded `TX-4A` poisons the managed wrapper after post-preflight mutation-pipeline failure, invalidates the current and previously touched lifecycle mutables, preserves the original exception, and gates managed reads, writes, callbacks, and commit until rollback or disposal. The adjacent managed commit-call fence preserves a throwing provider exception, records permanent `CommitOutcomeUnknown`, blocks managed reads/writes/callbacks/repeated commit/fallback, and permits only nonterminal rollback or scoped disposal without replacing uncertainty. Mutation/cache-callback managed reentrancy is rejected and completion entry points are serialized. A later definite terminal state supplies the terminal diagnostic rather than a stale mutation-poison diagnostic. This is not the `ML-1` or W3 exit. Database-outcome determination and provider-wide committed-cache repair after a throwing commit, raw low-level escape closure, attached/external completion beyond the narrow rollback fence, arbitrary local-cleanup fault injection, full provider commit-fault evidence, full concurrency semantics, and the `F6` live-read gate remain open.
...
- [x] Add the bounded managed `TX-4` provider-call safety fence: preserve a throwing provider exception, install permanent `CommitOutcomeUnknown`, reject managed reuse/fallback, permit only status-compatible rollback or scoped disposal, and keep low-level handles explicitly outside the claim.
- [ ] Complete the remaining `TX-4` database-outcome determination and provider-wide committed-cache recovery partition after a throwing commit.
- [ ] Complete attached-transaction lifecycle handling beyond the narrow external-closure rollback fence and define full concurrency semantics.
