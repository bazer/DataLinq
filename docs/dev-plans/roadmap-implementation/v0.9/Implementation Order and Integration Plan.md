...
For converter-backed metadata, the new admission is specifically canonical `Int32`. Other converted integral provider types stay on the legacy relation path; non-converted integral F6-B behavior is unchanged.
...
- [ ] Keep string/CHAR, `Guid`/UUID, composite, converter-backed integral types beyond `Int32`, raw/noncanonical model-wrapper operands at the neutral dispatch boundary, custom sources, key-only/preload neutralization, relation-wrapper redesign, memory relations, raw/manual readers, and all broader `F6` read/legacy families gated on their own normalization, transaction/cache characterization, and remaining W3 evidence.
...
- [ ] Complete SC-3 for converted joined-reader keys beyond exact scalar `Int32`, composite reader keys, and external relation/index lookup normalization without losing operand provenance.
