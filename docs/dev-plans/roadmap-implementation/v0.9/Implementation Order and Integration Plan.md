...
Known-value vectors, bounded `UUID-1A` declaration/codec primitives, and `UUID-1B` immutable provider-keyed resolution are green after `SC-1`. A bounded SQLite-first `UUID-2` checkpoint now consumes those definitions for direct non-key full-row `Guid` reads and mutation writes across Text36, Text32, little-endian BLOB, and RFC-order BLOB storage, plus nullable Text36/SQL NULL. Aggregate `UUID-2` remains open for MySQL/MariaDB, typed IDs, projections, defaults, and other reader routes. Wave 7 remains the integration/completion checkpoint after the scalar key/query/schema prerequisites are ready. Execute and close the UUID plan in its granular dependency order:
...
3. `UUID-2`: provider reads, writes, mutations, and generated/default hydration. In progress: direct non-key SQLite full-row `Guid` reads plus insert/update writes are green for Text36, Text32, little-endian BLOB, and RFC-order BLOB, with nullable Text36/SQL NULL evidence.
...
- [ ] Complete SC-2 for indexed defaults, rows with unknown non-auto keys, collation/codec-sensitive primary keys, MySQL/MariaDB UUID provider integration and SQLite typed-ID/projection/key/default UUID routes, member-init row evidence, joined/function-derived group keys, aggregate capability metadata and supported converter-backed aggregate execution, general `HAVING`, joined-local key handoff beyond primitive integral keys and neutral local-recipe hydration beyond integral SQL keys, and foreign-key/relation/index routing; bounded integral-canonical primary-key/cache-cold loading, converter-backed non-indexed default hydration, SQL `ScalarMember` conversion, aliased anonymous and explicit-constructor `SqlRow` conversion, direct model-compatible `GroupedAggregate` key conversion, single-source integral-keyed `ComputedRowLocal` `NewArray` recipe execution under `AotStrict`, primitive-key `JoinedRowLocal` `NewArray` SQL execution over converter-backed joined-source values, direct non-key SQLite full-row `Guid` mutation and materialization across Text36/Text32/little-endian-BLOB/RFC-BLOB plus nullable Text36/SQL NULL, and the converted-aggregate semantics guard are green.
...
Current integrated evidence after the bounded SQLite UUID provider-consumption slice is `1051/1051` unit and `760/760` SQLite compliance, with clean `DataLinq.SQLite` builds on `net8.0`, `net9.0`, and `net10.0`. The generator and server surfaces were unchanged and not rerun; their latest evidence remains `57/57` generator, `426/426` on each of MySQL 8.4 and MariaDB 10.11/11.4/11.8 (`1704/1704` sequential server executions), and `160/160` in the latest MySQL/MariaDB provider-specific lane. This is implementation evidence, not the final W11 or W13 frozen-candidate rerun.
