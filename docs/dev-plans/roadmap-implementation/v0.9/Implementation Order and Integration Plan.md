...
4. `UUID-3`: equality, nullable equality, membership, keys, cache, relations, update/delete predicates, and column-aware explicit queries. In progress: bounded `UUID-3A` is green for the initial column-aware non-primary-key expression-query matrix, and fully gated `UUID-3B` is green for captured-null comparisons and null-aware nullable membership on SQLite, MySQL, and MariaDB.
