...
SQLite UUID provider-consumption progress on 2026-07-13: A bounded SQLite-first `UUID-2` checkpoint now requires exact resolved SQLite UUID storage metadata when mapped canonical `Guid` values cross the provider boundary. The writer encodes direct non-key `Guid` mutation values through `GuidCodec`, and the column-aware reader decodes raw TEXT/BLOB values through the same resolved format before full-row materialization. Coverage includes non-nullable `Guid` across inferred Text36, explicit Text32, little-endian BLOB, and RFC-order BLOB, plus nullable Text36/SQL NULL. A frozen non-symmetric vector is written through an owned DataLinq transaction and inspected as exact raw strings/bytes; a second frozen non-symmetric vector is then seeded as raw SQL and cache-cleared back into canonical model values before a DataLinq update restores the original physical representation. The focused SQLite matrix passes `2/2`, the full unit and SQLite compliance gates pass `1051/1051` and `760/760`, and `DataLinq.SQLite` builds cleanly for `net8.0`, `net9.0`, and `net10.0`. Typed IDs over canonical `Guid`, projection result decoding, UUID predicates/membership, UUID keys/cache/relations/delete-key paths, defaults, ambiguous BLOB metadata, other SQLite reader routes, memory, and Native AOT/browser publication remain outside this slice; aggregate `UUID-2` is not complete.

MySQL/MariaDB UUID provider-consumption progress on 2026-07-13: Provider-owned access and transaction readers carry exact MySQL versus MariaDB identity into direct non-primary-key, non-converter full-row `Guid` decoding, and the shared mutation writer takes the same identity from the selected provider factory. Native MariaDB UUID, Text36, Text32, little-endian `BINARY(16)`, and RFC-order `BINARY(16)` consume resolved definitions. Binary reads use raw bytes; text/native reads accept raw text or connector-canonical `Guid`. Frozen non-symmetric vectors prove exact writes, independently raw-seeded reads, normal and transaction access, update re-encoding, nullable Text36/SQL NULL, and provider-differentiated binary storage under no `GuidFormat`, `Char32`, `Binary16`, and `LittleEndianBinary16`. Full gates pass `1051/1051` unit, `760/760` SQLite compliance, `429/429` on each server target (`1716/1716` total), and `414/414` combined provider-specific executions; `DataLinq.MySql` builds cleanly for `net8.0`, `net9.0`, and `net10.0`. Typed IDs, UUID primary keys, projections, predicates/membership, relations, defaults, provider-less public reader/access/transaction construction, memory, and Native AOT/browser publication remain outside this slice; aggregate `UUID-2` is not complete.
...
- SQLite direct non-key full-row provider-consumption tests covering non-nullable `Guid` across inferred Text36, explicit Text32, little-endian BLOB, and RFC-order BLOB, plus nullable Text36/SQL NULL, owned-transaction insert, update, exact raw write vectors, independently raw-seeded reads, and cache-cleared materialization
- MySQL/MariaDB direct non-primary-key, non-converter full-row `Guid` provider-consumption tests covering native MariaDB UUID, Text36, Text32, little-endian and RFC-order `BINARY(16)`, nullable Text36/SQL NULL, exact provider identity, normal and transaction readers, frozen raw writes, independently raw-seeded reads, update re-encoding, and no/adversarial connector `GuidFormat` modes
