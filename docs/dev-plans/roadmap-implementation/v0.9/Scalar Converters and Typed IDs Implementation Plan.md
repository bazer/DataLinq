...
Default-only insert progress on 2026-07-12 omits an unassigned null auto-increment primary-key slot only when the key is identity-mapped, non-converted, integral, and therefore reload-safe. When every write slot is omitted, the provider owns the zero-column SQL shape: SQLite renders `DEFAULT VALUES`, while MySQL and MariaDB render `() VALUES ()`. The focused active-provider SQL-shape and end-to-end matrices each pass `4/4`; full unit and SQLite compliance runs pass `989/989` and `736/736`. An assigned null or explicit auto-increment value remains an explicit write. Providers that do not declare default-only syntax retain the legacy null-key write and empty-query `VALUES (NULL)` fallback. Converted auto-increment keys, indexed or converter-backed defaults, unknown non-auto keys, UUID physical codecs, live read routing, and the remaining query/materialization routes remain open; SC-2 is not complete.
