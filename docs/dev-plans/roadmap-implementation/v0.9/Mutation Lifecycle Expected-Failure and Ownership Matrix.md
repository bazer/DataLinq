...
**Status:** W1 before-state audit complete. The `ML-2` pre-execution exits are implemented as of 2026-07-11 on the bounded `ML-1` provenance substrate; the full `ML-1` exit and `TX-1` through `TX-5` terminal/failure work remain assigned to W3.
...
| **Green control / amber failure coverage** | `TX2-PC-01` | Successfully write `M` in explicit transaction `T`, commit `T` through DataLinq, then dirty and save `M` through the same provider. | Commit promotes `M` from transaction-local to clean committed state only after provider commit and committed-cache publication. Later same-provider save succeeds from that promoted baseline. | The bounded provenance token now remains unresolved through provider commit, global publication, and transaction-cache cleanup, then normalizes the mutable to committed only after all three succeed. `EmployeesMutableLifecycleTests.TransactionHydration_*` observes both sides and existing reuse coverage remains green. | `TX-2`, provider regression `TX-6` | The successful ordering control is green. Full `TX-2` remains amber until the touched registry promotes every successful mutable and publication/cleanup fault partitions invalidate rather than normalize. |
...
| **Green control / amber terminal coverage** | `TX2-DEL-01` | Pass an existing mutable `M` to `Delete()` in transaction `T`, observe it before commit, commit through DataLinq, then attempt `Insert`, `Update`, `Save`, and another `Delete` with `M`. | After the statement, represent a transaction-local delete owned by `T`; after successful commit, promote it to a `Deleted` terminal state owned by the provider. Every later mutation rejects before SQL with an actionable deleted-row diagnostic. | Mutable delete now establishes transaction-local deleted provenance and the successful commit token normalizes it to provider-owned committed deletion; `ML-2` rejects every later mutation before command construction. The focused hydration/delete lifecycle test observes both states. | `TX-1` delete state, `TX-2` promotion, `ML-2` guard; provider matrix `TX-6` | Successful delete/guard behavior is green. Rollback, open-disposal, mutation-failure, and uncertain-commit invalidation remain amber/red until `TX-2` through `TX-4` consume the `TX-1A` entry and transition the touched mutable explicitly. |
...
Since W1, the bounded provenance substrate and `ML-2` implementation have made the explicitly marked pre-execution rows green with deterministic command-count and provider-matrix evidence. `TX-1A` now supplies the reference-identity registry for successfully transitioned lifecycle mutables. The remaining TX rows stay amber/red until `TX-1B` records only finalized changes and `TX-2` through `TX-5` consume that registry for terminal-transition, failure, and attached-completion behavior; successful guard work does not launder those later gaps.
