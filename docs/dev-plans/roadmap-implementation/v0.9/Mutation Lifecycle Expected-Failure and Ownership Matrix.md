...
Since W1, the bounded provenance substrate and `ML-2` implementation have made the explicitly marked pre-execution rows green with deterministic command-count and provider-matrix evidence. The remaining TX rows stay amber/red until their touched-registry, terminal-transition, failure, and attached-completion behavior is executable; successful guard work does not launder those later gaps.
