...
**Status:** Implementation in progress. F0-F2 and `F5` are complete; F3, F4, and F6 are active in bounded slices. The neutral row/materializer, primary-key loader, scalar-conversion, and UUID storage work documented below remains deliberately narrower than aggregate completion. Bounded `F4-B/F5-A/F5-B/F5-C/F5-D` adds one production request gate that selects the source-owned SQL backend and validates exactly once before work, plus complete SQL adapters for every retained expression-query result family with explicit result, cursor, cancellation, command, and reader ownership. Memory capabilities/backend, cache/relation migration, and legacy read-route removal remain open. F4, F6, UUID-2, and UUID-3 are not complete; W5 remains open.
