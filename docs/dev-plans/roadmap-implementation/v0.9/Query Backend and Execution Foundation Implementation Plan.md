...
**Status:** Implementation in progress. F0-F2, `F5`, the bounded `F7` vertical proof, D5 package promotion, aggregate M0, W10 steps 1-2 / RE-1D package-tool integration, W10 step 3 / RE-1A Testing CLI registration, W10 step 4 / RE-1C compatibility reporting, RE-1E aligned package-consumer evidence, the current-development RE-1F public-API checkpoint, the current-development W10 step-6 / RE-1G query and provider-free Memory heavy checkpoints, the RE-1H-A Testing CLI, RE-1H-B package-report, RE-1H-C benchmark history/comparison, and RE-1H-D compatibility size-report manifest-output implementation checkpoints, bounded M1-A exact non-null inequality, bounded M1-B exact Boolean composition, bounded M1-C exact non-null `Int32` relational comparison, bounded M1-D exact local `Int32` membership, bounded M1-E exact ordered final `Skip`, bounded M1-F exact `Single`/`SingleOrDefault`, bounded M1-G exact ordered `Skip`/`Take` window, and bounded M1-H exact ordered `First`/`FirstOrDefault` are complete; both the source-project and aligned-preview package-backed W10 step-5 eight-target matrices are green, while F3, F4, and F6 remain active in bounded slices. The neutral row/materializer, primary-key loader, bounded index-row loader, neutral test-source composition, scalar-conversion, UUID storage, and broader Memory query work remain deliberately narrower than aggregate completion. The current Memory backend exposes model-valued exact single-column `Find<TModel>(object)` and an exhaustive 57-token LINQ profile with nested `And`/`Or`/`Not` composition over exact direct non-nullable converter-free model/provider `Int32` comparison and local-membership leaves plus exact canonical-`Guid` `Equal`/`NotEqual` leaves, together with one exact ordered final-`Skip` sequence shape, one exact ordered final `Skip`/`Take` window, exact unpaged `Single`/`SingleOrDefault`, and exact ordered unpaged `First`/`FirstOrDefault` over root entities and direct-`Int32` scalars. Aggregate M0 freezes the complete metadata-only public neutral-source contract, the absence of SQL and provider-style post-seed CRUD/commit services from `MemoryDatabase<TDatabase>` and the neutral-source route, and DataLinq-owned rejection of the legacy inherited provider member plus parameterless `Delete()` extension without additional backend work. Broader filter/order/paging and membership semantics outside those exact fences, other projection forms, bare or unordered `First`/`FirstOrDefault`, broader ordered-first projections, `Last`/`LastOrDefault`, terminals after paging, wider parity, remaining typed-ID/`Guid` query behavior, composite lookup, and aggregate M1-M2 remain open. Aggregate RE-1H, manifest consumption, W10 steps 7-9, RE-5 final benchmark execution/disposition, aggregate RE-1/RE-4/W10/W11, final release-candidate closeout, and publication remain open. Remaining cache/relation migration and legacy read-route removal also remain open. F4, F6, UUID-2, and UUID-3 are not complete; W5 and the remaining W9 work remain open.

**F4-B/F5 capability and execution progress:** The current contract contains 613 contextual features, with 355 supported and 258 unsupported SQL dispositions. Fifteen post-F4-A refinements distinguish exact direct-`Int32` equality, exact non-nullable canonical-`Guid` column/scalar equality, one exact single-column-primary-key ordering, safe and unsafe same-scope paging composition including exact ordered final `Skip`, an exact nonnegative `Int32` scalar-binding count, an exact direct non-nullable converter-free `Int32` root scalar projection, and exact direct non-nullable converter-free `Int32` column/local-sequence membership from their broader alternatives. Twelve are SQL-supported, including both scalar-projection shapes, the canonical-`Guid` comparison shape, both descriptive membership-shape tokens, and the exact ordered final-`Skip` shape; repeated `Take`, `Take` before `Skip`, and repeated `Skip` deliberately reject because the retained SQL renderer stores only one limit and one offset per scope. Exact `Int32` shape classification now respects model-property as well as provider nullability. Explicit ordering conversions remain represented and reject as `Value:Converted@Ordering` until cast-aware rendering exists, avoiding silent narrowing misordering. Shared template validation now requires source/table coherence, projection-returning result/projection coherence, and canonical `bool`/`Int32` result types for `Any`/`Count`; non-aggregate results cannot carry aggregate selectors. SQL local entity reprojection preserves terminal kind while substituting the source entity type. These checks give the memory backend an exhaustive narrow fence; the initial F4-A 598/343/255 matrix remains documented below as historical checkpoint evidence. Every production expression query creates one `QueryExecutionRequest`, selects the source-owned backend, computes structural and invocation requirements, and validates exactly once before backend work. The SQL backend owns entity sequences and terminals, all six scalar reductions, direct `ScalarMember`/`SqlRow`/`GroupedAggregate` projections, and retained `Anonymous`/`ComputedRowLocal`/`JoinedRowLocal` recipes for their parser-supported result shapes. Projection execution returns final semantic `TResult` values through an owned cursor; SQL readers, aliases, `DBNull`, provider widths, converter-aware decoding, reflective row construction, and local recipe evaluation do not cross the backend contract. `SqlLocalProjectionExecutor` owns local SQL loading, joined-key buffering/cache hydration, result conversion, and bounded cancellation checks. `ExpressionQueryPlanExecutor` no longer constructs `QueryPlanSqlBuilder` for a supported projection. Contract tests freeze exactly-once backend dispatch for all three local kinds and terminals, `AotStrict` execution for `AotSafe` computed recipes, and pre-dispatch rejection for SQL-only anonymous and joined-local recipes. The integrated `F5-D` gate passes `57/57` generator tests, `1117/1117` unit tests, `788/788` SQLite compliance tests, `796/796` in each paired server run, and provider-specific batches of `160/160` and `162/162`. `F5` is complete. F4 remains open for the memory release profile/backend, while F6 retains neutral key/cache/relation and legacy read-route work; W5 is therefore not complete.

**F6-B index-row progress:** `SourceIndexRowRequest` and `SourceIndexRowLoadResult` add an optional, mutation-free full-row request/result pair for one frozen table index and canonical provider key. `IDataLinqIndexRowServices` is independent of the primary-key loader capability; existing SQL `DataSourceAccess` instances expose both through the same cached `DataSourceAccessSourceRowLoader`. The SQL adapter validates source ownership and read state, converts every index component through the provider writer, owns command/reader lifetime, observes cancellation at bounded points, and returns fully buffered canonical rows. The live `TableCache` dispatch is deliberately narrower than the contract: it admits an exact key for one supported canonical-provider index column. Integral columns retain their existing rules; a `Guid` column additionally requires a concrete SQLite, MySQL, or MariaDB source and resolved active-provider `GuidStorage`. Direct and converter-backed Guid metadata are eligible only after the caller supplies the exact canonical `Guid`; this boundary performs no model normalization or conversion, so model wrappers, raw bytes/text, missing or unresolved mappings, and composite indexes remain ineligible. Cold loads retain row-cache miss/store/materialization accounting; read-only loads publish the shared relation index so the warm path returns the same immutable instances without another command. A reproduced rollback-poisoning bug showed that a transaction-visible relation key set must never enter that shared committed index, so both full-row relation loading and the still-SQL-shaped key-only path now publish shared index entries only from `ReadOnlyAccess`. The original focused evidence is `6/6` contract tests, `11/11` adapter/route tests, `2/2` provider-key helper tests, and one SQLite-memory execution each for rollback isolation, cold/warm identity and telemetry, and the existing cold relation traversal. Later exact typed-ID checkpoints cover canonical `Int32`, `Int64`, and resolved canonical `Guid`; the Guid gate is format-agnostic after resolution, while provider relation execution is deliberately one converter-backed binary mapping across SQLite, MySQL, and MariaDB. Current integrated gates pass `60/60` generator, `1214/1214` unit, `811/811` SQLite file/memory, `819/819` in each paired server batch (`1638/1638` total), and the unchanged provider-specific baseline remains `380/380`. String/CHAR, composite and unresolved UUID shapes, raw/noncanonical wrapper operands, direct-`Guid` relation execution, text/native UUID relation evidence, custom/external/provider-less sources, key-only `GetKeys` neutralization, index preload, broader relation-wrapper work, memory relations, raw/manual reader routes, and broader legacy removal remain outside F6-B. F6 and W5 remain open.

The converter-backed integral admissions above are specifically canonical `Int32` and canonical `Int64`. Other converted integral provider types remain on the legacy relation path until their own end-to-end evidence lands; non-converted integral F6-B behavior is unchanged. The separate canonical-`Guid` admission is a resolved active-provider codec gate, not an integral-policy widening.
...
**Historical F7/W8 step-8 memory-spike checkpoint:** Separate non-packable `DataLinq.Memory` and non-packable TUnit `DataLinq.Tests.Memory` projects are in the solution. Each memory source owns immutable per-table `CanonicalProviderValueRow` state plus a `DataLinqKey`-to-row-ordinal primary-key index, while materialized identities remain in one existing `RowCache` per table. The internal canonical seed path publishes a table atomically, rejects duplicate keys with row context, and does not freeze a public seed API. Direct primary-key requests use `SourcePrimaryKeyRowRequest`, generated materialization, and the shared source-bound cache; a non-concurrent test eviction proves clearing that cache does not remove store rows. SQL and memory startup share one registry lock across metadata resolution, publication, and generated static binding; a gated unit test proves a concurrent resolver cannot return during the first binder, while a 32-way cold start proves every source converges on the one winning graph. The source-bound backend remains at exactly 31 supported tokens. Repeated direct `Where` predicates are restricted to exact non-nullable `Int32` root-column equality with an exact non-null `Int32` scalar binding; one ascending or descending direct non-nullable converter-free `Int32` single-column-primary-key ordering may be followed by one final exact nonnegative `Int32` scalar-binding `Take`. A final `ScalarMember` sequence may select a primary-key or non-key root column only when its result, model, and provider types are exact non-nullable converter-free `Int32`; an exhaustive scalar-projection shape rejects string, nullable, widened, boxed, converter-backed, typed-ID, `Guid`, and non-root alternatives before row access. Selectorless `Any` and `Count` reduce entity or admitted scalar-projection rows through the shared canonical-row cursor; predicate overloads stay within the exact equality shape. Empty input returns `false`/`0`, unordered `Any` short-circuits, `Count` exhausts with checked `Int32` arithmetic, and ordered reductions inherit full-match buffering. They materialize neither entities nor projected cells and perform zero cache work. Terminal paging remains unsupported because it is represented as `Pushdown`. Natural self-join, grouped-aggregate, and captured row-local projection plans freeze deterministic first capability failures (`SourceCount:Multiple` at `sources`, `Operation:GroupBy` at `operations[0]`, and `Projection:ComputedRowLocalExpression` at `projection`) before store/cache work; the captured value is redacted. Entity sequences still materialize through `RowCache`; scalar sequences still apply shared scalar materialization/result adaptation to selected canonical cells. Bounded step-8 comparison uses one generated model and five adversarial exact-`Int32` rows in both sources. Every paired query is parsed once into a `QueryPlanInvocation`, then the same invocation is executed through memory and SQLite. Hard-coded observations cover every representative current 31-token dimension across root/repeated-equality entities, both key orders, zero/bounded/over-cardinality `Take`, key/non-key scalar projections, composed and empty selections, and entity/projected `Any`/`Count`; unordered results are normalized only after execution, while ordered results are compared in returned order. One separate shared invocation makes post-`Take` terminal execution an explicit provider difference: SQLite succeeds, and memory rejects `Operation:Pushdown` without touching diagnostics. At that checkpoint focused memory and capability evidence passed `33/33` and `25/25`; integrated gates remained `1138/1138` unit, `57/57` generator, and `795/795` SQLite file/memory compliance tests. The test project intentionally carries SQLite/native assets for comparison; `DataLinq.Memory` remains non-packable, builds cleanly for net8/net9/net10 with zero warnings or errors, and its resolved runtime graphs remain free of SQL-provider and native-database packages. Internal cancellation evidence covers pre-execution and bounded entity, filtered, ordered, and projected yield points plus reduction pre-cancellation; generated public LINQ supplies `CancellationToken.None`. At step 8 other projection forms, the wider null/string/converter/typed-ID/`Guid` matrix, public seeding/API/package shape, concurrent cache maintenance, wider cross-backend parity, and constrained-runtime execution remained open. The later step-10 paragraph supersedes only the constrained-runtime, bounded `F7`, and ordered W8-step status; F4, M0, M1, and M2 remain incomplete.

The subsequent bounded W8 step-9 seed/read checkpoint keeps the 31-token query capability profile unchanged. Internal dense model rows now normalize through the shared `ModelValueConverter` before canonical-row/key publication. A Guid-backed typed-ID primary key, direct `Guid`, second non-null typed field, and nullable typed field prove canonical `Guid` storage, canonical lookup identity, null converter bypass, model-valued generated materialization, warm same-instance reuse, and rematerialization after explicit test cache eviction. Seed normalization calls `ToProvider` twice. Cold materialization adds two `FromProvider` calls plus one `ToProvider` for the existing generated immutable primary-key capture (cumulative three and two); warm lookup and enumeration add none; eviction and rematerialization add two `FromProvider` plus one key-capture `ToProvider` (cumulative four and four). A differential canonical-seed case sends canonical `Guid` cells through `SeedCanonical` with zero seed-time `ToProvider`; cold materialization invokes `FromProvider` for all three converted fields plus one `ToProvider` for immutable primary-key capture. Deliberately different SQLite, MySQL, and MariaDB `GuidStorage` declarations remain SQL physical metadata and never affect memory cells. Full model type/nullability preflight precedes a row's first converter, completed reseeds and concurrent same-table attempts reject before their converter execution, and invalid or duplicate rows leave the table unpublished; invalid-cell top-level diagnostics are value-redacted and retain row/column context. Converter code runs outside the seed monitor. At that checkpoint focused evidence passed `7/7`, the complete memory project passed `40/40`, and `DataLinq.Memory` built cleanly for net8/net9/net10. This does not widen query predicates, projections, membership, ordering, relations, or public API/package status; F4, M0, M1, and M2 remain incomplete. The following step-10 checkpoint supersedes only the constrained-runtime, bounded `F7`, and ordered W8-step status.

**Historical F7/W8 provider-free constrained-runtime checkpoint:** A separate non-packable `DataLinq.Memory.PlatformCompatibility.Smoke` generated-model runner fed non-packable Native AOT, full-trim, and Blazor WebAssembly hosts. It executed the then-current 31-token profile, not a runtime-specific capability variant: canonical/model-valued seed, primary-key hit/miss, captured equality, order plus `Take`, entity and direct scalar materialization, `Any`/`Count`, deterministic unsupported self-join rejection before work, pre-cancellation, and canonical Guid-backed key/direct-`Guid` storage. Native AOT and full-trim executables published and exited successfully. Isolated WebAssembly no-AOT and AOT publishes executed successfully in a real browser with no warning/error entries. All four published outputs scanned clean for SQL-provider and native-database payload names. A direct static-abstract generated-metadata binder method group caused a WebAssembly AOT signature mismatch; named `CreateMetadata` and `BindMetadata` wrappers preserved the same registry/binding semantics without that malformed generic delegate thunk. WebAssembly AOT/no-AOT mode switches required clean or isolated intermediate paths to avoid cross-mode stripped-IL contamination. The memory suite passed `42/42`. This completed bounded `F7` and W8 step 10, not F4's broader memory release profile, D5 public/package promotion, W9 capability completion, or W10 release integration.

**Historical SC-6A exact canonical-`Guid` equality checkpoint:** At that checkpoint, the exhaustive Memory profile contained 32 tokens and admitted only default-null-semantics equality between one direct non-null root column and one non-null scalar when the column was either identity-mapped `Guid` or a resolved converter-backed model type canonicalized to `Guid`. Both operand orders, hit/miss behavior, invocation rebinding, repeated mixed predicates, and entity `Any`/`Count` are proven. Typed bindings normalize exactly once per predicate through `ModelValueConverter`; direct `Guid` bindings invoke no converter, and row filtering compares canonical `Guid` values without consulting `GuidStorage`, parsing text, translating bytes, or invoking a provider codec. Converter failures cross the public memory boundary without retaining an arbitrary exception graph, while cancellation and fatal exceptions preserve exact identity. Nullable equality, inequality, local membership, ordering, scalar projection, typed-ID member unwrapping, relations, and public typed primary-key lookup remained unsupported before row work. One differential fixture parses each expression once and executes the same `QueryPlanInvocation` through Memory and independently raw-seeded SQLite, proving the bounded logical island across deliberately different Guid encodings without claiming general provider parity. The checkpoint gates passed `62/62` Memory, `1214/1214` Unit, and `60/60` Generator tests; the runtime built cleanly for net8, net9, and net10. Native AOT and full-trim executables passed, isolated real-browser WebAssembly no-AOT/AOT runs passed without warning/error entries, and all four output scans remained free of SQL-provider/native-database payloads. The public API was unchanged. At the SC-6A checkpoint, D5-B package promotion, the remaining F4/SC-6/W6 surface, and W10 integration remained open.

**D5-B package-boundary checkpoint:** `DataLinq.Memory` is packable as an experimental preview after aligned core/Memory candidates at `0.9.0-preview.d5b.5` pass direct nuspec, runtime/symbol archive, metadata/provenance, and banned-payload inspection plus an explicit zero-finding package report. Private MinVer aligns ordinary packing with core, SourceLink supplies repository provenance, and shared explicit overrides align the inspected candidate pair. Each net8/net9/net10 dependency group contains only a same-candidate `DataLinq` minimum; the runtime package has exactly three managed Memory assemblies and no analyzer/runtime/native/build/tool folder, while the symbol package has exactly three PDBs. At that checkpoint this closed D5 and W9 step 1 without widening the backend contract or public API; M0-M2, the remaining F4/SC-6/W6 surface, and all W10 release-tool and packaged compatibility integration remained open.

**M0-A public exact single-column lookup checkpoint:** `MemoryDatabase<TDatabase>.Find<TModel>(object)` now consumes the source-owned canonical primary-key and materialization seams directly. It accepts one non-null model-side value for exactly one generated key column, normalizes converter-backed keys through `ModelValueConverter`, probes the existing index without a scan, and returns a generated immutable model or `null`; unseeded misses, warm identity, model-valued `RowData`, and independent stores/read sources/identities are proven. Invalid model/canonical/numeric values, composite metadata, and ordinary failures during initial `ToProvider` normalization, canonical-to-model `FromProvider` materialization, or generated immutable primary-key `ToProvider` identity capture are value-redacted through public `MemoryLookupException`; literal null throws `ArgumentNullException`, fatal/cancellation exceptions at all three points preserve identity, and failed materialization or identity capture does not poison the cache. Focused lookup evidence passes `15/15`, the full Memory suite passes `77/77`, net8/net9/net10 builds have zero warnings and zero errors, Native AOT and full-trim executables pass, isolated WebAssembly no-AOT/AOT browser runs reach `passed`/`completed` with only expected logs, and banned-provider/native-payload scans remain clean. At that checkpoint this lookup was not a query-plan capability: the Memory profile remained exactly 32 tokens, no generated `Get(...)` overload whose source parameter was typed as `MemoryDatabase<TDatabase>` or `IDataLinqReadSource` alone was added, composite lookup remained unsupported, and aggregate M0 plus W10 remained open.

**W10 step-3 / RE-1A Testing CLI registration checkpoint:** `DataLinq.Tests.Memory` is now the non-target-batched `memory` suite and is included once in `all`. Direct built and explicit `--alias all` executions each pass `77/77` as one result with `Targets` `-`; the direct summary run leaves test-infrastructure state unchanged. The composite quick gate passes `2162/2162`, including memory `77` exactly once alongside generators `60`, unit `1214`, and SQLite compliance `811`. The project intentionally references SQLite for bounded differential parity, so this is project-based CLI registration rather than provider-free or package-consumer evidence. At that checkpoint W10 step 3 and RE-1A registration were complete; structural SQL-boundary and aggregate M0 closeout, compatibility/package integration, aggregate RE-1/W10, and release closeout remained open.

**Aggregate M0 structural-boundary checkpoint:** the complete public `IDataLinqReadSource` contract exposes only `Metadata` and inherits no operational interface; `MemoryDatabase<TDatabase>` remains limited to `Find`, `Query`, and `Seed`; and the Memory construction/query route supplies none of `IDataSourceAccess`, `IDatabaseProvider`, or `IDatabaseAccess` and exposes no provider-style post-seed CRUD/commit/transaction service. Primitive and converter-backed fixtures freeze the existing shared generated `Get(...)` source parameters to exactly `IDataSourceAccess`, `Database<TDatabase>`, or `Transaction<TDatabase>`, never `IDataLinqReadSource` alone or `MemoryDatabase<TDatabase>`. The canonical primitive fixture proves its row, root, and query provider expose no SQL access interface; consumer-authored partial members remain outside the Memory contract. The legacy inherited `GetDataSource()` member and parameterless `Delete()` extension reject without additional backend work, Memory diagnostics remain unchanged, and public lookup preserves stored identity. Focused public-boundary tests pass `14/14`, and the full targetless Memory suite passes `78/78`. Raw SQL is therefore structurally unavailable rather than represented by a fake throwing backend method; aggregate M0 is complete without changing production assemblies or the 32-token capability profile.
...
**RE-1H-D compatibility size-report tooling checkpoint:** Newly generated compatibility reports use schema `v0.9.compatibility-size-report.v6`. They retain resolved invocation/target and package provenance, timing, outcome and invocation completeness, guarded referenced-artifact paths with hashes, review state, candidate stability, and runner/candidate identity. `--output` is guarded below repository `artifacts`, rejects overlap with package input or mutable build state, and invalidates only a previous report pair; Markdown is promoted before JSON. `--release-evidence` fails unless the report itself is intrinsically valid: the exact ordered eight-target package-backed `v0.9` catalog, Release/default-RID invocation, restore/smoke/clean-output/threshold/failure policies, explicit output, complete stable candidate/artifacts, clean commit-aligned runners/candidate, and successful complete target/browser/provenance results are all required. Expected third-party `WASM0001` diagnostics remain `ReviewRequired` and need release disposition. This closes only the report-writer checkpoint; it does not relabel the dated v5 aligned-preview artifact, execute the final-RC constrained-runtime gate, consume a manifest, close aggregate RE-1H/RE-4/W10, or publish packages.
...
| [`SourceIndexRowRequest`](../../../../src/DataLinq/Instances/SourceRowLoading.cs) snapshots one frozen table index, canonical provider key, and cancellation token. The optional `ISourceIndexRowLoader` returns an owned finite canonical-row result and leaves matching equality to the backend. Existing SQL sources share one cached `DataSourceAccessSourceRowLoader` across their primary-key and index capabilities. | Bounded `F6-B` routes exact single-column integral canonical index keys and exact single-column canonical `Guid` keys with resolved active-provider SQLite/MySQL/MariaDB storage from `TableCache` through the SQL adapter. A converter-backed column is eligible only when upstream relation-key construction has already produced the exact canonical value; `TableCache` rejects raw model wrappers and performs no conversion. Read-only cold loads preserve materialization, telemetry, shared-index warming, and immutable identity; transaction loads retain scoped visibility but cannot publish their visible key set into the shared committed index. Guid provider evidence is one converter-backed binary mapping, so direct-`Guid` relation execution, text/native relation evidence, composite or unresolved UUID shapes, and every broader relation/index/key or legacy route remain explicit follow-up work. |
...
The later joined-key and exact resolved F6-B relation/index checkpoints supersede only the corresponding exclusions in that historical UUID-3C inventory. They do not add composite UUID keys, general scalar/projection decoding, manual/provider-less routes, memory support, text/native relation evidence, or aggregate UUID completion.
...
The F6-A exclusions above describe that checkpoint. The original bounded `F6-B` checkpoint on 2026-07-13 superseded only its blanket foreign-key/relation/index exclusion for one exact live integral family: it introduced `SourceIndexRowRequest`, `SourceIndexRowLoadResult`, and the optional `ISourceIndexRowLoader`/`IDataLinqIndexRowServices` capability without adding mutation responsibility or coupling it to primary-key loading. Existing SQL sources intentionally expose both capabilities through one cached `DataSourceAccessSourceRowLoader`. The adapter validates that the source owns the request metadata and may still read, converts each canonical index component through the selected provider writer, checks cancellation before provider work and at bounded reader points, owns/disposes command and reader state, decodes full canonical rows, and returns no live provider cursor. `TableCache` originally selected this route only when the relation/index had exactly one integral canonical-provider column and the caller supplied that exact canonical CLR type. Cold rows keep the established hit/miss, database-row, materialization, and store accounting; a read-only cold load may populate the shared index, and the warm load reuses the same immutable instances without another command.

Bounded SC-3 progress on 2026-07-14 composes that route with one `Int32`-backed typed-ID PK/FK model. The generator suppresses provider-typed collection/reference fast paths when the relation-side column has a scalar converter; the existing dynamic relation path therefore builds a canonical `DataLinqKey` through `KeyFactory` before cache dispatch. F6-B then admits the exact integer key despite converter-backed index metadata, without invoking either converter direction. A raw `QueryTypedId` supplied directly at the eligibility gate still falls back. Active-provider evidence proves cold collection loading, read-only index warming, warm immutable identity, singular parent reuse, canonical cache keys, and model-valued public IDs. The unchanged primitive cold/warm route and rollback-isolation characterization also pass across all active providers.

A second bounded SC-3 checkpoint closes one live joined-reader gap without generalizing the reader contract. `SqlLocalProjectionExecutor` still buffers every joined source key before cache hydration, but for a scalar converter-backed primary key whose canonical provider CLR type is exactly `Int32`, it now decodes the selected alias ordinal through `ProviderRowDecoder` and carries the result as a dynamic `DataLinqKey`. This prevents a public typed-ID wrapper or a bare provider integer from being mistaken for a generated scalar fast-path key. Active-provider self-join evidence passes `4/4` and proves both sources hydrate after cache clearing, repeated parent rows preserve immutable identity, warm dynamic-key lookups return the same instances, and the row-local recipe remains model-valued and `SqlOnlyCompatibility`. The unchanged primitive joined-local route passes `4/4`; full generator, unit, and SQLite file/memory gates pass `58/58`, `1138/1138`, and `799/799`. Converted `Int64`, UUID/`Guid`, string/CHAR, composite, external, raw/manual, and memory joined-reader keys remain unchanged.

The next bounded SC-3 checkpoint extends only those exact finalized integral seams to canonical `Int64`. `TableCache` now admits an already-canonical long for the same single-column `F6-B` relation/index shape, while raw model wrappers still fall back and the eligibility gate invokes no converter. `SqlLocalProjectionExecutor` decodes an exact converter-backed long primary key at its actual selected alias ordinal before creating the dynamic `DataLinqKey`. A dedicated typed-ID PK/FK fixture uses `5_000_000_101` through `6_000_000_203`, above `Int32.MaxValue`, and proves cold collection loading, read-only index warming, warm immutable identity, singular parent reuse, both joined sources, repeated-parent identity, canonical long cache keys, and model-valued public IDs. Focused evidence is `2/2` for the reader seam, including the `Int16` legacy-path exclusion, and `11/11` for the loader class; the relation/joined fixture passes `4/4` on SQLite file/memory plus `8/8` across the four server targets. The adjacent SC-5 evidence requires SQLite `INTEGER` or signed MySQL/MariaDB `BIGINT` and passes `19/19` in the scalar schema class plus `8/8` live server cases. Current integrated gates pass `60/60` generator, `1211/1211` unit, `807/807` SQLite file/memory, `815/815` in each paired server batch (`1630/1630` total), and `189/189` plus `191/191` provider-specific executions (`380/380` total). At that checkpoint, other integral CLR types, string/CHAR, UUID/`Guid`, composite, external, key-only/preload, raw/manual, custom-source, provider-less, and memory relation/join paths remained open. The following exact Guid-backed checkpoints supersede only the resolved joined-key and relation/index exclusions.

The subsequent joined Guid-backed typed-ID checkpoint extends only the `JoinedRowLocal` reader-key seam, not `F6-B` relation/index admission. The runtime gate accepts the representative explicit-inner shape whenever the concrete SQLite, MySQL, or MariaDB source resolves active-provider `GuidStorage`; it is format-agnostic. This checkpoint's provider evidence and support claim use one representative binary mapping. `SqlLocalProjectionExecutor` decodes the selected alias ordinal through the resolved column-aware provider reader to canonical `Guid`, constructs the dynamic `DataLinqKey`, and hydrates both joined sources through the existing cache. Non-symmetric raw bytes, repeated-parent identity, canonical cache keys, model-valued public results, and warm same-instance reuse prove the boundary without `byte[]` leakage. The focused reader class passes `5/5` and invokes neither converter direction. Cold end-to-end immutable construction legitimately records three `ToProvider` and five `FromProvider` calls; warm execution adds none. Focused compliance passes `2/2` on SQLite file/memory and `4/4` across the four server targets. At that checkpoint, joined-key evidence for text/native UUID storage, other typed mappings/formats, composites, outer or missing-source joins, UUID relation/index/foreign-key paths, external/key-only/preload/manual/provider-less readers, source-only resolution, converter-backed defaults, memory/AOT, and aggregate F6/W6/UUID completion remained open. The following F6-B checkpoint supersedes only the exact resolved relation/index exclusion.

The exact resolved canonical-`Guid` F6-B checkpoint admits one single-column relation/index key only when the source is concrete SQLite, MySQL, or MariaDB and the indexed column has resolved active-provider `GuidStorage`. Direct and converter-backed metadata pass the unit gate only with an already-canonical `Guid`; `TableCache` invokes neither converter direction and rejects model wrappers, raw `byte[]` or text, missing, unresolved, or unknown-provider mappings, and composite indexes. The runtime gate is format-agnostic after resolution, but current provider execution and support evidence is one converter-backed binary PK/FK mapping: SQLite file/memory and MariaDB use RFC-order bytes, while MySQL uses little-endian bytes. Non-symmetric raw PK/FK seeds, transaction-local delete/load/rollback, an empty committed shared index after rollback, one cold reader execution, read-only index warming, warm immutable identity, reverse-reference reuse, telemetry, and exact converter-call ownership prove the bounded path. Integrated gates pass `60/60` generator, `1214/1214` unit, `811/811` SQLite file/memory, `819/819` in each paired server batch (`1638/1638` total), and the unchanged provider-specific baseline remains `380/380`.

A bounded SC-4 compatibility checkpoint now rejects known-incompatible SQL join-key mappings before SQL state or commands exist. `QueryPlanSqlBuilder` recursively validates direct and pushed-down explicit and implicit joins for the concrete provider. It leaves converter-free non-UUID joins untouched; converted keys require converters on both sides plus the same resolved underlying model type, canonical provider type, and nominal converter CLR type. Canonical `Guid` keys additionally require exact resolved active-provider storage formats, comparing `GuidStorageFormat` but not the provenance-only `IsExplicit` bit. The validator invokes no converter. Unit evidence passes `10/10`; active-provider mismatched-converter and mismatched-UUID-format fixtures pass `8/8` with zero SQL commands and zero conversion calls, while `36/36` selected existing explicit/implicit, primitive/typed-ID, and pushdown executions remain green. Integrated generator, unit, and SQLite file/memory gates pass `58/58`, `1148/1148`, and `803/803`. This rule is intentionally nominal: converter CLR type is not semantic proof because `ScalarConversionContext` exposes the column. General mapping-equivalence metadata, mixed identity/converter compatibility, non-UUID physical schema compatibility, composite keys, memory joins, and typed-UUID joined-reader hydration beyond the exact representative resolved-storage slice remain open.
...
F6-B now admits exact single-column integral canonical keys and exact single-column canonical `Guid` keys with resolved active-provider SQLite/MySQL/MariaDB storage. It still does not admit string/CHAR, composite indices, converter-backed integral relation types beyond exact `Int32` and `Int64`, unresolved or missing UUID storage, raw/noncanonical model-wrapper operands presented directly at neutral dispatch, custom/external/provider-less sources, key-only `GetKeys` or index-preload neutralization, broader relation-wrapper redesign, memory relations, raw/manual reader paths, or broader legacy-route removal. Guid relation provider evidence remains one converter-backed binary mapping; direct-`Guid` relation execution and text/native UUID relation evidence are open. The original neutral-source contract-composition proof still does not exercise `TableCache`, generated static `Get(...)`, a memory or query backend, transactions, or production telemetry; the later exact SQL relation checkpoints do exercise `TableCache`, transaction rollback isolation, telemetry, and production collection/reference traversal without broadening that neutral-source proof. F6 and W5 remain open.
...
- prove converted lazy collection/reference relations normalize upstream and enter the exact integral index route without double conversion
...
- one primitive deterministic seed held as canonical provider-value buffers
- one bounded internal dense model-valued seed fixture with a Guid-backed typed-ID key and non-key field
- one direct `Guid` field plus one nullable typed-ID field, all stored as canonical `Guid` or null without applying SQL UUID codecs
...
The bounded primitive fixture now proves every required path in this list, including both `Any` and `Count`, same-token cancellation before work and at bounded yield points, and representative join, grouping, and computed-projection capability failures. The focused comparison below is also proven for the exact admitted primitive island. This closes only the bounded parity profile; wider parity remains W9 work.

At the bounded W8 seed/read checkpoint, the converted fixture proved only model-valued seed normalization, canonical Guid-backed key lookup, root entity materialization, and cache identity. It did not yet add typed-ID or `Guid` predicate, projection, membership, ordering, or relation capabilities; the later SC-6A checkpoint above supersedes only the exact equality part of that exclusion.
...
Bounded W8 step-8 evidence parsed each representative query shape exactly once into a single `QueryPlanInvocation` and executed that same invocation against both memory and SQLite. Both sources received the same five-row logical seed: `Int32.MinValue`, `-11`, `0`, `17`, and `Int32.MaxValue` keys, repeated exact-`Int32` group values, and distinct names. Hard-coded expected values covered the then-current 31-token profile's representative root/equality, ordering, `Take`, direct scalar projection, empty-result, `Any`, and `Count` dimensions. Unordered entity results were sorted only after both executions; ordered entity and scalar sequences retained provider-returned order. The deliberate difference was not washed away: a post-`Take` `Any` invocation succeeded on SQLite and rejected as `Operation:Pushdown` on memory before row access. The `DataLinq.Tests.Memory` parity host deliberately references SQLite and therefore carries its native test assets; the `DataLinq.Memory` runtime dependency graph remains SQL-provider- and native-database-free. This is focused regression evidence, not a claim of general SQL equivalence.
...
Bounded W8 step-10 evidence used one generated provider-free runner from Native AOT, full-trim, WebAssembly no-AOT, and WebAssembly AOT hosts. Each mode executed the exact then-current profile and reported 31 capabilities. Both executable hosts exited successfully, both browser modes reached the completed state with no warning/error entries, and recursive scans of all four outputs found no SQL-provider or native-database payload. This proved the vertical architecture in constrained runtimes; it was not a package, compatibility-catalog, size-threshold, or release-candidate claim. WebAssembly mode switches should use clean or isolated intermediate paths because sharing intermediates between AOT and no-AOT builds can retain stripped IL.
...
- **Bounded comparison complete:** the representative exact-`Int32` request matrix executes through both SQL and memory adapters
- **Bounded constrained-runtime proof complete:** the same provider-free memory route executes under Native AOT, full trimming, WebAssembly no-AOT, and WebAssembly AOT
- the architecture is ready for the separate read-only memory preview workstream; D5 public construction/seed review still controls package promotion
