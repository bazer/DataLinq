...
**F7/W8 first memory-spike checkpoint:** Separate non-packable `DataLinq.Memory` and non-packable TUnit `DataLinq.Tests.Memory` projects are now in the solution. Each memory source owns immutable per-table `CanonicalProviderValueRow` state plus a `DataLinqKey`-to-row-ordinal primary-key index, while materialized identities remain in one existing `RowCache` per table. The internal canonical seed path publishes a table atomically, rejects duplicate keys with row context, and does not freeze a public seed API. Direct primary-key requests use `SourcePrimaryKeyRowRequest`, generated materialization, and the shared source-bound cache; a non-concurrent test eviction proves clearing that cache does not remove store rows. SQL and memory startup now share one registry lock across metadata resolution, publication, and generated static binding; a gated unit test proves a concurrent resolver cannot return during the first binder, while a 32-way cold start proves every source converges on the one winning graph. A source-bound memory backend declares exactly eight supported baseline tokens and executes one real pass-through root entity sequence. Every other catalog feature remains unsupported; a `Where` plan fails capability validation before a scan or materialization. Internal request/cursor evidence covers pre-cancellation and deterministic cancellation between rows; generated public LINQ still supplies `CancellationToken.None`, so no public cancellable-query claim is made. Focused evidence is `6/6`; the integrated unit and generator gates pass `1129/1129` and `57/57`. The memory project builds cleanly for net8/net9/net10 with trim/AOT analyzers enabled, and its resolved package closure contains no SQLite, MySQL, connector, or native dependency. Captured equality, `Take`, ordering, projections, scalar results, terminals, joins/grouping diagnostics, SQLite parity, typed IDs, canonical `Guid`, public seeding/API/package shape, concurrent cache maintenance, raw-SQL and relation diagnostics, Native AOT, trimming publication, and browser/WebAssembly remain open. F4, F7, and W8 are not complete.
...
Loading that generated model from the actual memory backend remains an F7/W8 vertical-spike proof. It is deliberately not a circular prerequisite for completing the neutral F6 contracts and SQL migration.
