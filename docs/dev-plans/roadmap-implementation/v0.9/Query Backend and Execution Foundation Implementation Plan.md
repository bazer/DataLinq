...
**F7/W8 provider-free constrained-runtime checkpoint:** A separate non-packable `DataLinq.Memory.PlatformCompatibility.Smoke` generated-model runner feeds non-packable Native AOT, full-trim, and Blazor WebAssembly hosts. It executes the existing 31-token profile, not a runtime-specific capability variant: canonical/model-valued seed, primary-key hit/miss, captured equality, order plus `Take`, entity and direct scalar materialization, `Any`/`Count`, deterministic unsupported self-join rejection before work, pre-cancellation, and canonical Guid-backed key/direct-`Guid` storage. Native AOT and full-trim executables publish and exit successfully. Isolated WebAssembly no-AOT and AOT publishes execute successfully in a real browser with no warning/error entries. All four published outputs scan clean for SQL-provider and native-database payload names. A direct static-abstract generated-metadata binder method group caused a WebAssembly AOT signature mismatch; named `CreateMetadata` and `BindMetadata` wrappers now preserve the same registry/binding semantics without that malformed generic delegate thunk. WebAssembly AOT/no-AOT mode switches require clean or isolated intermediate paths to avoid cross-mode stripped-IL contamination. The memory suite passes `42/42`. This completes bounded `F7` and W8 step 10, not F4's broader memory release profile, D5 public/package promotion, W9 capability completion, or W10 release integration.
...
The bounded converted fixture proves only model-valued seed normalization, canonical Guid-backed key lookup, root entity materialization, and cache identity. It does not add typed-ID or `Guid` predicate, projection, membership, ordering, or relation capabilities.
...
Bounded W8 step-8 evidence parses each representative query shape exactly once into a single `QueryPlanInvocation` and executes that same invocation against both memory and SQLite. Both sources receive the same five-row logical seed: `Int32.MinValue`, `-11`, `0`, `17`, and `Int32.MaxValue` keys, repeated exact-`Int32` group values, and distinct names. Hard-coded expected values cover the current 31-token profile's representative root/equality, ordering, `Take`, direct scalar projection, empty-result, `Any`, and `Count` dimensions. Unordered entity results are sorted only after both executions; ordered entity and scalar sequences retain provider-returned order. The deliberate difference is not washed away: a post-`Take` `Any` invocation succeeds on SQLite and rejects as `Operation:Pushdown` on memory before row access. The `DataLinq.Tests.Memory` parity host deliberately references SQLite and therefore carries its native test assets; the `DataLinq.Memory` runtime dependency graph remains SQL-provider- and native-database-free. This is focused regression evidence, not a claim of general SQL equivalence.
...
Bounded W8 step-10 evidence uses one generated provider-free runner from Native AOT, full-trim, WebAssembly no-AOT, and WebAssembly AOT hosts. Each mode executes the exact current profile and reports 31 capabilities. Both executable hosts exit successfully, both browser modes reach the completed state with no warning/error entries, and recursive scans of all four outputs find no SQL-provider or native-database payload. This proves the vertical architecture in constrained runtimes; it is not a package, compatibility-catalog, size-threshold, or release-candidate claim. WebAssembly mode switches should use clean or isolated intermediate paths because sharing intermediates between AOT and no-AOT builds can retain stripped IL.
