...
**F7/W8 memory-spike checkpoint:** Separate non-packable `DataLinq.Memory` and non-packable TUnit `DataLinq.Tests.Memory` projects are in the solution. Each memory source owns immutable per-table `CanonicalProviderValueRow` state plus a `DataLinqKey`-to-row-ordinal primary-key index, while materialized identities remain in one existing `RowCache` per table. The internal canonical seed path publishes a table atomically, rejects duplicate keys with row context, and does not freeze a public seed API. Direct primary-key requests use `SourcePrimaryKeyRowRequest`, generated materialization, and the shared source-bound cache; a non-concurrent test eviction proves clearing that cache does not remove store rows. SQL and memory startup share one registry lock across metadata resolution, publication, and generated static binding; a gated unit test proves a concurrent resolver cannot return during the first binder, while a 32-way cold start proves every source converges on the one winning graph. The source-bound backend now declares exactly 26 supported tokens. Repeated direct `Where` predicates remain restricted to exact non-nullable `Int32` root-column equality with an exact non-null `Int32` scalar binding; the added slice admits exactly one ascending or descending direct non-nullable converter-free `Int32` single-column primary key, optionally followed by one final exact nonnegative `Int32` scalar-binding `Take`. Scope-wide ordering and paging-composition shapes plus source/result CLR invariants ensure multiple keys/`ThenBy`, manual repeated ordering, non-key/string/converted keys, unordered or repeated `Take`, and negative/non-exact counts reject during plan construction or capability validation. Explicit ordering conversions are no longer erased; a new primary `OrderBy` resets every earlier top-level ordering in its scope, while `ThenBy` without an immediately preceding primary ordering rejects. Ordered execution filters canonical rows, buffers all matches, cancellation-aware merge-sorts without subtraction, applies `Take`, and lazily materializes only selected identities. The unique admitted key makes the order total without defining tie stability; this checkpoint does not claim top-N streaming. `Take(0)` performs zero scans, over-cardinality counts return every ordered row, and public count arguments follow query-construction snapshot semantics. Focused memory and capability evidence passes `17/17` and `24/24`; integrated gates pass `1137/1137` unit, `57/57` generator, and `795/795` SQLite file/memory compliance tests. The memory project remains non-packable, builds cleanly for net8/net9/net10 with zero warnings or errors, and its resolved runtime graphs remain free of SQL-provider and native-database packages. Cancellation coverage still uses an internal spike surface; generated public LINQ supplies `CancellationToken.None`. This is not general ordering, `ThenBy`, `Skip`, stable ties, performance, or SQL parity support. Projections, scalar/terminal results, the wider null/string/converter/typed-ID/`Guid` matrix, joins/grouping diagnostics, public seeding/API/package shape, concurrent cache maintenance, raw-SQL and relation diagnostics, focused cross-backend parity, Native AOT, trimming publication, and browser/WebAssembly remain open. F4, F7, W8, M0, M1, and M2 are not complete.
