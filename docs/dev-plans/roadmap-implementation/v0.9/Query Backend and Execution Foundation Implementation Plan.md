...
**F6-B index-row progress:** `SourceIndexRowRequest` and `SourceIndexRowLoadResult` add an optional, mutation-free full-row request/result pair for one frozen table index and canonical provider key. `IDataLinqIndexRowServices` is independent of the primary-key loader capability; existing SQL `DataSourceAccess` instances expose both through the same cached `DataSourceAccessSourceRowLoader`. The SQL adapter validates source ownership and read state, converts every index component through the provider writer, owns command/reader lifetime, observes cancellation at bounded points, and returns fully buffered canonical rows. The live `TableCache` dispatch is deliberately narrower than the contract: only an exact key for one integral canonical-provider index column uses the neutral loader. Converter-backed index metadata is eligible only after the caller has supplied that exact canonical value; this boundary performs no model normalization or conversion, so a raw wrapper remains ineligible. Cold loads retain row-cache miss/store/materialization accounting; read-only loads publish the shared relation index so the warm path returns the same immutable instances without another command. A reproduced rollback-poisoning bug showed that a transaction-visible relation key set must never enter that shared committed index, so both full-row relation loading and the still-SQL-shaped key-only path now publish shared index entries only from `ReadOnlyAccess`. The original focused evidence is `6/6` contract tests, `11/11` adapter/route tests, `2/2` provider-key helper tests, and one SQLite-memory execution each for rollback isolation, cold/warm identity and telemetry, and the existing cold relation traversal. The later typed-ID relation checkpoint adds `1/1` generator-path evidence and `4/4` active-provider executions; full generator, unit, and SQLite file/memory gates pass `58/58`, `1138/1138`, and `797/797`. String/CHAR, `Guid`/UUID, composite, raw/noncanonical wrapper operands, custom sources, key-only `GetKeys` neutralization, index preload, broader relation-wrapper work, memory relations, raw/manual reader routes, and broader legacy removal remain outside F6-B. F6 and W5 remain open.

The converter-backed admissions above are specifically canonical `Int32` and canonical `Int64`. Other converted integral provider types remain on the legacy relation path until their own end-to-end evidence lands; non-converted integral F6-B behavior is unchanged.
...
F6-B and the contract-composition proof do not admit string/CHAR, `Guid`/UUID, composite indices, converter-backed integral relation types beyond exact `Int32` and `Int64`, raw/noncanonical model-wrapper operands presented directly at neutral dispatch, custom production sources, key-only `GetKeys` or index-preload neutralization, broader relation-wrapper redesign, memory relations, raw/manual reader paths, or broader legacy-route removal. The joined-reader checkpoints separately admit exact scalar converter-backed canonical `Int32` and `Int64` primary keys plus one representative Guid-backed key whenever concrete-provider storage is resolved; the current Guid-backed provider evidence uses binary mappings. All wider converted, missing text/native UUID evidence, other typed UUID mappings, composite, outer/missing-source, codec-sensitive, external, provider-less, and memory reader-key paths remain open. The composition proof also does not exercise `TableCache`, generated static `Get(...)`, a memory or query backend, transactions, or production telemetry. Those routes retain their prior behavior and gates. F6 and W5 remain open.
