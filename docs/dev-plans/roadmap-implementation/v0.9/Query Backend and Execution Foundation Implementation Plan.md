...
MySQL/MariaDB UUID provider-consumption progress on 2026-07-13: Provider-owned access and transaction readers now carry exact provider identity into column-aware direct non-primary-key, non-converter full-row `Guid` decoding, and the shared mutation writer takes the same identity from its selected factory. Native MariaDB UUID, Text36, Text32, little-endian `BINARY(16)`, and RFC-order `BINARY(16)` consume resolved metadata; binary reads use raw bytes while text/native reads accept raw text or connector-canonical `Guid`. Frozen non-symmetric vectors prove exact DataLinq writes, independently raw-seeded reads, normal and transaction access, update re-encoding, nullable Text36/SQL NULL, and provider-differentiated binary bytes under no `GuidFormat`, `Char32`, `Binary16`, and `LittleEndianBinary16`. Full gates pass `1051/1051` unit, `760/760` SQLite compliance, `429/429` on each server target (`1716/1716` total), and `414/414` combined provider-specific executions; `DataLinq.MySql` builds cleanly for `net8.0`, `net9.0`, and `net10.0`. Typed IDs, UUID primary keys, projections, predicates/membership, relations, defaults, provider-less public reader/access/transaction construction, memory, and Native AOT/browser publication remain outside this slice; aggregate `UUID-2` is not complete.
