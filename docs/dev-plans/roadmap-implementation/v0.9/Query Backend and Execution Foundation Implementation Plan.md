...
> [!NOTE]
> In the F4-B/F5 capability paragraph below, "current" means current at that historical checkpoint. The live preview-scope state is Memory `58`, catalog `617`, and SQL `359` supported / `258` unsupported.

**Status:** Complete for the scoped 0.9 contract. The production query backend boundary, capability validation, retained SQL execution families, typed-ID/UUID slices, and experimental Memory preview are implemented. Memory closes M0-M2 at the explicit 58-token boundary with direct exact-model-type scalar projection. Broader query shapes, cache/relation migration, legacy-route removal, and the unbounded remainder of F4/F6/UUID work are deferred to later releases instead of blocking 0.9.
