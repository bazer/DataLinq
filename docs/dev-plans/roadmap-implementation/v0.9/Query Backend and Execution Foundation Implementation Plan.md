...
**Status:** Implementation in progress. F0-F2 are complete; F3-F6 are active in bounded slices. The neutral row/materializer, primary-key loader, scalar-conversion, and UUID storage work documented below remains deliberately narrower than aggregate completion. Bounded `F4-B/F5-A/F5-B` adds one production request gate that selects the source-owned SQL backend and validates exactly once before work, plus complete entity and scalar-result SQL adapters with explicit result, cursor, cancellation, and command ownership. Direct-projection sequences/row terminals and retained local-projection execution remain in compatibility paths after validation; memory capabilities/backend, cache/relation migration, and compatibility-route removal remain open. F4-F6, UUID-2, and UUID-3 are not complete.

**F4-B/F5-A/F5-B capability and execution progress:** The initial contract remains frozen at 598 contextual features, with 343 supported and 255 unsupported SQL dispositions. Every production expression query creates one `QueryExecutionRequest`, selects the source-owned backend, computes structural and invocation requirements, and validates exactly once before backend work. The SQL backend owns entity sequences, all six entity terminals, and all six scalar reductions: `Count`, `Any`, `Sum`, `Min`, `Max`, and `Average`, including supported joined, paged, direct-projection, and grouped-projection `Count`/`Any` shapes. Scalar-primary-key recognition occurs only from the validated request. SQL scalar conversion remains inside the adapter, so `DBNull`, provider numeric widths, and numeric `Any` representations do not cross the neutral backend contract. A live unsupported two-argument `Substring` case proves sequence, terminal, and scalar rejection with a deterministic redacted diagnostic and zero commands, query metrics, cache probes, row loads, materializations, or stores. Entity cursors own enumeration disposal, while lazy reader and scalar execution own their commands; scalar cancellation is checked again after command creation and immediately before provider execution. Integrated evidence passes `1097/1097` unit, `774/774` SQLite compliance, `782/782` for MySQL 8.4 plus MariaDB 10.11, `782/782` for MariaDB 11.4 plus MariaDB 11.8, and provider-specific batches of `160/160` and `162/162`. `DataLinq` builds cleanly on `net8.0`, `net9.0`, and `net10.0`. This is not completed F4/F5: the memory profile/backend and direct-projection sequence/row-terminal, retained local-projection, cache/relation, and compatibility-removal work remain open.
...
| [`ValidatedQueryExecutionRequest`](../../../../src/DataLinq/Linq/Planning/QueryExecution.cs) selects and validates the source-owned backend before every production expression-query dispatch. [`SqlQueryPlanBackend`](../../../../src/DataLinq/Linq/Planning/Sql/SqlQueryPlanBackend.cs) constructs [`QueryPlanSqlBuilder`](../../../../src/DataLinq/Linq/Planning/Sql/QueryPlanSqlBuilder.cs) for the complete entity sequence/six-terminal family and the complete six-kind scalar-result family. | Bounded `F4-B/F5-A/F5-B` is live. Direct-projection sequences/row terminals and retained local-projection execution are validated by the same gate but still construct the SQL builder in `ExpressionQueryPlanExecutor` compatibility paths. |
