...
**Status:** Implementation in progress. F0-F2, `F5`, the bounded `F7` vertical proof, and D5 package promotion are complete; F3, F4, and F6 remain active in bounded slices. The neutral row/materializer, primary-key loader, bounded index-row loader, neutral test-source composition, scalar-conversion, UUID storage, and memory-preview work documented below remains deliberately narrower than aggregate completion. Bounded `F4-B/F5-A/F5-B/F5-C/F5-D` adds one production request gate that selects the source-owned SQL backend and validates exactly once before work, plus complete SQL adapters for every retained expression-query result family with explicit result, cursor, cancellation, command, and reader ownership. The current memory profile/backend executes pass-through root entity sequences, exact direct-`Int32` column/scalar equality, exact non-null direct-`Guid` and resolved Guid-backed typed-ID column/scalar equality, one exact `Int32` single-column-primary-key total ordering with an optional final exact `Take`, one final exact direct-`Int32` scalar sequence projection, selectorless `Any`/`Count`, bounded cancellation plus unsupported join/grouping/projection diagnostic evidence, focused SQLite parity for that exact island, and the same provider-free slice under Native AOT, full trimming, WebAssembly no-AOT, and WebAssembly AOT. Broader filter/order/paging semantics, other projection forms and terminals, wider parity, remaining typed-ID/`Guid` query behavior, M0-M2, and release-harness integration remain open. Remaining cache/relation migration and legacy read-route removal also remain open. F4, F6, UUID-2, and UUID-3 are not complete; W5 and the remaining W9 work remain open.
