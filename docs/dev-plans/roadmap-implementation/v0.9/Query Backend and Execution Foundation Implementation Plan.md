...
**Status:** Implementation in progress. F0-F2 are complete. F3 now has canonical provider-value rows, reader-free model-row construction, shared scalar materialization, source-scoped committed/transaction cache services, immutable primary-key row-loader contracts, a buffered SQL primary-key loader/decoder adapter, and genuine neutral generated immutable construction with legacy compatibility. Generated database roots, live cache-cold routing after the W3 gate, and the remaining SC-2 write/default paths are next.
