...
**Status:** Implementation in progress. F0-F2 are complete. F3 now has canonical provider-value rows, reader-free model-row construction, shared scalar materialization, model-to-canonical mutation serialization, checked integral auto-increment result hydration, canonical model-row/model-instance key normalization, source-scoped committed/transaction cache services, immutable primary-key row-loader contracts, a buffered SQL primary-key loader/decoder adapter, genuine neutral generated immutable construction, and neutral generated database roots with legacy compatibility. Live cache-cold routing after the W3 gate, explicit server-default slots, reader/external relation-key normalization, and the remaining SC-2 query paths are next.
