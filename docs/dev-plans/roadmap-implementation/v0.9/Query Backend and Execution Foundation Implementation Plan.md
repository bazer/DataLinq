...
**Status:** Implementation in progress. F0-F2 are complete. F3 now has canonical provider-value rows, reader-free model-row construction, shared scalar materialization, model-to-canonical mutation serialization, checked integral auto-increment result hydration, canonical model-row/model-instance key normalization, source-scoped committed/transaction cache services, immutable primary-key row-loader contracts, a buffered SQL primary-key loader/decoder adapter, genuine neutral generated immutable construction, neutral generated database roots with legacy compatibility, repository-owned root migration off the concrete SQL-shaped cast, and reload-safe insert write slots including provider-owned zero-column SQL for converter-backed IDs whose canonical provider type is integral. Bounded `F6-A` routes integral-canonical SQL single and batched primary-key cache misses through the neutral loader/materializer, enabling converter-backed non-indexed server-default hydration on that reload-safe shape. Bounded `UUID-3C` extends the same source-row route to exactly one canonical `Guid` primary-key component when the concrete SQLite, MySQL, or MariaDB source has resolved `GuidStorage`; direct `Guid` and a representative Guid-backed typed ID use exact provider reads/writes, scalar key selection, cold single/batched loading, warm identity, transaction authoritative reload, and update/delete predicates. Scalar converter-backed primary-key projections for non-simple entity queries decode to canonical provider keys before cache handoff. Converter-backed SQL `ScalarMember` results, converter-backed column members in aliased anonymous and explicit-constructor `SqlRow` projections, and model-compatible converter-backed direct `GroupedAggregate` keys use the same per-column decoder and scalar materializer; constructor-backed row/grouped results retain `SqlOnlyCompatibility`. A bounded single-source integral-keyed `ComputedRowLocal` `NewArray` slice preserves converter-backed model values under `AotStrict`. Primitive-key `JoinedRowLocal` `NewArray` SQL execution also preserves converter-backed joined-source values while retaining the projection's intentional `SqlOnlyCompatibility` fence. Non-primary-key full-row canonical `Guid` mutation and materialization consume resolved text/native/binary formats on SQLite, MySQL, and MariaDB for direct values, representative binary converter-backed typed IDs, and nullable direct/typed Text36/SQL NULL. `UUID-3A` proves exact column-aware non-key UUID parameters for the initial equality/membership matrix. Bounded `UUID-3B` treats captured scalar nullness and local-sequence total/null counts as invocation specializations, rejects explicit rebind mismatches, emits parameter-free scalar null comparisons, and partitions nullable membership into CLR-correct SQL while codec-binding only non-null values. Indexed defaults, unknown non-auto keys, string/CHAR keys, composite and other codec-sensitive key routing, UUID formats beyond the bounded direct/representative slices, general UUID projections, manual `SqlQuery`, joined typed-UUID key hydration, remaining predicate routes, relations, defaults, provider-less MySQL/MariaDB reader fallback removal, composite reader keys, member-init row evidence, joined/function-derived grouped keys, converter-backed aggregate values, `HAVING`, neutral local-recipe hydration beyond the supported scalar SQL keys, the terminal backend-validation route, legacy custom-source fallback removal, foreign-key/relation/index loads, reader/external relation-key normalization, memory, Native AOT/browser publication, and the remaining SC-2 query paths are next. F6, UUID-2, and UUID-3 remain incomplete.

**F4-A capability-contract progress:** The internal query-plan contract now has a finite, auditable requirement vocabulary, ordered recursive extraction, separate invocation-sensitive requirements, an exhaustive initial SQL disposition profile, and a focused capability exception. The frozen catalog currently contains 598 contextual features: 343 supported and 255 unsupported by the retained SQL renderer. Interaction requirements cover exact root-source topology, source nullability, join-right source roles, repeated and joined pushdown shapes, relation direction, normalized Boolean-function comparisons, nullable-inequality shape, SQL function arity shape, paging-count shape, grouped versus ordinary projection members, direct numeric aggregate selectors, projection recipes/dispositions, result kinds, and binding/null-sequence shapes. Neutral templates accept both legal `Substring` overloads, while the SQL profile rejects the two-argument form because the retained providers require start and length. Invocation-sensitive paging requirements reject null, negative, and non-convertible or overflowing counts without retaining their values. Diagnostics carry deterministic plan locations plus safe source/column context without retaining binding values. This is a contract checkpoint, not completed F4: production execution does not select a backend or run this validator yet, the exactly-once gate remains open, and the memory profile belongs to the later memory spike.
...
| SQL single and batched primary-key misses whose canonical key components are all integral use [`DataSourceAccessSourceRowLoader`](../../../../src/DataLinq/Cache/DataSourceAccessSourceRowLoader.cs), which adapts immutable [`SourcePrimaryKeyRowRequest`](../../../../src/DataLinq/Instances/SourceRowLoading.cs) batches to the existing SQL query builder, owns command/reader lifetime, and returns finite canonical `SourceRowLoadResult` batches through [`ProviderRowDecoder`](../../../../src/DataLinq/Instances/ProviderRowDecoder.cs). Bounded `UUID-3C` admits the same route for exactly one canonical `Guid` key component only when the concrete SQLite, MySQL, or MariaDB provider has exact resolved `GuidStorage`. Scalar direct and Guid-backed typed UUID key selection uses the per-column decoder before `DataLinqKey` construction. Converter-backed SQL `ScalarMember` results, converter-backed column members in aliased anonymous and explicit-constructor `SqlRow` projections, and model-compatible converter-backed direct `GroupedAggregate` keys use the same decoder plus `ProviderRowMaterializer.MaterializeValue` before result-shape adaptation; constructor-backed row/grouped results retain `SqlOnlyCompatibility`. Single-source integral-keyed `ComputedRowLocal` `NewArray` recipes consume converter-backed model values under `AotStrict`. Primitive-key `JoinedRowLocal` `NewArray` SQL execution preserves converter-backed joined-source values while retaining the projection's intentional `SqlOnlyCompatibility` fence. String/CHAR keys retain the legacy path because provider collation can differ from CLR equality; composite UUID, relation/foreign-key loading, other codec-sensitive key shapes, and custom legacy `IDataSourceAccess` implementations also retain the direct SQL-shaped routes. | Bounded `F6-A` is live for the integral-canonical primary-key/cache-cold miss family after its W3 core-fault gate, with the separately gated scalar UUID-3C extension. Terminal backend validation, composite reader keys, member-init row evidence, joined/function-derived grouped keys, converter-backed aggregate values, `HAVING`, joined typed-UUID and other non-primitive key handoff, neutral local-recipe hydration beyond the supported scalar SQL keys, remaining collation/codec-sensitive keys, legacy fallback removal, and foreign-key/relation/index requests remain later F5/F6 work. |
