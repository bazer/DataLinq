...
**Status:** Implementation in progress. F0-F2 are complete; F3-F6 are active in bounded slices. The neutral row/materializer, primary-key loader, scalar-conversion, and UUID storage work documented below remains deliberately narrower than aggregate completion. Bounded `F4-B/F5-A` adds one production request gate that selects the source-owned SQL backend and validates exactly once before work, plus a complete entity sequence/six-terminal adapter with post-validation scalar-primary-key recognition and explicit cursor/command ownership. Scalar/aggregate, direct-projection, and retained local-projection execution remains in compatibility paths after validation; memory capabilities/backend, cache/relation migration, and compatibility-route removal remain open. F4-F6, UUID-2, and UUID-3 are not complete.

**F4-B/F5-A capability and execution progress:** The initial contract remains frozen at 598 contextual features, with 343 supported and 255 unsupported SQL dispositions. Every production expression query now creates one `QueryExecutionRequest`, selects the source-owned backend, computes structural and invocation requirements, and validates exactly once before backend work. The SQL backend owns entity sequences and all six entity terminals; scalar-primary-key recognition occurs only from the validated request. A live unsupported two-argument `Substring` case proves both sequence and terminal rejection with a deterministic redacted diagnostic and zero commands, query metrics, cache probes, row loads, materializations, or stores. The entity cursor owns enumeration disposal on completion, early disposal, failure, and cancellation, while the underlying lazy `Select` reader owns its command lifetime. Integrated evidence passes `1088/1088` unit, `772/772` SQLite compliance, `780/780` for MySQL 8.4 plus MariaDB 10.11, and `780/780` for MariaDB 11.4 plus MariaDB 11.8. This is not completed F4/F5: the memory profile/backend and scalar/aggregate, direct-projection, retained local-projection, cache/relation, and compatibility-removal work remain open.
...
| [`ValidatedQueryExecutionRequest`](../../../../src/DataLinq/Linq/Planning/QueryExecution.cs) selects and validates the source-owned backend before every production expression-query dispatch. [`SqlQueryPlanBackend`](../../../../src/DataLinq/Linq/Planning/Sql/SqlQueryPlanBackend.cs) constructs [`QueryPlanSqlBuilder`](../../../../src/DataLinq/Linq/Planning/Sql/QueryPlanSqlBuilder.cs) for the complete entity sequence/six-terminal family. | Bounded `F4-B/F5-A` is live. Scalar/aggregate, direct-projection, and retained local-projection families are validated by the same gate but still construct the SQL builder in `ExpressionQueryPlanExecutor` compatibility paths. |
