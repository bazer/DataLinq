...
**Status:** Implementation in progress. F0-F2, `F5`, the bounded `F7` vertical proof, D5 package promotion, aggregate M0, W10 step-3 Testing CLI registration, bounded M1-A exact non-null inequality, and bounded M1-B exact Boolean composition are complete; F3, F4, and F6 remain active in bounded slices. The neutral row/materializer, primary-key loader, bounded index-row loader, neutral test-source composition, scalar-conversion, UUID storage, and broader Memory query work remain deliberately narrower than aggregate completion. The current Memory backend exposes model-valued exact single-column `Find<TModel>(object)` and an exhaustive 36-token LINQ profile with nested `And`/`Or`/`Not` composition over the existing exact `Int32` and canonical-`Guid` `Equal`/`NotEqual` leaves. Aggregate M0 freezes the complete metadata-only public neutral-source contract, the absence of SQL and provider-style post-seed CRUD/commit services from `MemoryDatabase<TDatabase>` and the neutral-source route, and DataLinq-owned rejection of the legacy inherited provider member plus parameterless `Delete()` extension without additional backend work. Broader filter/order/paging semantics, other projection forms and terminals, wider parity, remaining typed-ID/`Guid` query behavior, composite lookup, aggregate M1-M2, and the remaining release-harness integration remain open. Remaining cache/relation migration and legacy read-route removal also remain open. F4, F6, UUID-2, and UUID-3 are not complete; W5 and the remaining W9 work remain open.
