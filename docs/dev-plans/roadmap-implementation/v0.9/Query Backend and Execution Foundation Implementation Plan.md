...
**Status:** Implementation in progress. F0-F2, `F5`, the bounded `F7` vertical proof, D5 package promotion, aggregate M0, W10 steps 1-2 / RE-1D package-tool integration, W10 step 3 / RE-1A Testing CLI registration, bounded M1-A exact non-null inequality, bounded M1-B exact Boolean composition, bounded M1-C exact non-null `Int32` relational comparison, bounded M1-D exact local `Int32` membership, and bounded M1-E exact ordered final `Skip` are complete; F3, F4, and F6 remain active in bounded slices. The neutral row/materializer, primary-key loader, bounded index-row loader, neutral test-source composition, scalar-conversion, UUID storage, and broader Memory query work remain deliberately narrower than aggregate completion. The current Memory backend exposes model-valued exact single-column `Find<TModel>(object)` and an exhaustive 51-token LINQ profile with nested `And`/`Or`/`Not` composition over exact direct non-nullable converter-free model/provider `Int32` comparison and local-membership leaves plus exact canonical-`Guid` `Equal`/`NotEqual` leaves, together with one exact ordered final-`Skip` sequence shape. Aggregate M0 freezes the complete metadata-only public neutral-source contract, the absence of SQL and provider-style post-seed CRUD/commit services from `MemoryDatabase<TDatabase>` and the neutral-source route, and DataLinq-owned rejection of the legacy inherited provider member plus parameterless `Delete()` extension without additional backend work. Broader filter/order/paging and membership semantics outside those exact fences, other projection forms and terminals, wider parity, remaining typed-ID/`Guid` query behavior, composite lookup, and aggregate M1-M2 remain open. RE-1C, RE-1E/F/G/H, W10 steps 4-9, aggregate RE-1/RE-4/W10/W11, packaged constrained-runtime evidence, consumer smoke, final release-candidate closeout, and publication remain open. Remaining cache/relation migration and legacy read-route removal also remain open. F4, F6, UUID-2, and UUID-3 are not complete; W5 and the remaining W9 work remain open.
