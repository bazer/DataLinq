...
**Status:** Implementation in progress. F0-F2 are complete; F3-F6 are active in bounded slices. The neutral row/materializer, primary-key loader, scalar-conversion, and UUID storage work documented below remains deliberately narrower than aggregate completion. Bounded `F4-B/F5-A/F5-B/F5-C` adds one production request gate that selects the source-owned SQL backend and validates exactly once before work, plus complete entity, scalar-result, and direct-projection SQL adapters with explicit result, cursor, cancellation, command, and reader ownership. Retained local-projection execution remains in its compatibility path after validation; memory capabilities/backend, cache/relation migration, and compatibility-route removal remain open. F4-F6, UUID-2, and UUID-3 are not complete.

**F4-B/F5-A/F5-B/F5-C capability and execution progress:** The initial contract remains frozen at 598 contextual features, with 343 supported and 255 unsupported SQL dispositions. Every production expression query creates one `QueryExecutionRequest`, selects the source-owned backend, computes structural and invocation requirements, and validates exactly once before backend work. The SQL backend owns entity sequences and terminals, all six scalar reductions, and every currently supported direct SQL projection: `ScalarMember`, `SqlRow`, and `GroupedAggregate` sequences plus parser-supported scalar/row terminals. Direct projection execution returns final semantic `TResult` values through an owned cursor; SQL readers, aliases, `DBNull`, provider widths, converter-aware decoding, and reflective row construction do not cross the backend contract. Root, explicit-join, implicit-relation, joined-pushdown, grouped-row, typed-ID, and transaction-root evidence remains green. `ScalarMember` remains executable under `AotStrict`; reflective `SqlRow` and grouped construction retain their deliberate `SqlOnlyCompatibility` fence. A live unsupported two-argument `Substring` case proves entity, scalar, and direct-projection rejection with a deterministic redacted diagnostic and zero commands, query metrics, cache probes, row loads, materializations, or stores. Projection cursors dispose enumeration, reader, and command state on completion, early exit, failure, and cancellation; transaction reader telemetry is preserved without inventing an entity/scalar query classification. Integrated evidence passes `1109/1109` unit, `780/780` SQLite compliance, `788/788` for MySQL 8.4 plus MariaDB 10.11, `788/788` for MariaDB 11.4 plus MariaDB 11.8, and provider-specific batches of `160/160` and `162/162`. `DataLinq` builds cleanly on `net8.0`, `net9.0`, and `net10.0`. This is not completed F4/F5: the memory profile/backend and retained local-projection, cache/relation, and compatibility-removal work remain open.
...
| [`ValidatedQueryExecutionRequest`](../../../../src/DataLinq/Linq/Planning/QueryExecution.cs) selects and validates the source-owned backend before every production expression-query dispatch. [`SqlQueryPlanBackend`](../../../../src/DataLinq/Linq/Planning/Sql/SqlQueryPlanBackend.cs) constructs [`QueryPlanSqlBuilder`](../../../../src/DataLinq/Linq/Planning/Sql/QueryPlanSqlBuilder.cs) for the complete entity sequence/six-terminal family, complete six-kind scalar-result family, and supported direct-projection family. | Bounded `F4-B/F5-A/F5-B/F5-C` is live. Direct `ScalarMember`, `SqlRow`, and `GroupedAggregate` projection results use a semantic backend cursor. Retained local projection recipes are validated by the same gate but still construct the SQL builder in the `ExpressionQueryPlanExecutor` compatibility path. |
...
The family-specific result mechanics are now live for entity, scalar, and direct-projection results. The retained local-recipe adapter is still pending, so this is not the completed F5 surface.
