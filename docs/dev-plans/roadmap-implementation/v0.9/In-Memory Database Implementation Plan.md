...
**F7/W8 spike progress (2026-07-13):** The separate non-packable runtime and TUnit projects now exist. Dense `CanonicalProviderValueRow` instances sit behind per-table primary-key ordinal maps, while materialized immutable identities remain in separate existing `RowCache` instances. Direct neutral primary-key lookup and pass-through root entity plans execute without SQL. Repeated direct `Where` equality remains limited to an exact non-nullable `Int32` root column and exact non-null `Int32` scalar binding; the next bounded slice adds exactly one ascending or descending direct non-nullable converter-free `Int32` single-column primary-key ordering, optionally followed by one final exact nonnegative `Int32` scalar-binding `Take`. Its 26-token profile is exhaustive once shared source/table and projection/result CLR invariants are enforced: scope-wide ordering and paging-composition requirements plus the exact count shape reject multiple keys/`ThenBy`, manual repeated ordering, non-key/string/converted keys, unordered/repeated `Take`, and negative/non-exact counts before row access. The parser preserves explicit ordering conversions instead of erasing narrowing casts, resets every earlier top-level ordering when a new primary `OrderBy` appears, and rejects `ThenBy` without a preceding primary ordering. Ordered execution filters canonical rows, buffers all matches, performs a cancellation-aware merge sort without key subtraction, applies `Take`, and lazily materializes only selected identities. The admitted single-column primary key gives a total order without defining tie stability or promising top-N streaming. `Take(0)` performs no scan, over-cardinality counts return all ordered rows, and public count arguments are snapshotted at query construction. Canonical seed publication and source-local identity remain proven, while one shared SQL/memory registry lock serializes resolution through generated binding; a gated unit test proves a concurrent caller cannot observe the winner before binding completes, and a 32-way cold start proves convergence on that graph. The seed API remains internal and provider-valued. Focused memory and capability evidence passes `17/17` and `24/24`; integrated gates pass `1137/1137` unit, `57/57` generator, and `795/795` SQLite file/memory compliance tests. The runtime remains non-packable, builds cleanly for net8/net9/net10 with zero warnings or errors, and its resolved runtime graphs remain free of SQL-provider and native-database packages. Cancellation coverage still uses an internal spike surface; generated public LINQ supplies `CancellationToken.None`. This is an internal exact-`Int32` semantics proof, not general ordering, `ThenBy`, `Skip`, stable ties, performance, or SQL parity support. W8 step 6 remains partially complete because projection and `Any`/`Count` are open, and `M0`, `M1`, `M2`, `F7`, and W8 remain incomplete: model-valued seed conversion, the remaining query subset, wider semantics and focused parity, typed IDs/`Guid`, public API/package design, concurrent cache maintenance, and constrained-runtime smokes remain open.
