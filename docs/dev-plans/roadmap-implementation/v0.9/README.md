...
**Status:** Implementation in progress. W0-W2, `F5`, the bounded `F7` vertical proof, all ordered W8 spike steps, D5 package promotion, the bounded M0-A public exact single-column lookup, and W10 step-3 Testing CLI memory-suite registration are complete; W3-W6 and W9 remain active. The transaction, canonical-row/materialization, primary-key read, scalar-conversion, UUID storage, and memory checkpoints documented below remain bounded rather than aggregate completion claims. The memory preview covers canonical/model-valued seed, public model-valued `Find<TModel>(object)` for exactly one primary-key column, the current 32-token query island with exact direct-`Int32` equality plus exact non-null direct-`Guid` and resolved Guid-backed typed-ID column-to-scalar equality, representative diagnostics, focused SQLite parity, canonical Guid-backed/direct-`Guid` storage, and direct execution under Native AOT, full trimming, WebAssembly no-AOT, and WebAssembly AOT without SQL-provider/native-database payload. D5-A froze the earlier minimal public construction/seed/query surface; D5-B made `DataLinq.Memory` packable after inspecting an actual aligned core/Memory preview candidate; M0-A now adds the bounded public lookup without adding generated `Get(...)` methods or widening the LINQ capability profile. The wider query and semantic matrix, remaining typed-ID/`Guid` predicate shapes, projections, membership, ordering, relations, composite primary-key lookup, aggregate M0-M2 completion, raw-SQL rejection wording, compatibility-catalog integration, release-script integration, and final packaged release reruns remain open. Raw low-level escape prevention, local-cleanup fault boundaries, connector-native/full provider commit-fault evidence, general join compatibility, structured-query diagnostics, indexed and unknown-key server-default shapes, UUID routes beyond the bounded slices, legacy fallback removal, string/CHAR keys, composite UUID routing, broader relation/index/foreign-key UUID routing, general UUID projections and manual `SqlQuery`, broader joined typed-UUID key hydration, member-init row evidence, joined/function-derived grouped keys, converter-backed aggregate values, `HAVING`, provider-less readers, and remaining reader/external routes are also open. W3, W5, W6, F4, F6, the remaining W9 work, UUID-2, and UUID-3 remain incomplete.
