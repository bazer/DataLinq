...
**F7/W8 memory scalar-projection-spike progress:** The non-packable memory backend retains zero or more exact direct-`Int32` equality `Where` operations and exactly one optional ascending or descending ordering over the root entity's direct non-nullable converter-free `Int32` single-column primary key, optionally followed by one final exact nonnegative `Int32` scalar-binding `Take`. Its exhaustive profile now contains 29 tokens and also admits a final `ScalarMember` sequence over a direct non-nullable converter-free model/provider `Int32` root column with an exact `Int32` result; the selected column may be the primary key or a non-key column. `ScalarProjectionShape:Other` fences string, nullable, widened, boxed, converter-backed, typed-ID, `Guid`, joined, and broader projection shapes before store access. One canonical-row cursor owns scan accounting, equality filtering, ordering, `Take`, cancellation, and disposal. The entity wrapper materializes selected identities through the existing cache, while the scalar wrapper reads the selected canonical cell through shared scalar materialization/result adaptation without materializing an entity or touching `RowCache`. Unordered scalar projection streams lazily without promising stable seed order; ordered projection retains full-match buffering and total primary-key order. `Take(0)` performs no scan, counts above cardinality return all ordered rows, and a public query snapshots the count at construction. Focused memory and capability tests pass `22/22` and `25/25`; integrated gates pass `1138/1138` unit, `57/57` generator, and `795/795` SQLite file/memory compliance tests. The memory runtime builds cleanly for net8/net9/net10 with zero warnings or errors, and its resolved runtime graphs remain free of SQL-provider and native-database packages. Pre-cancellation and cancellation between projected rows are proven through the internal spike execution surface; generated public LINQ still supplies `CancellationToken.None`. This is not support for other scalar types, anonymous/constructed/local projections, post-projection operators, scalar projection terminals, `Any`/`Count`, general ordering, `ThenBy`, `Skip`, stable ties, top-N performance, or SQL parity. Focused cross-backend parity, public API/package design, concurrent cache maintenance, and Native AOT/browser evidence remain open. W8 step 6 remains partial because `Any`/`Count` is still open; `F7`, `W8`, `M0`, `M1`, and `M2` remain incomplete.
