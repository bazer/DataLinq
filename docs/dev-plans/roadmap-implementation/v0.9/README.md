...
**Status:** Implementation in progress. W0-W2 are complete; W3-W5 are active. The transaction, canonical-row/materialization, primary-key read, scalar-conversion, and UUID storage checkpoints documented below remain bounded rather than aggregate completion claims. Bounded `F4-B/F5-A/F5-B/F5-C` adds the production exactly-once capability gate plus complete entity, scalar-result, and direct-projection SQL adapters, including post-validation scalar-primary-key optimization and explicit result/cursor/cancellation/command ownership. Retained local-projection execution still uses its compatibility path after validation, and memory capabilities/backend remain open. Raw low-level escape prevention, local-cleanup fault boundaries, connector-native/full provider commit-fault evidence, join compatibility, structured-query diagnostics, indexed and unknown-key server-default shapes, UUID routes beyond the bounded slices, legacy fallback removal, string/CHAR keys, composite and relation/index/foreign-key UUID routing, general UUID projections and manual `SqlQuery`, joined typed-UUID key hydration, member-init row evidence, joined/function-derived grouped keys, converter-backed aggregate values, `HAVING`, provider-less readers, Native AOT/browser publication, and remaining reader/external routes are also open. W3, F4-F6, UUID-2, and UUID-3 remain incomplete.

**F4-B/F5-A/F5-B/F5-C capability and execution progress:** The 598-feature vocabulary and exhaustive SQL profile remain unchanged at 343 supported and 255 unsupported dispositions. Every production expression query creates one `QueryExecutionRequest`, selects the source-owned backend, extracts requirements, and validates exactly once before backend work. The SQL adapter owns the complete entity family, all six scalar reductions, and every currently supported direct SQL projection: `ScalarMember`, `SqlRow`, and `GroupedAggregate` sequences plus parser-supported scalar/row terminals. Its typed semantic projection cursor exposes no reader, command, `DBNull`, alias, or provider-width artifact, and owns enumeration disposal on completion, early exit, conversion/construction failure, and cancellation. Root, explicit-join, implicit-relation, joined-pushdown, grouped-row, typed-ID, and transaction-root evidence remains green; reflective `SqlRow` and grouped construction intentionally remain `SqlOnlyCompatibility`, while `ScalarMember` remains executable under `AotStrict`. Reader-command telemetry and transaction attribution are preserved without misclassifying projection work as entity or scalar query metrics. Unsupported two-argument `Substring` entity, scalar, and direct-projection plans are rejected before commands, query metrics, cache probes, row loads, or materialization. The full unit gate passes `1109/1109`; SQLite compliance passes `780/780`; paired server runs pass `788/788` for MySQL 8.4 plus MariaDB 10.11 and `788/788` for MariaDB 11.4 plus MariaDB 11.8; provider-specific batches pass `160/160` and `162/162`. Memory capabilities/backend and migration of retained local-projection compatibility paths remain open, so F4, F5, and W5 are not complete.
...
- Every production expression query now passes through one [`QueryExecutionRequest`](../../../../src/DataLinq/Linq/Planning/QueryExecution.cs), source-owned backend selection, and capability validation before dispatch. Entity sequences and terminals, all six scalar reductions, and the supported direct `ScalarMember`/`SqlRow`/`GroupedAggregate` projection family execute through [`SqlQueryPlanBackend`](../../../../src/DataLinq/Linq/Planning/Sql/SqlQueryPlanBackend.cs). Only retained local-projection recipes remain in the `ExpressionQueryPlanExecutor` compatibility path that still constructs [`QueryPlanSqlBuilder`](../../../../src/DataLinq/Linq/Planning/Sql/QueryPlanSqlBuilder.cs) after validation.
