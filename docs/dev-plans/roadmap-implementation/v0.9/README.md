...
**UUID validation/default-literal progress:** After exact active-provider SQL type equality, `SchemaComparer` now compares resolved UUID metadata or raw exact/default `[GuidStorage]` intent without invoking converters. MySQL/MariaDB `CHAR(36)`/`VARCHAR(36)`, `CHAR(32)`/`VARCHAR(32)`, and native MariaDB `UUID` are schema-observable; unhinted MySQL/MariaDB `BINARY(16)` and SQLite `BLOB` or `TEXT` remain `Error/Ambiguous` unresolved because those SQL types do not encode byte order or text shape. Trusted same-type format changes report a manual data migration and diff generation stays comment-only; physical type drift suppresses secondary format noise. Bare deferred `Guid`/`System.Guid`/`global::System.Guid` source is also unresolved because source-only metadata cannot disprove an assembly scalar registration, while a deferred property-converter marker is not misclassified as UUID intent. A second bounded handoff formats fixed `Guid` values already present in finalized direct-canonical metadata through the same resolved codec used by runtime writes before SQLite/MySQL/MariaDB DDL literal rendering. It covers all 13 text/native/binary provider-format combinations; ambiguous layouts and converter-backed defaults reject before SQL with zero converter calls. Defaults evidence is `9/9` MySQL/MariaDB literals, `3/3` SQLite literal/engine/guard cases, and `4/4` active servers; integrated gates pass `58/58` generator, `1173/1173` unit, `803/803` SQLite file/memory, and `354/354` provider-specific tests across all server targets. This is finalized-metadata DDL evidence, not public source-model support: legal fixed-`Guid` source representation/regeneration, converter-backed model defaults, `DefaultNewUUID` Version4/Version7 client/server and provider-version semantics, net8 client generation, independent canonical compatibility, authoritative source-only typed-ID resolution, and aggregate SC-5/UUID-4 completion remain open.
