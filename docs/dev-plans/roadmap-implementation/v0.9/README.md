...
**F7/W8 memory ordering/`Take`-spike progress:** The non-packable memory backend retains zero or more exact direct-`Int32` equality `Where` operations and now admits exactly one ascending or descending ordering over the root entity's direct non-nullable converter-free `Int32` single-column primary key, optionally followed by one final exact nonnegative `Int32` scalar-binding `Take`. Its 26-token profile is exhaustive for that conjunction once the shared template's source/result CLR invariants are enforced. Global ordering and paging-composition shapes fence multiple keys/`ThenBy`, manual repeated orderings, non-key/string/converted orderings, unordered or repeated `Take`, and negative or non-exact counts before store access. The parser now preserves explicit ordering conversions instead of erasing narrowing casts, resets every earlier top-level primary ordering in the scope, and rejects `ThenBy` without a preceding primary ordering. Ordered execution filters canonical rows, buffers all matches, performs a cancellation-aware merge sort without subtracting keys, applies `Take`, and lazily materializes only selected rows through the existing identity cache. The admitted single-column primary key makes the order total, so no tie or stable-seed-order contract is implied. `Take(0)` performs no scan; counts above cardinality return all ordered rows; a public query snapshots the count at construction and must be rebuilt to observe a changed local. Focused memory and capability tests pass `17/17` and `24/24`; integrated gates pass `1137/1137` unit, `57/57` generator, and `795/795` SQLite file/memory compliance tests. The memory runtime builds cleanly for net8/net9/net10 with zero warnings or errors, and its resolved runtime graphs remain free of SQL-provider and native-database packages. Cancellation coverage remains request/cursor evidence through an internal spike surface; generated public LINQ still supplies `CancellationToken.None`. This is not general ordering, `ThenBy`, `Skip`, stable-tie behavior, top-N performance, or SQL parity evidence. Projection, `Any`/`Count`, the wider null/string/converter/typed-ID/`Guid` matrix, focused cross-backend parity, public API/package design, concurrent cache maintenance, and Native AOT/browser evidence remain open. W8 step 6 is only partially entered; `F7`, `W8`, `M0`, `M1`, and `M2` remain incomplete.
