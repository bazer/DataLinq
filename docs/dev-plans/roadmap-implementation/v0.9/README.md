...
**F7/W8 memory equality-spike progress:** The non-packable memory backend now executes zero or more direct `Where` operations when each predicate is one `Equal` comparison between a direct non-nullable `Int32` root column and an exact non-null `Int32` scalar binding. The 17-token profile is exhaustive for that slice: broader default-null-semantics shapes, including string equality, column-to-column equality, nullable or mismatched scalar declarations, and C# numeric-promotion cases, fail capability validation before row access. Invocation-local compilation normalizes each scalar once, preserves capture rebinding and operand reversal, short-circuits repeated filters over canonical rows, and materializes only accepted rows through the existing identity cache. Focused memory tests pass `11/11`; the shared capability contracts pass `19/19`; integrated gates pass `1130/1130` unit, `57/57` generator, and `792/792` SQLite file/memory compliance tests. Cancellation coverage is request/cursor evidence through an internal spike surface; generated public LINQ still supplies `CancellationToken.None`, so no public cancellable-query API is claimed. This is an internal exact-`Int32` equality proof, not general `Where` or numeric-comparison support. Ordering and `Take`, projection, scalar/terminal results, null/string/converter/typed-ID/`Guid` semantics, SQLite parity for the memory result, public API/package design, concurrent cache maintenance, and Native AOT/browser evidence remain open. W8 step 6 is only partially entered; `F7`, `W8`, `M0`, `M1`, and `M2` remain incomplete.
