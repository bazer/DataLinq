...
**F7/W8 provider-free constrained-runtime checkpoint:** `DataLinq.Memory.PlatformCompatibility.Smoke` now supplies the same generated-model runner to separate non-packable Native AOT, full-trim, and Blazor WebAssembly hosts. The current 31-token profile executes canonical/model-valued seed, primary-key hit/miss, captured equality, order plus `Take`, entity and scalar results, `Any`/`Count`, deterministic self-join rejection before work, pre-cancellation, and canonical Guid-backed/direct-`Guid` storage. Native AOT and full-trim executables publish and exit successfully; isolated WebAssembly no-AOT and AOT publishes complete in a real browser with zero warning/error entries. All four published outputs scan clean for `DataLinq.SQLite`, `DataLinq.MySql`, `Microsoft.Data.Sqlite`, `MySqlConnector`, `SQLitePCLRaw`, and `e_sqlite3`. Clean or isolated intermediate paths are required when switching WebAssembly AOT modes to avoid cross-mode stripped-IL contamination. The memory suite passes `42/42`. This completes bounded `F7` and W8 step 10 without widening the query profile, making `DataLinq.Memory` packable, freezing a public seed API, or completing W9/W10.
...
The bounded spike now proves every bullet above for its current 31-token profile. Step 8 sends each representative parsed invocation unchanged through SQLite and memory, and step 10 executes the provider-free route under Native AOT, full trimming, WebAssembly no-AOT, and WebAssembly AOT. The unsupported matrix covers a valid self-join, grouped aggregate, and captured computed projection with exact capability feature/location, captured-value redaction, and unchanged memory diagnostics. Cancellation evidence preserves the original token before work and at bounded row-yield points. This closes bounded `F7`; it does not claim broader parity, W9 capabilities, public/package promotion, or W10 release evidence.
