...
**Status:** Implementation in progress. W0-W2 are complete; W3-W5 are active. The transaction, canonical-row/materialization, primary-key read, scalar-conversion, and UUID storage checkpoints documented below remain bounded rather than aggregate completion claims. Bounded `F4-B/F5-A` adds the production exactly-once capability gate and the complete entity sequence/six-terminal SQL adapter, including post-validation scalar-primary-key optimization and explicit cursor/command ownership. Scalar/aggregate, direct-projection, and retained local-projection execution still use compatibility paths after validation, and memory capabilities/backend remain open. Raw low-level escape prevention, local-cleanup fault boundaries, connector-native/full provider commit-fault evidence, join compatibility, structured-query diagnostics, indexed and unknown-key server-default shapes, UUID routes beyond the bounded slices, legacy fallback removal, string/CHAR keys, composite and relation/index/foreign-key UUID routing, general UUID projections and manual `SqlQuery`, joined typed-UUID key hydration, member-init row evidence, joined/function-derived grouped keys, converter-backed aggregate values, `HAVING`, provider-less readers, Native AOT/browser publication, and remaining reader/external routes are also open. W3, F4-F6, UUID-2, and UUID-3 remain incomplete.

**F4-B/F5-A capability and execution progress:** The 598-feature vocabulary and exhaustive SQL profile remain unchanged at 343 supported and 255 unsupported dispositions. Every production expression query now creates one `QueryExecutionRequest`, selects the source-owned backend, extracts requirements, and validates exactly once before backend work. The SQL adapter owns the complete entity family: sequences plus `Single`, `SingleOrDefault`, `First`, `FirstOrDefault`, `Last`, and `LastOrDefault`. Its scalar-primary-key shortcut is recognized only from the validated request, so the optimization cannot bypass capability validation. Unsupported two-argument `Substring` sequence and terminal queries are rejected before any command, query metric, cache probe, row load, or materialization, and the diagnostic does not retain the captured value. The entity cursor disposes its enumerator on completion, early disposal, failure, and cancellation; the underlying `Select` iterator explicitly owns its command lifetime. The full unit gate passes `1088/1088`; SQLite compliance passes `772/772`; paired server runs pass `780/780` for MySQL 8.4 plus MariaDB 10.11 and `780/780` for MariaDB 11.4 plus MariaDB 11.8. Memory capabilities/backend and migration of scalar/aggregate, direct-projection, and retained local-projection compatibility paths remain open, so F4, F5, and W5 are not complete.
...
- Every production expression query now passes through one [`QueryExecutionRequest`](../../../../src/DataLinq/Linq/Planning/QueryExecution.cs), source-owned backend selection, and capability validation before dispatch. Entity sequences and all six entity terminals execute through [`SqlQueryPlanBackend`](../../../../src/DataLinq/Linq/Planning/Sql/SqlQueryPlanBackend.cs), while scalar/aggregate, direct-projection, and retained local-projection families remain in `ExpressionQueryPlanExecutor` compatibility paths that still construct [`QueryPlanSqlBuilder`](../../../../src/DataLinq/Linq/Planning/Sql/QueryPlanSqlBuilder.cs) after validation.
