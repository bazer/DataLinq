...
**UUID schema-format progress:** After exact active-provider SQL type equality, `SchemaComparer` now compares resolved UUID metadata or raw exact/default `[GuidStorage]` intent without invoking converters. MySQL/MariaDB `CHAR(36)`/`VARCHAR(36)`, `CHAR(32)`/`VARCHAR(32)`, and native MariaDB `UUID` are schema-observable; unhinted MySQL/MariaDB `BINARY(16)` and SQLite `BLOB` or `TEXT` remain `Error/Ambiguous` unresolved because those SQL types do not encode byte order or text shape. Trusted same-type format changes report a manual data migration and diff generation stays comment-only; physical type drift suppresses secondary format noise. Bare deferred `Guid`/`System.Guid`/`global::System.Guid` source is also unresolved because source-only metadata cannot disprove an assembly scalar registration, while a deferred property-converter marker is not misclassified as UUID intent. Focused evidence is `13/13` comparer, `6/6` configured SQLite validator, `2/2` live MySQL/MariaDB binary-import, and `1/1` live MariaDB native-UUID tests; integrated generator, unit, and SQLite file/memory gates pass `58/58`, `1170/1170`, and `803/803`, and the full MySQL/MariaDB provider-specific lane passes `326/326` across all supported server targets. Independent canonical compatibility, source-only typed-ID converter resolution, static UUID defaults, UUID-version diagnostics, and aggregate SC-5/UUID-4 completion remain open.
