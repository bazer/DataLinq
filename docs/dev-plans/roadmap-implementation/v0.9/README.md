...
**Status:** Implementation in progress. W0-W2 and `F5` are complete; W3-W6 remain active, and W8/F7 is in its separate-project memory spike. The transaction, canonical-row/materialization, primary-key read, scalar-conversion, UUID storage, and memory checkpoints documented below remain bounded rather than aggregate completion claims. Bounded `F4-B/F5-A/F5-B/F5-C/F5-D` adds the production exactly-once capability gate plus complete SQL adapters for every retained expression-query result family, including post-validation scalar-primary-key optimization and explicit result/cursor/cancellation/command ownership. The memory spike now has canonical read-only storage, neutral primary-key lookup, pass-through entity scans, exact direct-`Int32` equality, one total-order slice over an exact `Int32` single-column primary key with an optional final exact-`Int32` `Take`, one final exact direct-`Int32` scalar sequence projection, selectorless `Any`/`Count` reductions, bounded cancellation plus unsupported join/grouping/projection diagnostic evidence, and a focused SQLite/memory comparison for that exact admitted island. Its wider query subset, broader semantic matrix, typed-ID/`Guid` predicates, projections, membership, ordering, and relations beyond the bounded model-valued seed/read checkpoint, public package/API, and constrained-runtime evidence remain open. Raw low-level escape prevention, local-cleanup fault boundaries, connector-native/full provider commit-fault evidence, general join compatibility, structured-query diagnostics, indexed and unknown-key server-default shapes, UUID routes beyond the bounded slices, legacy fallback removal, string/CHAR keys, composite UUID routing, relation/index/foreign-key UUID routing beyond the exact resolved canonical-`Guid` F6-B slice, general UUID projections and manual `SqlQuery`, joined typed-UUID key hydration beyond the exact representative binary-storage slice, member-init row evidence, joined/function-derived grouped keys, converter-backed aggregate values, `HAVING`, provider-less readers, Native AOT/browser publication, and remaining reader/external routes are also open. W3, W5, W6, W8, F4, F6, F7, UUID-2, and UUID-3 remain incomplete.
...
The bounded primitive checkpoint now proves every bullet above except SQL-adapter parity and browser/WebAssembly AOT execution; it executes both `Any` and `Count`. The unsupported matrix covers a valid self-join, grouped aggregate, and captured computed projection with exact capability feature/location, captured-value redaction, and unchanged memory diagnostics. Cancellation evidence preserves the original token before work and at bounded row-yield points. This closes W8 step 7, not the remaining parity or constrained-runtime gates.
