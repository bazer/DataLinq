...
**W3 current boundary:** `TX-0` is closed. The bounded `ML-1`/`ML-2` substrate captures provider and transaction provenance and rejects invalid, terminal, wrong-owner, read-only, unsafe-shape, primary-key-drift, and foreign-metadata mutations before provider work. `TX-1A`/`TX-1B` supply reference-identity touched ownership and private successful-only commit authority. `TX-2A` finalizes confirmed owned commits before wrapper `Committed` observation; bounded `TX-2B` recovers known-committed publication/local-cleanup failure with permanent invalidation and provider-wide cache clearing. Bounded `TX-3` finalizes managed rollback/open disposal with provider-first completion, exact scoped cleanup, terminal mutable ownership, and deferred finalized rollback observation. Bounded `TX-4A` poisons post-preflight mutation failures, while managed `CommitOutcomeUnknown` recovery preserves a throwing provider exception, invalidates/clears touched and scoped state, evicts provider-wide caches before recovery notifications, and rejects unsafe reuse without claiming a database outcome. `TX-5A` proves active attached wrapper-only commit promotion/reuse and rollback invalidation across every provider, and routes wrapper commit after external commit/rollback into unknown-outcome recovery instead of guessed publication. `TX-5B` detects the inactive original handle before managed read/write/fallback/dispose, installs permanent `ExternalCompletionUnknown`, and extends provider-wide recovery to external-completion rollback/disposal; active-provider tests rematerialize the real external commit/rollback result afterward. Shipped guidance and API remarks define wrapper-only completion, raw-write/cache limitations, low-level escape behavior, provider settings, and attached connection lifetime, completing `TX-5`. Managed reentrancy is rejected and completion entry points are serialized. This is still not the W3 exit: provider-specific evidence after a throwing commit, raw `DatabaseAccess`/`IDbTransaction` escape prevention, broader local-cleanup fault injection, full provider commit-fault evidence, full concurrency semantics, and `F6` live-read routing remain open.
