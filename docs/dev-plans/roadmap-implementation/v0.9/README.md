...
**F7/W8 memory cancellation-and-diagnostics-spike progress:** The non-packable memory backend retains zero or more exact direct-`Int32` equality `Where` operations and exactly one optional ascending or descending ordering over the root entity's direct non-nullable converter-free `Int32` single-column primary key, optionally followed by one final exact nonnegative `Int32` scalar-binding `Take`. Its exhaustive profile remains 31 tokens and also admits a final `ScalarMember` sequence over a direct non-nullable converter-free model/provider `Int32` root column with an exact `Int32` result; the selected column may be the primary key or a non-key column. Selectorless `Any` and `Count` reduce either root entities or that exact scalar projection through the shared canonical-row cursor; predicate overloads remain bounded to the admitted exact root-column equality shape. Empty input returns `false`/`0`, unordered `Any` short-circuits, `Count` exhausts the selected cursor with checked `Int32` arithmetic, and ordered reductions inherit full-match buffering. Reductions materialize neither entities nor projected cells and perform zero `RowCache` work. Terminal operations after paging remain parser pushdowns and reject before row access. `ScalarProjectionShape:Other` still fences string, nullable, widened, boxed, converter-backed, typed-ID, `Guid`, joined, and broader projection shapes. Natural self-join, grouped-aggregate, and captured row-local projection plans now freeze deterministic first capability failures (`SourceCount:Multiple` at `sources`, `Operation:GroupBy` at `operations[0]`, and `Projection:ComputedRowLocalExpression` at `projection`) before any store/cache diagnostic changes, and the captured value is redacted. The same canonical-row cursor owns scan accounting, filtering, ordering, `Take`, cancellation, and disposal. Entity sequences materialize selected identities through the existing cache, while scalar sequences read selected canonical cells through shared scalar materialization/result adaptation. Unordered scalar projection streams lazily without promising stable seed order; ordered projection retains full-match buffering and total primary-key order. `Take(0)` performs no scan, `Take` limits above cardinality return all ordered rows, and a public query snapshots the count at construction. Focused memory and capability tests pass `31/31` and `25/25`; integrated gates pass `1138/1138` unit, `57/57` generator, and `795/795` SQLite file/memory compliance tests. The memory runtime builds cleanly for net8/net9/net10 with zero warnings or errors, and its resolved runtime graphs remain free of SQL-provider and native-database packages. Internal pre-cancellation and bounded entity, filtered, ordered, and projected yield cancellation plus scalar-reduction pre-cancellation are proven; generated public LINQ still supplies `CancellationToken.None`. This is not support for other scalar types, anonymous/constructed/local projections, projected-value predicates, post-projection composition beyond selectorless `Any`/`Count`, `LongCount`, other scalar reductions, element terminals, general ordering, `ThenBy`, `Skip`, stable ties, top-N performance, or SQL parity. Focused cross-backend parity, public API/package design, concurrent cache maintenance, and Native AOT/browser evidence remain open. W8 steps 6 and 7 are complete at this bounded checkpoint; `F7`, `W8`, `M0`, `M1`, and `M2` remain incomplete.
