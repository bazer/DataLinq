...
**W3 relation/index qualification:** The final blanket relation/index phrase above now has one bounded exception: an already-normalized exact single-column integral key may traverse the neutral `F6-B` relation-index loader even when the column has a scalar converter. All other relation/index families remain open, and `TableCache` itself still performs no model conversion.
