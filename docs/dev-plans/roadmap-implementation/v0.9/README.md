...
**W3 current boundary:** `TX-0` is closed by that W1 evidence. The bounded `ML-1` substrate captures exact provider origin from an immutable row, uses an opaque exact-transaction token for transaction-local provenance, and binds successful transaction hydration and mutable delete through owner-aware internal transitions. The `ML-2` slice rejects read-only/terminal transactions, invalid or deleted mutables, wrong provider or transaction owners, unsafe insert/update shapes, tracked or untracked primary-key drift, foreign metadata handles, and immutable deletes with a conflicting known owner before command construction. Public `StateChange` execution now enters the same transaction-owned finalization path as normal mutations: its captured mutation values are detached from in-place array drift, relation/index impact keys captured after provider execution remain provisional until the authoritative reload finalizes them, and later mutation drift is rejected during preflight without poisoning the transaction. `TX-1A` registers lifecycle-aware insert/update and mutable-delete inputs by reference identity only after their successful authoritative transition. `TX-1B` keeps the ordered commit authority private, exposes `Changes` only as a detached snapshot, and records a candidate only after provider execution, transaction-local cache application, authoritative hydration, impact-key finalization, and lifecycle finalization all succeed. `TX-2A` owns the confirmed-success path for DataLinq-owned transactions: after provider commit returns, it publishes the private successful changes globally, removes transaction-local cache state, explicitly promotes every touched mutable while preserving committed deletion, commits the ownership token, clears the touched registry, and only then raises the deferred wrapper `Committed` event. During the narrower interval where the provider already reports `Committed` but local finalization is still running, transaction-bound immutable, foreign-key, and relation read-source fallback is rejected instead of switching early to committed reads. Bounded `TX-2B` now handles the distinct case where `DatabaseAccess.Commit()` returned successfully and later committed publication or transaction-cache cleanup failed: it reports `TransactionCommitFinalizationException` with the original failure as `InnerException` and any recovery failures separately, never rolls back or raises the wrapper `Committed` event, invalidates the ownership token and touched mutables with `CommittedStateFinalizationFailed`, attempts transaction-local removal from every table, then clears provider-wide committed rows and indices before best-effort recovery notifications. Bounded `TX-3` covers explicit rollback and direct disposal of an open or poisoned managed wrapper. It attempts provider completion first; distinguishes confirmed `RolledBack` from `RollbackOutcomeUnknown` and direct `OpenTransactionDisposed`; invalidates touched mutables without replacing an earlier `MutationFailed`; clears the registry; discards exact transaction rows/subscriptions without clearing committed state; and defers wrapper `RolledBack` observation until finalization. A failed rollback that leaves the provider open permits only disposal. Exact provider failures remain primary with DataLinq finalization context and secondary failures attached. Bounded `TX-4A` rethrows the original post-preflight mutation failure, poisons the DataLinq wrapper, invalidates the current and previously touched lifecycle mutables, and blocks managed reads, writes, callbacks, and `Commit()` while leaving `Rollback()` and `Dispose()` legal. The adjacent managed commit-call recovery preserves a throwing provider exception, records permanent `CommitOutcomeUnknown`, invalidates/clears touched and exact transaction state, structurally evicts provider-wide committed rows and indices before recovery notifications, retains recovery faults as secondary exception context, gates managed reads/writes/callbacks/repeated commit/fallback, and allows only status-compatible rollback or disposal without claiming a database outcome. Mutation/cache-callback managed reentrancy is rejected and completion entry points are serialized. Definite terminal diagnostics take precedence once the wrapper reaches that state, while commit uncertainty remains permanent. This is still not the W3 exit: provider-specific evidence for what happened after a throwing commit, attached/external completion beyond the narrow externally completed rollback fence, raw `DatabaseAccess`/underlying `IDbTransaction` escape closure, local-cleanup fault injection beyond the supported cache primitives, full provider commit-fault evidence, full concurrency semantics, and `F6` live-read routing remain open.
