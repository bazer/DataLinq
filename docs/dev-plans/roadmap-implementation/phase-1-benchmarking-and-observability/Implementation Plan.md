...
- table-level row-cache hits
- table-level row-cache misses
- table-level database rows loaded
- table-level row materializations
- table-level row-cache stores
- table-level relation reference cache hits and loads
- table-level relation collection cache hits and loads
- table-level cache-notification subscription and sweep metrics

## Missing or Weak

- DB command count by command type
- DB command duration
- DB command failure count
- transaction start count
- transaction commit count
- transaction rollback count
- transaction duration
- transaction change count
- mutation counts by insert/update/delete
- affected row counts
- cache clear and eviction counts
- cleanup duration and removed-row counts
- index cache occupancy and churn
- end-to-end query duration
- startup or first-query cost tracking
