...
The query-plan, Remotion isolation, and supported-subset parser work became the 0.8 focus and is now closed through [DataLinq 0.8 Roadmap](v0.8/README.md) Phase 7: query contract baseline, Remotion plan adapter, SQL generation on the plan, supported-subset parser, projection/AOT cleanup, dual-run parity, production provider switch, and Remotion dependency removal. The remainder of 0.8 is now focused on the generated SQLite browser AOT release gate, which currently reports a real SQLite/WebAssembly runtime failure, then completing query-composition/subquery-pushdown hardening, source-slot joins, relation-aware joins, and implicit joins on the DataLinq-owned query plan.
