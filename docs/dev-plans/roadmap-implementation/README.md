...
| Phase 12C: CLI Configuration and Regeneration Workflow | Next implementation priority | `phase-12c-cli-configuration-and-regeneration-workflow/` |
| Phase 13: Explicit Multi-Join Composition | Planned after Phase 12C | `phase-13-explicit-multi-join-composition/` |
...
As of 2026-05-17, Phases 1 through 12 are closed execution history, and Phase 12B is complete as the generation-trust hardening insertion before query API expansion. Phase 12C is now the next implementation priority so DataLinq can clean up the CLI, configuration, regeneration, schema, and secrets workflow before returning to query API expansion.
...
Phase 12B completed generation trust before the runtime/query roadmap resumes: aggregate validation diagnostics, source-location fidelity, safe CLI generation, partial source-generator output, generated-file banners, optional header stamping, and nullable-reference-generation defaults. Phase 12C now handles the pre-1.0 CLI cleanup and configuration workflow. The roadmap then continues with Phase 13 explicit multi-join composition, relation-aware joins, scalar converters, result-set caching, and Remotion isolation.
