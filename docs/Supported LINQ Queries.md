...
In 0.8 and later, `Database.Query()` runs through DataLinq's expression parser and query-plan SQL renderer. In the current 0.9 implementation, the production path is `ExpressionQueryPlanProvider` -> `ExpressionQueryPlanParser` -> `QueryPlanTemplate` + `QueryPlanInvocation` -> `QueryExecutionRequest` -> backend selection and capability validation -> `SqlQueryPlanBackend` -> `QueryPlanSqlBuilder`. Entity sequences, the six entity terminals (`Single`, `SingleOrDefault`, `First`, `FirstOrDefault`, `Last`, and `LastOrDefault`), and the six scalar reductions (`Count`, `Any`, `Sum`, `Min`, `Max`, and `Average`) execute through the SQL backend. Direct-projection sequences/row terminals and retained local-projection execution are still selected and validated through the same production gate, but temporarily use compatibility paths while their backend adapters remain roadmap work.
