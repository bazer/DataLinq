...
In 0.8 and later, `Database.Query()` runs through DataLinq's expression parser and query-plan SQL renderer. In the current 0.9 implementation, the production path is `ExpressionQueryPlanProvider` -> `ExpressionQueryPlanParser` -> `QueryPlanTemplate` + `QueryPlanInvocation` -> `QueryExecutionRequest` -> backend selection and capability validation -> `SqlQueryPlanBackend` -> `QueryPlanSqlBuilder`. Entity sequences and the six entity terminals (`Single`, `SingleOrDefault`, `First`, `FirstOrDefault`, `Last`, and `LastOrDefault`) use the SQL backend's owned entity cursor. Scalar/aggregate, direct-projection, and retained local-projection families are still selected and validated through the same production gate, but temporarily execute through compatibility paths while their backend adapters remain roadmap work.
