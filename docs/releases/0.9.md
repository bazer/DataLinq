...
The explicit `latest` test defaults are MySQL 9.7 and MariaDB 12.3. The [provider guide](https://github.com/bazer/DataLinq/blob/0.9.0/docs/backends/MySQL-MariaDB.md) owns the supported-version contract, while the [test-provider matrix](https://github.com/bazer/DataLinq/blob/0.9.0/docs/support-matrices/Test%20Provider%20Matrix.md) owns exact aliases and images. [Full Matrix Nightly #139](https://github.com/bazer/DataLinq/actions/runs/32417436568) passed all 17 source/test shards and the aggregate gate for 5,428/5,428 cases after the provider expansion; that is durable pre-release source evidence, not a substitute for the final package-backed release record.
...
The final package comparison found no unexplained public API breaks against 0.8.0. The [0.9 release checklist](https://github.com/bazer/DataLinq/issues/80) records the approved commit and package hashes, 6,711/6,711 final test cases, exact-package restore/build/execution on .NET 8/9/10, zero API breaks, all eight packaged constrained-runtime targets passing, the accepted benchmark dispositions, and the explicit maintainer GO.
