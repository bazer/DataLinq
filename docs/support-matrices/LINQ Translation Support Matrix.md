...
| Nullable value predicates | `.HasValue`, `!HasValue`, guarded `.Value` comparisons, selected guarded date/time member access, mixed nullable/non-nullable equality and inequality, and captured null equality/inequality | `Translation/EmployeesNullablePredicateTests.cs`, `Translation/EmployeesDateTimeMemberTests.cs`, `Translation/QueryPlanSqlParityTests.cs` | The support boundary is intentionally documented. `nullable != nonNullable` includes null rows, while captured null comparisons render parameter-free `IS NULL`/`IS NOT NULL`, matching C# lifted nullable semantics. |
...
| Singular implicit relation traversal | root-row singular relation member access in `Where`, `OrderBy`, `ThenBy`, direct `Select(...)`, and supported computed row-local projection recipes, rendered as an implicit inner join and reused for repeated relation references | `Translation/EmployeesImplicitRelationJoinTests.cs`, `Translation/EmployeesProjectionTranslationTests.cs`, `Translation/QueryPlanSnapshotTests.cs` | Direct members are SQL-backed aliases. Computed related-member recipes use SQL-owned joined-row materialization and are therefore SQL-only in 0.9. Relation object projection, collection relation projection, multi-hop traversal, left-join null semantics, and fluent relation-aware join APIs remain outside the shipped boundary. |
...
| Nullable local membership | positive and negated `Contains(...)` plus equality-shaped `Any(predicate)` over mixed, non-null-only, null-only, and empty nullable sequences | `Translation/EmployeesNullablePredicateTests.cs`, `Translation/QueryPlanSqlParityTests.cs` | SQL partitions null from non-null members so C# semantics survive positive and negative membership; only non-null members bind as parameters. |
| Local object-list `Any(predicate)` | scalar item equality, object-member equality, reversed equality, nullable wrapper normalization, and negated membership | `Translation/EmployeesEmptyListQueryTests.cs`, `Translation/EmployeesLocalAnyPredicateTests.cs`, `Translation/EmployeesNullablePredicateTests.cs` | Equality-membership shapes are covered. Nullable sequences use the same null partition as `Contains(...)`; compound non-empty local predicates remain unsupported. |
...
| Computed scalar projection | self-contained row-local recipes for string concatenation and supported materialized member chains after SQL filtering, ordering, and paging, including a computed member chain over an implicit singular relation | `Translation/EmployeesProjectionTranslationTests.cs` | Single-source recipes are AOT-safe. Joined-row recipes are self-contained but SQL-only in 0.9. Do not generalize projection functions to SQL predicate translation. |
...
| Area | Currently tested support | Evidence | Audit notes |
| --- | --- | --- | --- |
| Inner `Join(...)` and single query-syntax inner join | one explicit inner join between two direct DataLinq query sources, direct member equality keys, nullable `.Value` key normalization, SQL-backed direct source-slot projection, self-contained supported row-local result recipes, single query-syntax transparent-identifier binding, composed `Where`, ordering, paging, `Any`, and `Count` over projected members, and post-paging `Where`/ordering/`Any`/`Count` over SQL-backed joined projection rows | `Translation/EmployeesJoinTranslationTests.cs`, `Translation/QueryPlanSnapshotTests.cs`, `Translation/QueryPlanUnsupportedShapeTests.cs`, `Translation/QueryPlanSqlParityTests.cs`, `Translation/JoinKeyCompatibilityTranslationTests.cs`, `src/DataLinq.Tests.Unit/Linq/QueryPlanSqlJoinCompatibilityValidatorTests.cs` | Direct joined rows read SQL aliases. Row-local joined recipes buffer ordered primary keys and materialize both sources before recipe evaluation; this path remains SQL-only in 0.9. Joined pushdown applies only to SQL-backed rows. Converter-aware explicit and implicit joins reject known nominal mapping or active-provider UUID-format mismatches before SQL, but converter-type equality is not behavioral proof. |
| Unsupported join shapes | composite anonymous-object keys, `GroupJoin(...)`, relation-property joins, relation object/collection relation result projection, opaque transparent identifiers, row-local joined post-paging composition, and unsupported result recipe operations fail with `QueryTranslationException` | `Translation/EmployeesJoinTranslationTests.cs`, `Translation/QueryPlanUnsupportedShapeTests.cs` | Outer joins, multi-join pipelines, scalar aggregates beyond joined `Any`/`Count`, and query syntax that projects whole source entities are outside the documented boundary. |
...
6. Unsupported diagnostics keep row-local joined post-paging composition, relation projection, grouped joins, and opaque transparent identifiers outside the supported pushdown boundary.
