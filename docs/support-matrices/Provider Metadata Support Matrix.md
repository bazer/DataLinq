...
| Defaults | Supported with UUID caveats | Supported with UUID caveats | Partially supported | MySQL/MariaDB preserve ordinary typed literals, current date/time/timestamp aliases, and raw expressions through provider-scoped `[DefaultSql]`. An imported exact `UUID()` remains provider-scoped raw SQL rather than being mislabeled UUIDv4/v7. A fixed direct-`Guid` model value can be declared as exact-D `[DefaultGuid("...")]`; parsing normalizes that storage-neutral carrier to an ordinary real-`Guid` default, model regeneration emits canonical lowercase, and resolved UUID codecs render the same value across the covered 13 SQLite/MySQL/MariaDB formats. Regeneration is limited to the carrier or exact base `DefaultAttribute(Guid)` with a null `CodeExpression`; custom Guid-valued default subclasses and expression-carrying defaults fail closed rather than losing source semantics. This is not a new semantic default category, and `[Default("guid-text")]` remains invalid for `Guid`. Direct `[DefaultNewUUID]` Version4/Version7 client initialization works across net8/net9/net10 and evaluates once per new generated mutable instance; Version7 uses UTC Unix milliseconds plus 74 random bits and does not promise same-millisecond monotonicity. Converter-backed defaults and static provider-default import remain open. DDL generation for `[DefaultNewUUID]` intentionally fails closed on SQLite, MySQL, and MariaDB until provider-version capability and physical-storage mappings are verified; explicit provider-scoped `[DefaultSql]` remains the escape hatch for an intentionally accepted expression. UUID version survives direct parsing, direct metadata-to-model regeneration, comparison, snapshots, and equivalence digests. Source/database merge precedence remains incomplete. SQLite preserves ordinary typed literals and current date/time/timestamp defaults, while unsupported expression/blob defaults still warn and skip. |
