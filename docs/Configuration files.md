...
```bash
datalinq create-models -n AppDb --stamp-generated-header
```
...
- `create-models` writes generated files to `DestinationDirectory`.
- When validation or rendering fails before file replacement, `create-models` reports the issues and does not replace generated output.
