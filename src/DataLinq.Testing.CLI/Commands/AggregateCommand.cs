...
internal static class AggregateCommand
{
    public static Command Create(TestInfraCliSettings settings)
    {
        ...
        {
            Description = "Previous successful per-shard case-count baseline to ratchet without rejecting growth."
        };
        ...
        command.SetAction(parseResult => CommandHelpers.ExecuteSafely(() =>
        {
            ...
            var baselineInput = parseResult.GetValue(baselineOption);
            var baseline = baselineInput is null ? null : Path.GetFullPath(baselineInput);
            ...
            var baselineDescription = aggregate.PreviousBaselineCommitSha is null
                ? "source-controlled count floors"
                : $"previous successful baseline {aggregate.PreviousBaselineCommitSha}";
            Console.WriteLine(
                $"Validated {aggregate.Shards.Count} full-matrix shards and {aggregate.TotalCases} cases for " +
                $"{aggregate.CommitSha} against {baselineDescription}.");
        }));
