    ...
    : MetadataFromSqlFactory(options, DatabaseType.MySQL)
{
    ...
    protected void ParseIndices(DatabaseDefinition database, MySqlDatabase<MySQLInformationSchema> informationSchemaDb)
    {
        ...
        foreach (var dbIndexGroup in indexGroups)
        {
            ...
            {
                options.Log?.Invoke($"Warning: Skipping unsupported MySQL {unsupportedIndexReason} index '{dbIndexGroup.First().INDEX_NAME}' on table '{dbIndexGroup.First().TABLE_NAME}'.");
                continue;
            }
            ...
            foreach (var indexColumn in indexedColumns)
            {
                ...
                if (column == null)
                {
                    options.Log?.Invoke($"Warning: Skipping MySQL index '{dbIndexGroup.First().INDEX_NAME}' on table '{dbIndexGroup.First().TABLE_NAME}' because column '{indexColumn.COLUMN_NAME}' was not imported.");
                    skipIndex = true;
                    break;
                }
    ...
    {
        if (indexedColumns.Any(x => string.IsNullOrWhiteSpace(x.COLUMN_NAME) || !string.IsNullOrWhiteSpace(x.EXPRESSION)))
            return "expression";
        ...
        if (indexedColumns.Any(x => string.Equals(x.IS_VISIBLE, "NO", StringComparison.OrdinalIgnoreCase)))
            return "invisible";

        return null;
    }

    protected void ParseRelations(DatabaseDefinition database, MySqlDatabase<MySQLInformationSchema> informationSchemaDb)
    {
    ...
    private static string? TryGetViewDefinitionFromShowCreate(MySqlDatabase<MySQLInformationSchema> informationSchemaDb, string databaseName, string viewName)
    {
        try
        {
