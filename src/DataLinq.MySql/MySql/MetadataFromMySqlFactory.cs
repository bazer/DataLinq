...
public class MetadataFromMySqlFactory : MetadataFromSqlFactory
{
    ...
    protected Option<bool, IDLOptionFailure> ParseIndices(ProviderDatabaseDraft database, MySqlDatabase<MySQLInformationSchema> informationSchemaDb)
    {
        ...
        foreach (var dbIndexGroup in indexGroups)
        {
            ...
            {
                options.Log?.Invoke($"Warning: Skipping unsupported MySQL {unsupportedIndexReason} index '{indexName}' on table '{tableName}'.");
                continue;
            }
            ...
            foreach (var indexColumn in indexedColumns)
            {
                ...
                if (column == null)
                {
                    options.Log?.Invoke($"Warning: Skipping MySQL index '{indexName}' on table '{tableName}' because column '{indexColumn.COLUMN_NAME}' was not imported.");
                    skipIndex = true;
                    break;
                }
