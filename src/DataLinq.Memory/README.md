...
The preview intentionally supports only its capability-gated subset. The current profile contains exactly 57 capability tokens:
...
- a final direct, non-null, converter-free `Int32` scalar projection;
- selectorless `Any` and `Count` over admitted unpaged entity or scalar sequences;
- `Single` and `SingleOrDefault` over an admitted unpaged root entity or direct-`Int32` scalar sequence, optionally using the admitted predicates and exact primary-key ordering; and
- `First` and `FirstOrDefault` over an admitted unpaged root entity or direct-`Int32` scalar sequence with exactly one ascending or descending direct-`Int32` single-column-primary-key ordering and only admitted `Where` operations around it.
