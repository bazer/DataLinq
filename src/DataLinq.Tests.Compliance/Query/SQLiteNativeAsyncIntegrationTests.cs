...
public sealed class SQLiteNativeAsyncIntegrationTests
{
    [Test]
    ...
    public async Task NativeFailuresRetainRequestedQueryAndMutationIdentity(TestProviderDescriptor descriptor)
    {
        ...
        try
        {
            var failure = await Assert.That(async () => { await transaction.InsertAsyncCore(new MutableDepartment { DeptNo = existing.DeptNo, Name = "Duplicate" }); }).Throws<SqliteException>();
        ...
        database.Provider.DatabaseAccess.ExecuteNonQuery("PRAGMA foreign_keys=OFF; DROP TABLE departments; PRAGMA foreign_keys=ON");
        database.Provider.State.ClearCache();
        var count = database.PrepareQuery(0, _ => database.Query().Departments.Count());
        var scalarFailure = await Assert.That(async () => { await count.ExecuteAsyncCore(database, 0); }).Throws<SqliteException>();
        ...
        var rowFailure = await Assert.That(async () => { await Rows(AsyncPlan(database.Query().Departments)); }).Throws<SqliteException>();
    ...
    public async Task MutationsHydratePrivatelyAndPublishOnlyAfterCommit(TestProviderDescriptor descriptor)
    {
        ...
        var cache = database.Provider.State.Cache.GetTableCache(database.Provider.Metadata.GetTableModel(typeof(Department)).Table);
        var committedRows = cache.RowCount;
        ...
        try
        {
            ...
            // Shared-memory SQLite serializes competing table readers. Inspect
            // committed cache publication without opening a blocked outside read.
            await Assert.That(cache.RowCount).IsEqualTo(committedRows);
