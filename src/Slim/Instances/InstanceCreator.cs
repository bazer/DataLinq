...
{
    public static class ImmutableInstanceCreator<T> where T : class, IModel
    {
        private static ProxyGenerator generator = new ProxyGenerator();

        public static T NewInstance(InstanceData instanceData)
        {
            return generator.CreateInterfaceProxyWithoutTarget<T>(new ImmutableInterceptor(instanceData));
        }

        internal class ImmutableInterceptor : IInterceptor
        {
            public InstanceData InstanceData { get; }

            public ImmutableInterceptor(InstanceData instanceData)
            {
