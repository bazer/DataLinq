...
public class MetadataDefinitionFactoryTests
{
    ...
    [Test]
    public async Task Build_GeneratedManyToOneRelationPropertyNameCollision_GeneratesFallbackName()
    {
        var database = CreateRelationTypedDraft(
        ...
        var relationProperty = orderModel.RelationProperties["CustomerRelation"];

        await Assert.That(orderModel.RelationProperties.ContainsKey("CustomerRelation")).IsTrue();
        await Assert.That(MetadataFactory.TryGetGeneratedRelationPropertyFallback(
            relationProperty,
            out var preferredPropertyName,
            out var existingPropertyKind)).IsTrue();
        await Assert.That(preferredPropertyName).IsEqualTo("Customer");
        await Assert.That(existingPropertyKind).IsEqualTo("value property");
    }

    [Test]
    public async Task Build_GeneratedOneToManyRelationPropertyNameCollision_GeneratesFallbackName()
    {
        var database = CreateRelationTypedDraft(
        ...
        var relationProperty = userModel.RelationProperties["OrderRelation"];
        ...
        await Assert.That(MetadataFactory.TryGetGeneratedRelationPropertyFallback(
            ...
            out var preferredPropertyName,
            out var existingPropertyKind)).IsTrue();
        await Assert.That(preferredPropertyName).IsEqualTo("Order");
        await Assert.That(existingPropertyKind).IsEqualTo("value property");
    }

    [Test]
    public async Task Build_MultipleGeneratedRelationPropertyNameCollisions_GeneratesFallbackNames()
    {
        var database = CreateRelationTypedDraft(
            orderAdditionalValueProperties:
            [
        ...
        await Assert.That(MetadataFactory.TryGetGeneratedRelationPropertyFallback(
            orderModel.RelationProperties["CustomerRelation"],
            out var customerPreferredPropertyName,
            out _)).IsTrue();
        await Assert.That(customerPreferredPropertyName).IsEqualTo("Customer");
        await Assert.That(MetadataFactory.TryGetGeneratedRelationPropertyFallback(
            orderModel.RelationProperties["ApproverRelation"],
            out var approverPreferredPropertyName,
            out _)).IsTrue();
        await Assert.That(approverPreferredPropertyName).IsEqualTo("Approver");
    }

    [Test]
    public async Task Build_ExplicitRelationNameAvoidsGeneratedFallbackDiagnostic()
    {
        var database = CreateRelationTypedDraft(
            orderRelationProperties:
            [
        ...
        await Assert.That(orderModel.RelationProperties.ContainsKey("Customer_dl")).IsTrue();
        await Assert.That(MetadataFactory.TryGetGeneratedRelationPropertyFallback(
            orderModel.RelationProperties["Customer_dl"],
            out _,
            out _)).IsFalse();
    }
    ...
    [Test]
    public async Task Build_RelationPropertyWithMismatchedStoredRelationName_ReturnsInvalidModelFailureBeforeSnapshot()
    {
        var database = CreateRelationDraft();
        ParseMetadataIndices(database).ValueOrException();
        ParseMetadataRelations(database).ValueOrException();
        ...
        SetRelationPropertyName(relationProperty, "FK_Other");
        ...
        await Assert.That(failure.FailureType).IsEqualTo(DLFailureType.InvalidModel);
        await Assert.That(failure.Message).Contains("Relation property 'Order.Customer'");
        await Assert.That(failure.Message).Contains("stores relation name 'FK_Other'");
        await Assert.That(failure.Message).Contains("linked to relation 'FK_Order_User'");
    }

    [Test]
    public async Task Build_RelationPropertyWithMismatchedRelationAttributeTarget_ReturnsInvalidModelFailureBeforeSnapshot()
    {
        var database = CreateRelationDraft();
        ParseMetadataIndices(database).ValueOrException();
        ParseMetadataRelations(database).ValueOrException();
        ...
        await Assert.That(failure.FailureType).IsEqualTo(DLFailureType.InvalidModel);
        await Assert.That(failure.Message).Contains("Relation property 'Order.Customer'");
        await Assert.That(failure.Message).Contains("targets columns 'user_name'");
        await Assert.That(failure.Message).Contains("linked to relation side columns 'user_id'");
    }
    ...
    public async Task Build_ForeignKeyRelationPropertyWithMismatchedCsType_ReturnsInvalidModelFailureBeforeSnapshot()
    {
        ...
        var orderModel = database.TableModels.Single(tm => tm.Table.DbName == "orders").Model;
        var relationProperty = orderModel.RelationProperties["Customer"];
        SetPropertyCsType(relationProperty, new CsTypeDeclaration("Order", "TestNamespace", ModelCsType.Class));
        ...
        await Assert.That(failure.FailureType).IsEqualTo(DLFailureType.InvalidModel);
        await Assert.That(failure.Message).Contains("Relation property 'Order.Customer'");
        await Assert.That(failure.Message).Contains("uses C# type 'Order'");
        await Assert.That(failure.Message).Contains("relation side on table 'users' requires 'User'");
    }

    [Test]
    public async Task Build_CandidateKeyRelationPropertyWithMismatchedCsType_ReturnsInvalidModelFailureBeforeSnapshot()
    {
        var database = CreateRelationDraft();
        ParseMetadataIndices(database).ValueOrException();
        ParseMetadataRelations(database).ValueOrException();
        ...
        var userModel = database.TableModels.Single(tm => tm.Table.DbName == "users").Model;
        var relationProperty = userModel.RelationProperties["Order"];
        SetPropertyCsType(
            relationProperty,
            new CsTypeDeclaration("IImmutableRelation<User>", "DataLinq.Instances", ModelCsType.Interface));
        ...
        await Assert.That(result.TryUnwrap(out _, out var failure)).IsFalse();
        await Assert.That(failure.FailureType).IsEqualTo(DLFailureType.InvalidModel);
        await Assert.That(failure.Message).Contains("Relation property 'User.Order'");
        await Assert.That(failure.Message).Contains("uses C# type 'IImmutableRelation<User>'");
        await Assert.That(failure.Message).Contains("relation side on table 'orders' requires 'IImmutableRelation<Order>'");
    }
    ...
    [Test]
    public async Task Build_TableWithNullColumn_ReturnsInvalidModelFailureBeforePrimaryKeyValidation()
    {
        var database = CreateSingleTableDraft(
            ("Id", typeof(int), [new PrimaryKeyAttribute(), new ColumnAttribute("id")]));
        var table = database.TableModels.Single().Table;
        SetTableColumns(table, [table.Columns.Single(), null!]);
        ...
        await Assert.That(result.TryUnwrap(out _, out var failure)).IsFalse();
        await Assert.That(failure.FailureType).IsEqualTo(DLFailureType.InvalidModel);
        await Assert.That(failure.Message).Contains("Table 'items' contains a null column");
    }
    ...
    [Test]
    public async Task Build_ExistingRelationPartRegisteredTwice_ReturnsInvalidModelFailureBeforeSnapshot()
    {
        var database = CreateRelationDraft();
        var users = database.TableModels.Single(tm => tm.Table.DbName == "users");
        var orders = database.TableModels.Single(tm => tm.Table.DbName == "orders");
        ...
        await Assert.That(result.TryUnwrap(out _, out var failure)).IsFalse();
        await Assert.That(failure.FailureType).IsEqualTo(DLFailureType.InvalidModel);
        await Assert.That(failure.Message).Contains("Index 'FK_Broken' on table 'orders'");
        await Assert.That(failure.Message).Contains("duplicate relation part for relation 'FK_Broken'");
    }
    ...
    [Test]
    public async Task Build_ExistingRelationWithSimpleForeignKeyIndex_ReturnsInvalidModelFailureBeforeSnapshot()
    {
        var database = CreateRelationDraft();
        var users = database.TableModels.Single(tm => tm.Table.DbName == "users");
        var orders = database.TableModels.Single(tm => tm.Table.DbName == "orders");
        ...
        var customerId = orders.Table.Columns.Single(column => column.DbName == "customer_id");
        var candidateKeyIndex = new ColumnIndex("users_primary_key", IndexCharacteristic.PrimaryKey, IndexType.BTREE, [userId]);
        var foreignKeyIndex = new ColumnIndex("FK_Broken", IndexCharacteristic.Simple, IndexType.BTREE, [customerId]);
        AddMetadataListItem(users.Table.ColumnIndices, candidateKeyIndex);
        AddMetadataListItem(orders.Table.ColumnIndices, foreignKeyIndex);
        var relation = new RelationDefinition("FK_Broken", RelationType.OneToMany);
        var foreignKeyPart = new RelationPart(foreignKeyIndex, relation, RelationPartType.ForeignKey, "Customer");
        ...
        await Assert.That(failure.FailureType).IsEqualTo(DLFailureType.InvalidModel);
        await Assert.That(failure.Message).Contains("Existing relation 'FK_Broken'");
        await Assert.That(failure.Message).Contains("foreign-key part on index 'FK_Broken'");
        await Assert.That(failure.Message).Contains("marked as 'Simple'");
        await Assert.That(failure.Message).Contains("instead of 'ForeignKey'");
    }

    [Test]
    public async Task Build_ExistingRelationWithSimpleCandidateKeyIndex_ReturnsInvalidModelFailureBeforeSnapshot()
    {
        var database = CreateRelationDraft();
        var users = database.TableModels.Single(tm => tm.Table.DbName == "users");
        var orders = database.TableModels.Single(tm => tm.Table.DbName == "orders");
        var userId = users.Table.Columns.Single(column => column.DbName == "user_id");
        var customerId = orders.Table.Columns.Single(column => column.DbName == "customer_id");
        var candidateKeyIndex = new ColumnIndex("users_candidate", IndexCharacteristic.Simple, IndexType.BTREE, [userId]);
        var foreignKeyIndex = new ColumnIndex("FK_Broken", IndexCharacteristic.ForeignKey, IndexType.BTREE, [customerId]);
        AddMetadataListItem(users.Table.ColumnIndices, candidateKeyIndex);
        AddMetadataListItem(orders.Table.ColumnIndices, foreignKeyIndex);
        var relation = new RelationDefinition("FK_Broken", RelationType.OneToMany);
        ...
        await Assert.That(failure.FailureType).IsEqualTo(DLFailureType.InvalidModel);
        await Assert.That(failure.Message).Contains("Existing relation 'FK_Broken'");
        await Assert.That(failure.Message).Contains("candidate-key part on index 'users_candidate'");
        await Assert.That(failure.Message).Contains("candidate-key indexes must be primary or unique");
    }
    ...
    public async Task Build_ExistingRelationWithUnregisteredCandidateKeyIndex_ReturnsInvalidModelFailureBeforeSnapshot()
    {
        ...
        await Assert.That(failure.FailureType).IsEqualTo(DLFailureType.InvalidModel);
        await Assert.That(failure.Message).Contains("Existing relation 'FK_Broken'");
        await Assert.That(failure.Message).Contains("candidate-key part on index 'users_primary_key'");
        await Assert.That(failure.Message).Contains("not registered on table 'users'");
    }
