...
public sealed class MemoryPlatformCompatibilitySmokeTests
{
    ...
    public async Task SharedRunner_ExecutesCurrentBoundedMemoryProfile()
    {
        ...
        await Assert.That(result.SupportedCapabilityTokenCount).IsEqualTo(58);
        await Assert.That(result.ProjectedNames).IsEquivalentTo([
            "minus-five",
            "seventeen",
            "forty-two"
        ]);
        await Assert.That(result.ProjectedTypedGuidIds).IsEquivalentTo([
            Guid.Parse("00112233-4455-6677-8899-aabbccddeeff")
        ]);
        await Assert.That(result.NotEqualFilteredIds).IsEquivalentTo([42]);
        await Assert.That(result.CompoundFilteredIds).IsEquivalentTo([-5, 42]);
        await Assert.That(result.RangeFilteredIds).IsEquivalentTo([-5, 17]);
        await Assert.That(result.MembershipFilteredIds).IsEquivalentTo([-5, 42]);
        await Assert.That(result.SkippedIds).IsEquivalentTo([17, 42]);
        await Assert.That(result.WindowedIds).IsEquivalentTo([17]);
        await Assert.That(result.SingleEntityId).IsEqualTo(17);
        await Assert.That(result.SingleEntityDefaultIsNull).IsTrue();
        await Assert.That(result.SingleScalarValue).IsEqualTo(3);
        await Assert.That(result.SingleScalarDefaultValue).IsEqualTo(0);
        await Assert.That(result.MultipleSingleRejectedBeforeMaterialization).IsTrue();
        await Assert.That(result.FirstEntityId).IsEqualTo(17);
        await Assert.That(result.FirstEntityDefaultIsNull).IsTrue();
        await Assert.That(result.FirstScalarValue).IsEqualTo(7);
        await Assert.That(result.FirstScalarDefaultValue).IsEqualTo(0);
