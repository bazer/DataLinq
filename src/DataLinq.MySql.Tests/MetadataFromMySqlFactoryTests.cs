...
public class MetadataFromMySqlFactoryTests(DatabaseFixture fixture) : IClassFixture<DatabaseFixture>
{
    ...
    private DataLinqDatabaseConnection DataLinqDatabase(DatabaseType databaseType)
    {
        return _fixture.EmployeeConnections.SingleOrDefault(c => c.Type == databaseType)
            ?? throw new InvalidOperationException($"Database connection not found for {databaseType}. Ensure it's configured in the fixture.");
    }

    // Helper to run the parser with default options
    private DatabaseDefinition ParseEmployeesDb(DatabaseType databaseType, MetadataFromDatabaseFactoryOptions? options = null)
    {
        var factory = MetadataFromSqlFactory.GetSqlFactory(options ?? new MetadataFromDatabaseFactoryOptions(), databaseType);
        var _mySqlConnection = DataLinqDatabase(databaseType);
    ...
    public void TestParseDatabase_Employees_MySql(DatabaseType databaseType)
    {
        // Act
        var dbDefinition = ParseEmployeesDb(databaseType);
        var _mySqlConnection = DataLinqDatabase(databaseType);
    ...
    [Theory]
    [MemberData(nameof(DatabaseTypes))]
    public void TestParseTable_SpecificTable_MySql(DatabaseType databaseType)
    {
        // Arrange
        ...
        // Act
        var dbDefinition = ParseEmployeesDb(databaseType, options);
    ...
    [Theory]
    [MemberData(nameof(DatabaseTypes))]
    public void TestParseColumn_SpecificColumn_MySql(DatabaseType databaseType)
    {
        // Arrange
        var dbDefinition = ParseEmployeesDb(databaseType);
    ...
    [Theory]
    [MemberData(nameof(DatabaseTypes))]
    public void TestParseRelations_MySql(DatabaseType databaseType)
    {
        // Arrange
        var dbDefinition = ParseEmployeesDb(databaseType);
    ...
    [Theory]
    [MemberData(nameof(DatabaseTypes))]
    public void TestParseIndices_MySql(DatabaseType databaseType)
    {
        // Arrange
        var dbDefinition = ParseEmployeesDb(databaseType);
        var departmentsTable = dbDefinition.TableModels.Single(tm => tm.Table.DbName == "departments").Table;
    ...
    [Theory]
    [MemberData(nameof(DatabaseTypes))]
    public void TestViewParsing_MySql(DatabaseType databaseType)
    {
        // Arrange
        var dbDefinition = ParseEmployeesDb(databaseType);
        ...
        Assert.Contains("select", viewDefinition.Definition, StringComparison.OrdinalIgnoreCase);
    ...
    [Theory]
    [MemberData(nameof(DatabaseTypes))]
    public void TestTableFiltering_MySql_TablesOnly(DatabaseType databaseType)
    {
        // Arrange
    ...
    [Theory]
    [MemberData(nameof(DatabaseTypes))]
    public void TestTableFiltering_MySql_ViewsOnly(DatabaseType databaseType)
    {
        // Arrange
    ...
    [Theory]
    [MemberData(nameof(DatabaseTypes))]
    public void TestTableFiltering_MySql_SpecificTablesAndViews(DatabaseType databaseType)
    {
        // Arrange
    ...
    [Theory]
    [MemberData(nameof(DatabaseTypes))]
    public void TestTableFiltering_MySql_MissingTable(DatabaseType databaseType)
    {
        // Arrange
