...
public class MetadataFromInterfacesFactory
{
    ...
    public IEnumerable<DatabaseMetadata> ReadFiles(string csType, IEnumerable<string> srcPaths)
    {
        ...
        var modelSyntaxes = trees
            ...
            .SelectMany(x => x.GetCompilationUnitRoot().DescendantNodes().OfType<TypeDeclarationSyntax>()
                    .Where(cls => cls.BaseList?.Types.Any(baseType => IsModelInterface(baseType.ToString())) == true))
            .ToImmutableArray();

        return ReadSyntaxTrees(modelSyntaxes);
    }

    public IEnumerable<DatabaseMetadata> ReadSyntaxTrees(ImmutableArray<TypeDeclarationSyntax> modelSyntaxes)
    {
        ...
        foreach (var dbType in dbModelClasses)
        {
            ...
            if (database.TableModels.Any(x => x.CsPropertyName == database.CsTypeName))
